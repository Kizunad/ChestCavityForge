...
## 清风轮蛊（Qing Feng Lun）— 风道计数器官

[被动] 轻风缓冲：提供速度、跳跃、防御等基线 Organscore，并每 4 秒自动消耗 60 基础真元维持。
[被动] 风势累积：在地面或空中保持移动时每秒获得 1 层风势（上限 10），停滞 2 秒清空，满层时记录维持时长。
[被动] 御风翔行：四阶起在滑翔状态自动免疫摔落并获得缓降，同时按秒消耗 140000 基础真元。
[主动] 疾风冲刺：6 格直线突进（冷却 6 秒），命中敌人造成 4 点打击与轻度击退，基础消耗 2200。
[主动] 风裂步：冲刺结束 0.25 秒内可触发 5 段风刃（与冲刺共享冷却），基础消耗 38000。
[主动] 风神领域：满风势时可施放 10 秒领域（冷却 45 秒），友军移速 +20%、敌方弹道伤害 -30%，基础消耗 120000 并在持续期每秒追加 20000。

### 计数解锁路线
* 二阶（风轮形）：奔跑里程累计 ≥3000（以米×100 记录）且成功冲刺 ≥50 次后解锁疾风冲刺与风行加速。
* 三阶（破阵风）：冲刺命中 ≥50 次并记录弹道近失 ≥30 次，解锁风裂步与移动闪避判定。
* 四阶（御风轮）：滑翔累计 ≥300 秒且风环成功格挡 ≥25 次，获得免疫摔落、风环护盾与御风翔行。
* 五阶（风神轮）：满层风势维持 ≥12 次且单次滑翔 ≥12 秒累计 ≥10 次，解锁风神领域并启用风势层数增益。
