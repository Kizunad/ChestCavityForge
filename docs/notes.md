...
好！我把“月相被动 → 夜间全属性强化”的方案拉满，并给出可直接落地的数值与实现要点。你可以直接挑这一版进代码。

# 月相被动·「清辉赐福」（夜间）

**触发条件（默认）：** 夜间 + 头顶见天（天空可见 + 天光≤7）。若不见天，仅按 **50%** 生效（可配）。
**PVP 系数：** 乘 **0.75**（可配）。
**白天：** 效果失活（可选：保留 20% 残余，默认关闭）。

---

## 月相 → 强化数值表（`level.getMoonPhase()`：0~7）

| 月相    | 说明     |      最大生命上限 |   伤害吸收（护盾） |    伤害减免 |     移动速度 | 跳跃          |
| ----- | ------ | ----------: | ---------: | ------: | -------: | ----------- |
| 0     | 满月（最强） |    **+30%** | **8**（生命值） | **12%** | **+10%** | 跳跃提升 **II** |
| 1 / 7 | 凸月     |        +22% |          6 |      9% |      +7% | 跳跃提升 I      |
| 2 / 6 | 上/下弦月  |        +14% |          4 |      6% |      +5% | 跳跃提升 I      |
| 3 / 5 | 娥眉月    |         +8% |          2 |      4% |      +3% | 轻度跳跃（I）     |
| 4     | 朔月（最弱） | **-5%**（可关） |          0 |      0% |       0% | 无           |

**说明**

* “最大生命上限”是对 `generic.max_health` 的 **百分比加成**（如玩家基础 20HP，满月变 26HP）。
* “伤害吸收（护盾）”为**可再生月辉护盾**（见下）。
* “伤害减免”对**剩余伤害**做乘法减免（吸收后再结算）。
* “移动速度”为 `generic.movement_speed` 乘法加成。
* “跳跃”使用 **跳跃提升**（MobEffect：Jump Boost），I=+0.5格左右，II≈+1格。
* 朔月减益默认 **-5% 最大生命**；可在配置中改为 **0**（无惩罚）。

---

## 月辉护盾（Lunar Ward）

* **容量**：如表（2/4/6/8 HP，随月相变化）。
* **再生**：离战 **2s** 后，每 **0.5s** 恢复 **1 HP**，直到上限；受“见天”与 PVP 系数影响。
* **破盾惩罚**：护盾被打空后 **1.5s** 内不再生。
* **结算顺序**：先扣护盾 → 再做“伤害减免%” → 再扣真实生命。
* **链接通道**：`Linkage.channel("guzhenren:lunar_ward")` 可供其他器官读取（如与雷道/光道联动）。

---

## 细节与边界

* **室内/洞穴**：按 50% 生效（可配 25%/75%）。
* **水中/雨天**：不影响判定，仅取决于“见天”。
* **叠加关系**：
  * 最大生命为独立 AttributeModifier（稳定 UUID）；
  * 伤害减免为自定义 IncomingDamage Hook 百分比缩放（避免与抗性药水冲突叠爆）；
  * 跳跃用效果，不与鞋类附魔硬冲突；
  * 速度为乘法修饰，和“疾行”等可并存但做软上限（建议总速 ≤ +30%）。
* **卸下器官**：即时移除所有修饰与护盾，吸收量清零。
...
```java
// 0..7 → 数据表
record LunarTier(float hpPct, int wardCap, float drPct, float spdPct, int jumpAmp) {}
static final LunarTier[] TIERS = {
  new LunarTier(0.30f, 8, 0.12f, 0.10f, 1), // 0 满月（Jump II → amp=1）
  new LunarTier(0.22f, 6, 0.09f, 0.07f, 0), // 1
  new LunarTier(0.14f, 4, 0.06f, 0.05f, 0), // 2
  new LunarTier(0.08f, 2, 0.04f, 0.03f, 0), // 3
  new LunarTier(-0.05f,0, 0.00f, 0.00f,-1), // 4 朔月（无跳跃）
  new LunarTier(0.08f, 2, 0.04f, 0.03f, 0), // 5
  new LunarTier(0.14f, 4, 0.06f, 0.05f, 0), // 6
  new LunarTier(0.22f, 6, 0.09f, 0.07f, 0)  // 7
};
```

> 注：`jumpAmp=1` 表示 Jump Boost II（因为放大系数从 0 开始计）；朔月若关惩罚，就把 `hpPct` 设 0。

**B. 条件判定**

* 夜间：`level.isNight()`；
* 见天：`level.canSeeSkyFromBelowWater(player.blockPosition().above()) && player.level().getMaxLocalRawBrightness(pos) <= 7`;
* 不满足时将 `scale = 0.5f` 应用到全部数值（PVP 再乘 0.75）。

**C. 属性/效果施加（稳定 UUID）**

* `MAX_HEALTH`、`MOVEMENT_SPEED` 用 AttributeInstance 永久修饰（当夜间→见天时添加，离开条件时移除/改值）。
* 跳跃：`MobEffects.JUMP` 动态加减（每 40~60 tick 刷新一次，防止闪烁）。
* 伤害减免：在 `LivingIncomingDamageEvent` 上读取当前 `drPct`，做 `amount *= (1 - drPct)`。
* 月辉护盾：

  * 用 `Capability` 或 `Linkage` 存 `wardCap / wardNow / lastHitTs`；
  * 在 `PlayerTick` 的 slowTick 中做再生；
  * 在 `LivingHurtEvent` 先吃护盾；吃完再进入减伤。

**D. 可配置项（`common.toml`）**

* `pvpMultiplier`（默认 0.75）
* `indoorsMultiplier`（默认 0.5）
* `newMoonPenaltyEnabled`（默认 true）
* `maxSpeedBonusCap`（默认 0.30）

---

## 与其它器官的自然联动（可选）

* **雷盾蛊**：当“月辉护盾”吸收伤害时，若雷盾存在且未破，回充雷盾 **+2%**（1s 内置 CD）。
* **小光蛊/光系**：满月夜间，光系主动结束时额外恢复 **+1** 护盾。
* **寿蛊**：满月时额外 +2 临时生命（随夜消失，不可叠）。
