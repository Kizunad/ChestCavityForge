...
class GuScriptCompilerTest {
  ...
  private static final ResourceLocation TEST_ITEM_ID =
      ResourceLocation.fromNamespaceAndPath("chestcavity", "test_leaf");

  @BeforeEach
  void setUpRegistry() {
    GuScriptRegistry.updateLeaves(
        Map.of(
            TEST_ITEM_ID,
            new GuScriptRegistry.LeafDefinition("骨道叶", ImmutableMultiset.of("骨道"), List.of())));

    ReactionRule doubleBone =
        ReactionRule.builder("chestcavity:test_double_bone")
            .arity(1)
  ...
  void toScaledLeaf_multipliesTagsByCount() {
    ...
    GuScriptReducer.ReductionResult result =
        reducer.reduce(List.of(scaled), GuScriptRegistry.reactionRules());
  ...
  void computeSignature_ignoresFlowParamOrdering() throws Exception {
    GuScriptPageState forward =
        newPageState(
            Map.of(
                "alpha", "1",
                "beta", "2"));

    GuScriptPageState reversed =
        newPageState(
            Map.of(
                "beta", "2",
                "alpha", "1"));
    ...
    Method computeSignature =
        GuScriptCompiler.class.getDeclaredMethod("computeSignature", GuScriptPageState.class);
    ...
    assertEquals(
        forwardSignature, reversedSignature, "Flow parameter ordering should not change signature");
  }
  ...
  private static GuScriptPageState allocatePageState() throws Exception {
    ...
    Constructor<GuScriptPageState> serializationCtor =
        (Constructor<GuScriptPageState>)
