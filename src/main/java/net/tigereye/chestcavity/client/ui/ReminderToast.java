...
public class ReminderToast implements Toast {
    ...
    public static void show(String title, String message, ResourceLocation texture) {
        ...
        if (mc != null && mc.getToasts() != null) {
            mc.getToasts().addToast(new ReminderToast(Component.literal(title), Component.literal(message), texture, ItemStack.EMPTY, 3000L));
        }
    }
    ...
    public static void showItem(String title, String message, ItemStack item) {
        Minecraft mc = Minecraft.getInstance();
        if (mc != null && mc.getToasts() != null) {
            mc.getToasts().addToast(new ReminderToast(Component.literal(title), Component.literal(message), null, item, 3000L));
        }
    }

    public ReminderToast(Component title, Component message, ResourceLocation texture, ItemStack itemIcon, long durationMs) {
        this.title = title;
