...
/**
 * Fragment that renders a Modern UI surface bound to a {@link TestModernUIContainerMenu}.
 * Uses individual {@link MinecraftSurfaceView} instances per slot to bridge item rendering
 * while delegating item transfer logic to the underlying container menu.
 */
public class TestModernUIContainerFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        subtitle.setText("Drag items between grids to verify Modern UI + menu sync.\nShift-click for quick move.");
        ...
        column.addView(buildStorageGrid(root), new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT));

        addSpacer(column, root.dp(12));

        column.addView(buildPlayerInventoryGrid(root), new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT));

        addSpacer(column, root.dp(6));

        column.addView(buildHotbarRow(root), new LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                ViewGroup.LayoutParams.WRAP_CONTENT));
    ...
    private final class SlotView extends FrameLayout {
        ...
        SlotView(@NonNull icyllis.modernui.core.Context context, Slot slot, int sizePx) {
            ...
            setOnClickListener(v -> {
                boolean quickMove = Minecraft.getInstance().player != null
