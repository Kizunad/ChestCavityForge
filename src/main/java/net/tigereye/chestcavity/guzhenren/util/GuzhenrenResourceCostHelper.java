...
/**
 * Centralised helper for consuming Guzhenren resources. Prefers draining zhenyuan/jingli from
 * players via {@link GuzhenrenResourceBridge} and falls back to a health payment for other entities
 * when requested.
 *
 * <p>Relocated from the compat namespace alongside the new guzhenren util module.
 */
public final class GuzhenrenResourceCostHelper {
  ...
  private static volatile boolean debugEnabled =
      Boolean.getBoolean("chestcavity.guzhenren.resources.debug");
  ...
  public static ConsumptionResult consumeStrict(
      LivingEntity entity, double baseZhenyuanCost, double baseJingliCost) {
    return consume(
        entity, baseZhenyuanCost, baseJingliCost, DEFAULT_RESOURCE_TO_HEALTH_RATIO, false);
  }

  public static ConsumptionResult consumeWithFallback(
      LivingEntity entity, double baseZhenyuanCost, double baseJingliCost) {
    return consume(
        entity, baseZhenyuanCost, baseJingliCost, DEFAULT_RESOURCE_TO_HEALTH_RATIO, true);
  }

  /**
   * Consumes hunpo (魂魄) from the given entity. - For players with Guzhenren attachment, deducts
   * from the {@code hunpo} field (clamped to [0, max]). - For non-players, optionally falls back to
   * draining health if {@code allowHealthFallback} is true.
   *
   * <p>The health fallback uses the same {@link #DEFAULT_RESOURCE_TO_HEALTH_RATIO} as
   * zhenyuan/jingli.
   */
  public static ConsumptionResult consumeHunpo(
      LivingEntity entity, double hunpoCost, boolean allowHealthFallback) {
  ...
  private static ConsumptionResult consumeForPlayer(
    ...
    if (handleOpt.isPresent()) {
      ...
      try (ResourceTransaction transaction = new ResourceTransaction(player, handle)) {
        ...
        debug(
            "consumeForPlayer succeeded for {}: zhenyuanSpent={}, jingliSpent={}",
            player.getName().getString(),
            result.zhenyuanSpent(),
            result.jingliSpent());
        return result;
      }
    }
  ...
  private static ConsumptionResult consumeWithHealth(
      LivingEntity entity, double zhenyuanCost, double jingliCost, double ratio) {
  ...
  public record ConsumptionResult(
    ...
    public static ConsumptionResult successWithResources(double zhenyuanSpent, double jingliSpent) {
      return new ConsumptionResult(
          true, zhenyuanSpent, jingliSpent, 0.0f, Mode.PLAYER_RESOURCES, FailureReason.NONE);
    }

    public static ConsumptionResult successWithHealth(float healthSpent) {
      return new ConsumptionResult(
          true, 0.0, 0.0, healthSpent, Mode.HEALTH_FALLBACK, FailureReason.NONE);
    }
    ...
    boolean executed =
        withHandle(
            ...
            handle -> {
              ...
              debug(
                  "Refunded resources for {}: zhenyuan={}, jingli={}",
                  player.getName().getString(),
                  result.zhenyuanSpent(),
                  result.jingliSpent());
            });
  ...
  public static boolean drainHealth(
    ...
    if (reserve > 0.0f) {
      if (postDrain < reserve) {
        debug(
            "Health drain rejected for {} (available={}, amount={}, reserve={})",
    ...
    debug(
        "Drained health for {}: amount={}, remainingHealth={}, remainingAbsorption={}",
        entity.getName().getString(),
        amount,
        entity.getHealth(),
  ...
  public static boolean drainHealth(LivingEntity entity, float amount) {
    return drainHealth(
        entity, amount, 0.0f, entity == null ? null : entity.damageSources().generic());
  }
  ...
  private static final class ResourceTransaction implements AutoCloseable {
    ...
    private FailureReason spendZhenyuan(double baseCost) {
      ...
      zhenyuanSpent += spent;
      debug(
          "{} consumed zhenyuan: baseCost={}, spent={}, remaining={}",
          player.getName().getString(),
          cost,
          spent,
          after);
    ...
    private FailureReason spendJingli(double baseCost) {
      ...
      try {
        if (player != null
            && player.hasEffect(
                net.tigereye.chestcavity.registration.CCStatusEffects.HLTN_STAMINA_REDUCE)) {
          // 采用行为级配置，若未提供则默认0.7
          double multiplier =
              net.tigereye.chestcavity.compat.guzhenren.util.behavior.BehaviorConfigAccess.getFloat(
                  net.tigereye.chestcavity.compat.guzhenren.item.li_dao.behavior
                      .HuangLuoTianNiuGuOrganBehavior.class,
                  "ACTIVE_STAMINA_REDUCTION_MULTIPLIER",
                  0.7f);
      ...
      jingliSpent += spent;
      debug(
          "{} consumed jingli: cost={}, spent={}, remaining={}",
          player.getName().getString(),
          cost,
          spent,
          after);
    ...
    private void rollback() {
      ...
      if (zhenyuanSpent > 0.0 || jingliSpent > 0.0) {
        debug(
            "Rolled back transaction for {} (zhenyuan={}, jingli={})",
            player.getName().getString(),
            zhenyuanSpent,
            jingliSpent);
      }
