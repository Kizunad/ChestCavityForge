...
final class GuzhenrenClientNetworkBridge {
    ...
    static void bootstrap() {
        LOGGER.debug("[compat/guzhenren][network/client] bootstrap (installed={}, bridgeAvailable={})",
                installed, GuzhenrenResourceBridge.isAvailable());
        if (installed) {
            LOGGER.debug("[compat/guzhenren][network/client] bootstrap aborted: already installed");
            return;
        }
        if (!GuzhenrenResourceBridge.isAvailable()) {
            LOGGER.debug("[compat/guzhenren][network/client] bootstrap aborted: Guzhenren bridge unavailable");
            return;
        }
        synchronized (GuzhenrenClientNetworkBridge.class) {
            if (installed) {
                LOGGER.debug("[compat/guzhenren][network/client] bootstrap abort inside sync: installed flag set");
                return;
            }
            try {
                installInterceptor();
                ...
                LOGGER.warn("[compat/guzhenren][network/client] Failed to hook Guzhenren player variable payload", throwable);
            }
        }
    }
    ...
    static boolean registerListener(GuzhenrenPayloadListener listener) {
        ...
        LOGGER.debug("[compat/guzhenren][network/client] listener {} registered? {} (total={})",
    ...
    static boolean unregisterListener(GuzhenrenPayloadListener listener) {
        ...
        LOGGER.debug("[compat/guzhenren][network/client] listener {} removed? {} (total={})",
    ...
    private static void installInterceptor() throws Exception {
        ...
        if (existing == null) {
            LOGGER.warn("[compat/guzhenren][network/client] PlayerVariablesSyncMessage not present in Guzhenren message map");
            return;
        }
        ...
        if (originalHandler == null) {
            LOGGER.warn("[compat/guzhenren][network/client] PlayerVariablesSyncMessage handler unavailable");
            return;
        }
        ...
        IPayloadHandler<CustomPacketPayload> wrapper = new IPayloadHandler<>() {
            ...
            public void handle(CustomPacketPayload payload, IPayloadContext context) {
                ...
                if (context.flow() != PacketFlow.CLIENTBOUND) {
                    LOGGER.trace("[compat/guzhenren][network/client] payload ignored due to flow {}", context.flow());
                    return;
                }
                Player player = context.player();
                if (player == null || !player.level().isClientSide()) {
                    LOGGER.trace("[compat/guzhenren][network/client] payload ignored (player null or not clientside)");
                    return;
                }
                LOGGER.trace("[compat/guzhenren][network/client] payload received for {}", player.getScoreboardName());
        ...
        LOGGER.debug("[compat/guzhenren][network/client] installed wrapper handler {} for payload {}",
    ...
    private static void dispatch(Player player) {
        GuzhenrenResourceBridge.open(player).ifPresent(handle -> {
            ...
            if (snapshot.isEmpty()) {
                LOGGER.trace("[compat/guzhenren][network/client] dispatch aborted: empty snapshot for {}", player.getScoreboardName());
                return;
            }
            LOGGER.trace("[compat/guzhenren][network/client] dispatching snapshot with {} keys for {}", snapshot.size(), player.getScoreboardName());
            ...
            for (GuzhenrenPayloadListener listener : LISTENERS) {
                try {
                    ...
                    LOGGER.warn("[compat/guzhenren][network/client] payload listener threw", throwable);
                }
            }
        });
    }
