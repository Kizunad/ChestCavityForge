...
/**
 * Entry point for Guzhenren player-variable interception. Client-only logic lives in {@link
 * GuzhenrenClientNetworkBridge}; this wrapper keeps the dedicated server classpath clean.
 */
  ...
  public static boolean registerListener(GuzhenrenPayloadListener listener) {
    if (!FMLEnvironment.dist.isClient()) {
      ChestCavity.LOGGER.debug(
          "[compat/guzhenren][network] listener registration skipped: non-client dist");
      return false;
    }
