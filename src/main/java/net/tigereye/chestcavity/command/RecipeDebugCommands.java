...
public final class RecipeDebugCommands {
  ...
  public static void register(RegisterCommandsEvent event) {
    ...
    dispatcher.register(
        Commands.literal("cc_dump_recipes")
            .requires(src -> src.hasPermission(2))
            .executes(ctx -> dumpNamespace(ctx.getSource(), "chestcavity"))
            .then(
                Commands.argument(
                        "namespace", com.mojang.brigadier.arguments.StringArgumentType.string())
                    .executes(
                        ctx ->
                            dumpNamespace(
                                ctx.getSource(),
  ...
  private static int dumpNamespace(CommandSourceStack source, String namespace) {
    ...
    List<ResourceLocation> ids =
        rm.getAllRecipesFor(RecipeType.CRAFTING).stream()
            ...
            .filter(
                id ->
                    namespace == null || namespace.isBlank() || id.getNamespace().equals(namespace))
    ...
    source.sendSuccess(
        () ->
            Component.literal(String.format(Locale.ROOT, "命名空间 '%s' 的合成配方数: %d", namespace, total)),
        false);
    ...
    source.sendSuccess(
        () -> Component.literal(String.format(Locale.ROOT, "全部合成配方总数: %d", craftingTotal)), false);
