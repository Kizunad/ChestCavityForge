...
public final class LeafGuNode implements GuNode {
  ...
  public LeafGuNode(
      String name, Multiset<String> tags, List<Action> actions, int pageIndex, int slotIndex) {
    this(name, tags, actions, pageIndex < 0 ? null : pageIndex, slotIndex < 0 ? null : slotIndex);
  }
  ...
  public String toString() {
    return "LeafGuNode{"
        ...
        + actions
        + ", pageIndex="
        + (pageIndex == null ? "<unset>" : pageIndex)
        + ", slotIndex="
        + (slotIndex == null ? "<unset>" : slotIndex)
        + '}';
  }
}
