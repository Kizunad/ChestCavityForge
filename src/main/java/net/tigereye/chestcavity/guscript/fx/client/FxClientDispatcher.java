  ...
  public static void handle(FxEventPayload payload) {
    ...
    Vec3 target =
        payload.hasTarget()
            ...
            : null;
    FxRegistry.FxContext context =
        new FxRegistry.FxContext(
            origin,
            fallback,
            look,
            target,
  ...
  public static void play(ResourceLocation id, FxRegistry.FxContext context) {
    ...
    for (FxDefinition.FxModule module : definition.modules()) {
      if (module instanceof SoundModule soundModule) {
        ...
        FxClientHooks.addScreenShake(
            screenShake.intensity() * context.intensity(), screenShake.durationTicks());
      }
    }
  }

  private static void playSound(
      ClientLevel level, FxRegistry.FxContext context, SoundModule module) {
    ...
    level.playLocalSound(
        ...
        event,
        SoundSource.PLAYERS,
        module.volume() * context.intensity(),
        module.pitch(),
        false);
  }

  private static void spawnParticles(
      ClientLevel level, FxRegistry.FxContext context, ParticleModule module, RandomSource random) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      level.addParticle(
          options, origin.x + dx, origin.y + dy, origin.z + dz, dx * speed, dy * speed, dz * speed);
    }
  }

  private static void spawnTrail(
      ClientLevel level, FxRegistry.FxContext context, TrailModule module, RandomSource random) {
    ParticleOptions options = resolveParticleOptions(module.settings());
    if (options == null) {
      return;
    }
    ...
    if ("minecraft:dust_color_transition".equals(particleKey)) {
      if (settings.primaryColor() == null || settings.secondaryColor() == null) {
        ChestCavity.LOGGER.warn(
            "[GuScript] dust_color_transition requires from_color and to_color in FX definition");
        return null;
      }
      float size = settings.size() <= 0.0F ? 1.0F : settings.size();
      return new DustColorTransitionOptions(
          colorToVector(settings.primaryColor()), colorToVector(settings.secondaryColor()), size);
    }
    ChestCavity.LOGGER.warn(
        "[GuScript] Unsupported particle type {} for FX", settings.particleId());
    return null;
  }
