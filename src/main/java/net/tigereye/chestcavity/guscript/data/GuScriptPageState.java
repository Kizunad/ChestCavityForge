...
public final class GuScriptPageState {
  ...
  /**
   * Creates a deep copy of this page, including item stacks and metadata but excluding cached
  ...
  public void setBindingTarget(BindingTarget target) {
    ...
    if (this.bindingTarget != target) {
      ChestCavity.LOGGER.debug(
          "[GuScript] Page binding target changed: {} -> {}", this.bindingTarget, target);
  ...
  public void setListenerType(ListenerType type) {
    ...
    if (this.listenerType != type) {
      ChestCavity.LOGGER.debug(
          "[GuScript] Page listener type changed: {} -> {}", this.listenerType, type);
  ...
  public static GuScriptPageState load(
      CompoundTag tag, HolderLookup.Provider provider, int expectedRows) {
