...
final class VariableFlowActions {
    ...
    static FlowEdgeAction addVariableFromVariable(String targetName, String sourceName, double scale, boolean asDouble) {
        return new FlowEdgeAction() {
            @Override
            public void apply(Player performer, LivingEntity target, FlowController controller, long gameTime) {
    ...
    static FlowEdgeAction clampVariable(String name, double min, double max, boolean asDouble) {
        ...
        return new FlowEdgeAction() {
            ...
            public void apply(Player performer, LivingEntity target, FlowController controller, long gameTime) {
                ...
                if (asDouble) {
                    controller.setDouble(name, net.minecraft.util.Mth.clamp(controller.getDouble(name, 0.0D), lower, upper));
                } else {
                    long current = controller.getLong(name, 0L);
                    long clamped = Math.max((long) Math.ceil(lower), Math.min((long) Math.floor(upper), current));
    ...
    static FlowEdgeAction setVariableFromParam(String paramKey, String variableName, double defaultValue) {
        return new FlowEdgeAction() {
            @Override
            public void apply(Player performer, LivingEntity target, FlowController controller, long gameTime) {
    ...
    static FlowEdgeAction copyVariable(String sourceName, String targetName, double scale, double offset, boolean asDouble) {
        return new FlowEdgeAction() {
            @Override
            public void apply(Player performer, LivingEntity target, FlowController controller, long gameTime) {
