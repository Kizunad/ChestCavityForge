  ...
  public static final class FlowMirror {
    ...
    void update(FlowSyncPayload payload) {
      ...
      ChestCavity.LOGGER.debug(
          "[Flow][Client] Entity {} program {} -> {} (t={} start={})",
          entityId,
          payload.programId(),
          payload.state(),
          payload.ticksInState(),
          payload.stateGameTime());
      if (Minecraft.getInstance().player != null
          && Minecraft.getInstance().player.getId() == entityId) {
    ...
    private void playEnterFx(FlowSyncPayload payload) {
      ...
      Vec3 origin =
          new Vec3(living.getX(), living.getY() + living.getBbHeight() * 0.5D, living.getZ());
      Vec3 look = living.getLookAngle();
      FxRegistry.FxContext context =
          new FxRegistry.FxContext(origin, look, look, null, 1.0F, living.getId(), -1);
