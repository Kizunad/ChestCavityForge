...
/**
 * Compiles the contents of a GuScript page into executable AST roots, caching the results when
 * possible.
 */
public final class GuScriptCompiler {
  ...
  public static GuScriptProgramCache compileIfNeeded(GuScriptAttachment attachment, long gameTime) {
    return compilePageIfNeeded(
        attachment, attachment.activePage(), attachment.getCurrentPageIndex(), gameTime);
  }

  public static GuScriptProgramCache compilePageIfNeeded(
      GuScriptAttachment attachment, GuScriptPageState page, int pageIndex, long gameTime) {
    ...
    if (!leaves.isEmpty()) {
      ...
      ChestCavity.LOGGER.info(
          ...
          page.listenerType(),
          roots.size(),
          roots.stream().map(node -> node.kind() + ":" + node.name()).toList());
    } else {
      ChestCavity.LOGGER.info(
  ...
  static LeafGuNode toScaledLeaf(
      GuScriptRegistry.LeafDefinition definition, int count, int pageIndex, int slotIndex) {
    ...
    return new LeafGuNode(
        definition.name(), scaledTags, definition.actions(), pageIndex, slotIndex);
  }
