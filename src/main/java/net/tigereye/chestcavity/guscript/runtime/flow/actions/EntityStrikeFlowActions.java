...
final class EntityStrikeFlowActions {
  ...
  static FlowEdgeAction entityStrike(
    ...
    ResourceLocation resolvedSound =
        soundId == null
    ...
    return new FlowEdgeAction() {
      ...
      public void apply(
        ...
        LivingEntity striker =
            resolveStrikeSource(server, performer, controller, entityIdVariable, radius);
        ...
        Vec3 finalPosition =
            computeEntityStrikePosition(
                performer.position(), performer.getYRot(), offset, additionalYaw, dash);
  ...
  static Vec3 computeEntityStrikePosition(
    ...
    Vec3 dashVector =
        Vec3.directionFromRotation(0.0F, performerYaw + yawOffset).scale(dashDistance);
  ...
  private static LivingEntity resolveStrikeSource(
    ...
    List<LivingEntity> allies =
        server.getEntitiesOfClass(
            LivingEntity.class, box, entity -> FlowActionUtils.isAlly(performer, entity));
    if (allies.isEmpty()) {
      return null;
    }
  ...
  private static void playStrikeSound(
      ServerLevel level, LivingEntity striker, ResourceLocation soundId) {
