...
public final class GuScriptReducer {
  ...
  public ReductionResult reduce(List<GuNode> leafNodes, List<ReactionRule> rules) {
    ...
    while (iterations++ < MAX_ITERATIONS) {
      Optional<ApplicationCandidate> candidate =
          findBestApplication(pool, sortedRules, failedCombos, preferUiOrder);
      ...
      if (maybeNode.isEmpty()) {
        ChestCavity.LOGGER.warn(
            "[GuScript] Rule {} refused nodes {}, skipping", best.rule.id(), best.selectedNodes);
      ...
      if (result instanceof OperatorGuNode operator) {
        ...
        result =
            operator.withOrderingMetadata(
                primaryIndex, adjacencySpan, best.rule.priority(), pageIndex, sequence);
      }
    ...
    if (iterations >= MAX_ITERATIONS) {
      ChestCavity.LOGGER.warn(
          "[GuScript] Reduction aborted after {} iterations to prevent infinite loop",
          MAX_ITERATIONS);
    }
  ...
  private static Optional<ApplicationCandidate> findBestApplication(
      List<GuNode> pool,
      List<ReactionRule> rules,
      Set<FailureKey> failures,
      boolean preferUiOrder) {
    ApplicationCandidate best = null;
    for (ReactionRule rule : rules) {
      Optional<ApplicationCandidate> candidate =
          findApplicationForRule(pool, rule, failures, preferUiOrder);
      if (candidate.isEmpty()) {
        continue;
      }
  ...
  private static Optional<ApplicationCandidate> findApplicationForRule(
      List<GuNode> pool, ReactionRule rule, Set<FailureKey> failures, boolean preferUiOrder) {
  ...
  private static void search(
    ...
    if (depth == rule.arity()) {
      ...
      int adjacencySpan =
          indices.isEmpty()
              ? UNKNOWN_SLOT_INDEX
              : (indices.get(indices.size() - 1) - indices.get(0));
      ...
      ApplicationCandidate candidate =
          new ApplicationCandidate(
              ...
              selected,
              ImmutableMultiset.copyOf(tagCounts),
              coverageScore(tagCounts, rule),
              adjacencySpan,
              primarySlotIndex,
              pageIndex,
  ...
  private static boolean coversRequired(
      Multiset<String> tagCounts, Multiset<String> requiredCounts) {
  ...
  private static GuNode adjustCompositeKind(GuNode node) {
    if (node instanceof OperatorGuNode operator) {
      if (operator.kind() == GuNodeKind.OPERATOR
          && operator.children().stream().noneMatch(GuNode::isLeaf)) {
  ...
  private record ApplicationCandidate(
      ReactionRule rule,
      List<Integer> selectedIndices,
      List<GuNode> selectedNodes,
      ImmutableMultiset<String> tagCounts,
      int coverageScore,
      int adjacencySpan,
  ...
  private record FailureKey(String ruleId, List<Integer> nodeIdentity) {
    static FailureKey from(ReactionRule rule, List<GuNode> nodes) {
      List<Integer> ids = nodes.stream().map(System::identityHashCode).sorted().toList();
