...
final class SlashFlowActions {
  ...
  static FlowEdgeAction slashArea(double radius, double damage, double breakPower) {
    ...
    return new FlowEdgeAction() {
      ...
      public void apply(
        ...
        if (controller != null) {
          resolvedDamage =
              resolveDamageOverride(
                  controller, SwordSlashConstants.FLOW_PARAM_DAMAGE_AREA, resolvedDamage);
        }
  ...
  static FlowEdgeAction slashDelayedRay(
    ...
    return new FlowEdgeAction() {
      ...
      public void apply(
        ...
        if (controller != null) {
          resolvedDamage =
              resolveDamageOverride(
                  controller, SwordSlashConstants.FLOW_PARAM_DAMAGE_RAY, resolvedDamage);
        }
        final double damageForRay = resolvedDamage;
        Runnable task =
            () ->
                runLockedSlashRay(
                    server, performerId, start, direction, len, damageForRay, bp, radius);
        if (controller != null) {
  ...
  private static void slashBreakArea(
      ServerLevel server, Player performer, double radius, double damage, double breakPower) {
  ...
  private static void runSlashRay(
      ServerLevel server,
      UUID performerId,
      double length,
      double damage,
  ...
  private static List<LivingEntity> slashDamageEntities(
      ServerLevel server, Player performer, AABB area, double damage) {
  ...
  private static List<LivingEntity> slashDamageEntities(
    ...
    List<LivingEntity> candidates =
        server.getEntitiesOfClass(
            LivingEntity.class, area, entity -> canDamageSlashTarget(performer, entity));
    ...
    for (LivingEntity entity : candidates) {
      if (rayStart != null && rayEnd != null && rayRadius > 0.0D) {
        double distanceSq =
            distanceToSegmentSquared(entity.getBoundingBox().getCenter(), rayStart, rayEnd);
  ...
  // ✅ 新方法：在施法瞬间锁定起点与方向
  private static void runLockedSlashRay(
      ServerLevel server,
      UUID performerId,
      Vec3 start,
      Vec3 direction,
    ...
    ChestCavity.LOGGER.info(
        ...
        hits.isEmpty()
            ? "-"
            : hits.stream()
                .map(SlashFlowActions::describeEntity)
  ...
  private static double resolveDamageOverride(
      FlowController controller, String key, double fallback) {
