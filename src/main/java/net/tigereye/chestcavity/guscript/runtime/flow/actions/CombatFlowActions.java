...
final class CombatFlowActions {
  ...
  static FlowEdgeAction areaEffect(
      ...
      public void apply(
        ...
        List<LivingEntity> entities =
            level.getEntitiesOfClass(
                LivingEntity.class,
                box,
                entity -> entity.isAlive() && (includeSelf || entity != performer));
        for (LivingEntity entity : entities) {
          ...
          entity.addEffect(
              new MobEffectInstance(
                  effect, actualDuration, Math.max(0, amplifier), false, showParticles, showIcon));
        }
      }
  ...
  static FlowEdgeAction dampenProjectiles(
    ...
    return new FlowEdgeAction() {
      ...
      public void apply(
        ...
        AABB box = new AABB(origin, origin).inflate(resolvedRadius);
        int remaining = cap;
        for (AbstractArrow arrow :
            level.getEntitiesOfClass(AbstractArrow.class, box, entity -> entity.isAlive())) {
        ...
        for (ThrowableProjectile projectile :
            level.getEntitiesOfClass(ThrowableProjectile.class, box, entity -> entity.isAlive())) {
  ...
  static FlowEdgeAction highlightHostiles(double radius, String radiusVariable, int durationTicks) {
    ...
    return new FlowEdgeAction() {
      ...
      public void apply(
        ...
        List<LivingEntity> entities =
            level.getEntitiesOfClass(
                LivingEntity.class, box, entity -> entity instanceof Enemy && entity.isAlive());
        for (LivingEntity hostile : entities) {
          hostile.addEffect(
              new MobEffectInstance(MobEffects.GLOWING, duration, 0, false, false, false));
        }
      }
