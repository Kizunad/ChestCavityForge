...
/**
 * If-guard that attempts to right-click use a specific item (by id) and, if successful,
 * applies an ender-pearl-like cooldown to that item for the performer, then runs nested actions.
 *
 * Parameters:
 * - itemId: namespaced id, e.g. "guzhenren:ling_xian_gu"（灵涎蛊）
 * - cooldownTicks: cooldown ticks to apply after successful use (default 40 = 2s)
 * - preferOffhand: if true and offhand already holds the item, use OFF_HAND first
 * - actions: nested actions to execute only when use succeeds
 */
public record IfItemUseAction(ResourceLocation itemId, int cooldownTicks, boolean preferOffhand, List<Action> actions) implements Action {
    ...
    private static Pair<InteractionHand, Integer> chooseHandAndSlot(Player p, Item item, boolean preferOffhand) {
