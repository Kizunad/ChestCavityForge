...
public class EmitProjectileAction implements Action {
  ...
  public record ParameterSource(
      String flowParam, Double constantValue, Double defaultValue, boolean integerHint) {
    ...
    private double resolve(GuScriptContext context) {
      if (flowParam != null && !flowParam.isBlank()) {
        double fallback =
            defaultValue != null ? defaultValue : (constantValue != null ? constantValue : 0.0D);
