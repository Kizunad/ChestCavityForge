...
public final class GuScriptFlowLoader extends SimpleJsonResourceReloadListener {
  ...
  private static FlowStateDefinition parseStateDefinition(JsonObject json) {
    ...
    return new FlowStateDefinition(
        enterActions, enterFx, transitions, updateActions, Math.max(0, updatePeriod));
  }
  ...
  private static FlowGuard parseGuard(JsonObject json) {
    ...
    return switch (type) {
      case "resource" -> FlowGuards.hasResource(
          GsonHelper.getAsString(json, "identifier"), GsonHelper.getAsDouble(json, "minimum"));
      case "resource_below" -> FlowGuards.resourceBelow(
          GsonHelper.getAsString(json, "identifier"), GsonHelper.getAsDouble(json, "maximum"));
      ...
      case "variable_at_most" -> FlowGuards.variableAtMost(
          ...
          !json.has("value_type")
              || !GsonHelper.getAsString(json, "value_type").equalsIgnoreCase("long"));
      case "variable_at_least" -> FlowGuards.variableAtLeast(
          GsonHelper.getAsString(json, "name"),
          GsonHelper.getAsDouble(json, "minimum"),
          !json.has("value_type")
              || !GsonHelper.getAsString(json, "value_type").equalsIgnoreCase("long"));
      case "hun_shou_hua_used" -> FlowGuards.hunShouHuaUsed(
          GsonHelper.getAsBoolean(json, "expected", true));
      default -> throw new IllegalArgumentException("Unknown flow guard type: " + type);
    };
  }

  private static net.tigereye.chestcavity.guscript.runtime.flow.FlowEdgeAction parseAction(
      JsonObject json) {
    String type = GsonHelper.getAsString(json, "type");
    return switch (type) {
      case "consume_resource" -> FlowActions.consumeResource(
          GsonHelper.getAsString(json, "identifier"), GsonHelper.getAsDouble(json, "amount"));
      ...
      case "set_cooldown" -> FlowActions.setCooldown(
          GsonHelper.getAsString(json, "key"), GsonHelper.getAsLong(json, "ticks"));
      ...
      case "entity_strike" -> FlowActions.entityStrike(
          ...
          json.has("entity_id_variable")
              ? GsonHelper.getAsString(json, "entity_id_variable")
              : null);
  ...
  private static GeckoFxParameters parseGeckoFxParameters(JsonObject json) {
    ...
    GeckoFxAnchor anchor =
        GeckoFxAnchor.fromName(GsonHelper.getAsString(json, "anchor", "performer"));
  ...
  private static AttributeModifier.Operation parseOperation(String raw) {
    ...
    return switch (normalized) {
      ...
      case "MULTIPLY_TOTAL", "TOTAL_MULTIPLY", "ADD_MULTIPLIED_TOTAL" -> AttributeModifier.Operation
  ...
  private static void ensureRequiredStates(
      Map<FlowState, FlowStateDefinition> definitions, ResourceLocation id) {
