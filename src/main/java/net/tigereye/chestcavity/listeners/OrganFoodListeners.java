...
public class OrganFoodListeners {
  ...
  public static EffectiveFoodScores callMethods(
      Item food, FoodProperties foodComponent, ChestCavityEntity cce, EffectiveFoodScores efs) {
  ...
  private static EffectiveFoodScores applyHerbivorousCarnivorous(
      Item food, FoodProperties foodComponent, ChestCavityEntity cce, EffectiveFoodScores efs) {
    ItemStack sample = food.getDefaultInstance();
    if (sample.is(ItemTags.MEAT) || sample.is(CCTags.CARNIVORE_FOOD)) {
      efs.digestion +=
          cce.getChestCavityInstance().getOrganScore(CCOrganScores.CARNIVOROUS_DIGESTION);
      efs.nutrition +=
          cce.getChestCavityInstance().getOrganScore(CCOrganScores.CARNIVOROUS_NUTRITION);
    } else {
      efs.digestion +=
          cce.getChestCavityInstance().getOrganScore(CCOrganScores.HERBIVOROUS_DIGESTION);
      efs.nutrition +=
          cce.getChestCavityInstance().getOrganScore(CCOrganScores.HERBIVOROUS_NUTRITION);
    }
    return efs;
  }

  private static EffectiveFoodScores applyRot(
      Item food, FoodProperties foodComponent, ChestCavityEntity cce, EffectiveFoodScores efs) {
  ...
  private static EffectiveFoodScores applyFurnacePower(
      Item food, FoodProperties foodComponent, ChestCavityEntity cce, EffectiveFoodScores efs) {
    if (food == CCItems.FURNACE_POWER.get()) {
      int power = 0;
      if (cce.getChestCavityInstance().owner.hasEffect(CCStatusEffects.FURNACE_POWER)) {
        power =
            cce.getChestCavityInstance()
                    .owner
                    .getEffect(CCStatusEffects.FURNACE_POWER)
                    .getAmplifier()
                + 1;
      }
