...
public final class OrganHealingEvents {
  ...
  private static boolean shouldCancelPlayerHeal(
      Player player, ChestCavityInstance cc, float amount) {
  ...
  private static void fallbackRefund(Player player, FoodData food) {
    ...
    if (food.getSaturationLevel() < food.getFoodLevel()) {
      food.setSaturation(
          Math.min(
              food.getSaturationLevel() + MAX_EXPECTED_SATURATION_REFUND, food.getFoodLevel()));
    ...
    if (food.getExhaustionLevel() > EPSILON) {
      player.causeFoodExhaustion(
          -Math.min(food.getExhaustionLevel(), MAX_EXPECTED_EXHAUSTION_REFUND));
    }
  }
  ...
  static final class HungerSnapshot {
    ...
    static HungerSnapshot capture(FoodData food) {
      return new HungerSnapshot(
          food.getFoodLevel(), food.getSaturationLevel(), food.getExhaustionLevel());
    }
