...
public class OrganTickListeners {
  ...
  public static void TickBuoyant(LivingEntity entity, ChestCavityInstance chestCavity) {
    float buoyancy =
        chestCavity.getOrganScore(CCOrganScores.BUOYANT)
            - chestCavity.getChestCavityType().getDefaultOrganScore(CCOrganScores.BUOYANT);
    if ((entity instanceof Player ent && ent.isCreative() && ent.getAbilities().flying)
        || entity.onGround()
        || entity.isNoGravity()
        || buoyancy == 0) {
      return;
  ...
  public static void TickCrystalsynthesis(LivingEntity entity, ChestCavityInstance cc) {
    ...
    if (crystalsynthesis != 0
        && entity.level().getDayTime() % ChestCavity.config.CRYSTALSYNTHESIS_FREQUENCY == 0
        && !(entity instanceof EnderDragon)) {
      EndCrystal oldcrystal = cc.connectedCrystal;
      // attempt to bind to a crystal
      List<EndCrystal> list =
          entity
              .level()
              .getEntitiesOfClass(
                  EndCrystal.class,
                  entity.getBoundingBox().inflate(ChestCavity.config.CRYSTALSYNTHESIS_RANGE));
        ...
        if (entity instanceof Player) {
          ...
          if (hungerManager.needsFood()) {
            if (crystalsynthesis >= 5
                || entity.level().getDayTime() % (ChestCavity.config.CRYSTALSYNTHESIS_FREQUENCY * 5)
                    < (ChestCavity.config.CRYSTALSYNTHESIS_FREQUENCY * crystalsynthesis)) {
  ...
  public static void TickPhotosynthesis(LivingEntity entity, ChestCavityInstance cc) {
    ...
    float photosynthesis =
        cc.getOrganScore(CCOrganScores.PHOTOSYNTHESIS)
            - cc.getChestCavityType().getDefaultOrganScore(CCOrganScores.PHOTOSYNTHESIS);
    if (photosynthesis > 0) {
      int lightLevel = entity.level().getMaxLocalRawBrightness(entity.getOnPos().above());
      cc.photosynthesisProgress += photosynthesis * lightLevel;
  ...
  public static void TickHealth(LivingEntity entity, ChestCavityInstance cc) {
    if (cc.getOrganScore(CCOrganScores.HEALTH) <= 0
      ...
      if (entity.level().getDayTime() % ChestCavity.config.HEARTBLEED_RATE == 0) {
        ...
        entity.hurt(
            CCDamageSources.heartbleed(entity),
            Math.min(cc.heartBleedTimer, cc.getChestCavityType().getHeartBleedCap()));
      }
    } else {
  ...
  public static void TickFiltration(LivingEntity entity, ChestCavityInstance cc) {
    ...
    if (cc.getChestCavityType().getDefaultOrganScore(CCOrganScores.FILTRATION)
        <= 0) { // don't bother if the target doesn't need kidneys
      return;
    }
    float KidneyRatio =
        cc.getOrganScore(CCOrganScores.FILTRATION)
            / cc.getChestCavityType().getDefaultOrganScore(CCOrganScores.FILTRATION);
    if (KidneyRatio < 1) {
      cc.bloodPoisonTimer = cc.bloodPoisonTimer + 1;
      if (cc.bloodPoisonTimer >= ChestCavity.config.KIDNEY_RATE) {
        entity.addEffect(
            new MobEffectInstance(MobEffects.POISON, (int) Math.max(1, 48 * (1 - KidneyRatio))));
  ...
  private static void TickHydroallergenic(LivingEntity entity, ChestCavityInstance cc) {
    ...
    if (entity.isInWater()) {
      if (!entity.hasEffect(CCStatusEffects.WATER_VULNERABILITY)) {
        entity.hurt(entity.damageSources().magic(), 10);
        entity.addEffect(
            new MobEffectInstance(
                CCStatusEffects.WATER_VULNERABILITY,
                (int) (260 / Hydroallergy),
                0,
                false,
                false,
                true));
  ...
  public static void TickHydrophobia(LivingEntity entity, ChestCavityInstance cc) {
    ...
    if (hydrophobia <= 0 // do nothing if the target isn't hydrophobic
        || cc.getChestCavityType().getDefaultOrganScore(CCOrganScores.HYDROPHOBIA)
            != 0) { // do nothing if they are by default, otherwise endermen will spaz even harder
  ...
  public static void TickIncompatibility(LivingEntity entity, ChestCavityInstance chestCavity) {
    if (entity.level().isClientSide()
        || ChestCavity.config.DISABLE_ORGAN_REJECTION) { // this is a server-side event
      return;
    }
    float incompatibility = chestCavity.getOrganScore(CCOrganScores.INCOMPATIBILITY);
    if (incompatibility > 0) {
      if (!entity.hasEffect(CCStatusEffects.ORGAN_REJECTION)) {
        entity.addEffect(
            new MobEffectInstance(
                CCStatusEffects.ORGAN_REJECTION,
                (int) (ChestCavity.config.ORGAN_REJECTION_RATE / incompatibility),
                0,
                false,
                true,
                true));
