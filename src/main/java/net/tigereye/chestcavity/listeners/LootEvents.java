...
public final class LootEvents {
    ...
    private static final ResourceLocation DESERT_PYRAMID = ResourceLocation.parse("minecraft:chests/desert_pyramid");
    ...
    public static void onLivingDrops(LivingDropsEvent event) {
        ...
        HolderLookup<Enchantment> enchantments = level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT);
        Holder<Enchantment> lootingHolder = enchantments.getOrThrow(Enchantments.LOOTING);
        Optional<Holder<Enchantment>> surgicalHolder = CCEnchantments.holder(level, CCEnchantments.SURGICAL);
        Optional<Holder<Enchantment>> tomophobiaHolder = CCEnchantments.holder(level, CCEnchantments.TOMOPHOBIA);
        Optional<Holder<Enchantment>> malpracticeHolder = CCEnchantments.holder(level, CCEnchantments.MALPRACTICE);
        ...
        if (killer != null) {
            if (tomophobiaHolder.map(holder -> EnchantmentHelper.getEnchantmentLevel(holder, killer) > 0).orElse(false)) {
                return;
            }

            lootingLevel = EnchantmentHelper.getEnchantmentLevel(lootingHolder, killer);
            lootingLevel += surgicalHolder.map(holder -> EnchantmentHelper.getEnchantmentLevel(holder, killer)).orElse(0) * 2;
        ...
        if (killer != null) {
            ...
            if (!killer.getItemInHand(killer.getUsedItemHand()).isEmpty()
                    && malpracticeHolder.map(holder -> killer.getItemInHand(killer.getUsedItemHand()).getEnchantmentLevel(holder) > 0).orElse(false)) {
    ...
    private static void processSalvage(LivingDropsEvent event, Level level, LivingEntity entity, LivingEntity killer) {
        ...
        salvageCounts.forEach((recipe, total) -> {
