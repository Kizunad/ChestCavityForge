...
public class VenomGland extends Item implements OrganOnHitListener {
    ...
    public float onHit(DamageSource source, LivingEntity attacker, LivingEntity target, ChestCavityInstance cc, ItemStack organ, float damage) {
        ...
        if(attacker.getItemInHand(attacker.getUsedItemHand()).isEmpty() || !isNonLlamaProjectile){
            ...
            attacker.addEffect(new MobEffectInstance(CCStatusEffects.VENOM_COOLDOWN, ChestCavity.config.VENOM_COOLDOWN, 0));
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag flag) {
        ...
        CommonOrganUtil.getStatusEffects(stack).forEach(effect -> {
            Component name = Component.translatable(effect.getDescriptionId());
            int amplifier = effect.getAmplifier() + 1;
            tooltip.add(Component.translatable("tooltip.chestcavity.venom_gland.effect", name, amplifier).withStyle(ChatFormatting.GREEN));
        });
    }
