...
/**
 * Central access point for linkage contexts. Relocated from the legacy compat layer so linkage
 * becomes a first-class feature. Provides a weakly-referenced cache of {@link ActiveLinkageContext}
 * keyed by chest cavities so the GC can reclaim unused entries once the owning entity is gone.
 */
public final class LinkageManager {

  private static final Map<ChestCavityInstance, ActiveLinkageContext> CONTEXTS =
      new WeakHashMap<>();
      ...
      if (context == null) {
        ...
        if (ChestCavity.LOGGER.isDebugEnabled()) {
          ChestCavity.LOGGER.debug(
              "[linkage] Created linkage context for {}", describeChestCavity(cc));
        }
      } else if (ChestCavity.LOGGER.isTraceEnabled()) {
        ChestCavity.LOGGER.trace(
            "[linkage] Reusing cached linkage context for {}", describeChestCavity(cc));
      }
      return context;
    }
  }
  ...
  /**
   * Dispatches a slow tick to the linkage context if one exists. Callers should gate this on
   * whatever cadence they consider a "slow" tick (e.g. once every 20 game ticks).
   */
  public static void tickSlow(ChestCavityInstance cc) {
    ...
    if (context != null) {
      ...
      ChestCavity.LOGGER.trace(
          "[linkage] Slow tick skipped for {} because no context exists", describeChestCavity(cc));
    }
  }
  ...
  public static void save(ChestCavityInstance cc, CompoundTag ccTag) {
    ...
    if (context == null || context.isEmpty()) {
      if (ChestCavity.LOGGER.isTraceEnabled()) {
        ChestCavity.LOGGER.trace(
            "[linkage] Clearing linkage data for {} during save", describeChestCavity(cc));
      }
    ...
    if (!data.isEmpty()) {
      ...
      if (ChestCavity.LOGGER.isTraceEnabled()) {
        ChestCavity.LOGGER.trace(
            "[linkage] No linkage data persisted for {} (empty tag)", describeChestCavity(cc));
      }
    }
  }
  ...
  public static void load(ChestCavityInstance cc, CompoundTag ccTag) {
    if (ccTag == null || !ccTag.contains(STORAGE_KEY)) {
      if (ChestCavity.LOGGER.isTraceEnabled()) {
        ChestCavity.LOGGER.trace(
            "[linkage] No linkage NBT found when loading {}", describeChestCavity(cc));
      }
      return;
    }
