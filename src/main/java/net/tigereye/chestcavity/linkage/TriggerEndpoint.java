 ...
 *
 * A trigger endpoint represents either a passive or active reaction to a linkage event. It can enforce
 * a cooldown/minimum interval between firings to avoid over-processing.
 */
public final class TriggerEndpoint {
    ...
    public TriggerEndpoint(TriggerType type, Activation activation, long minIntervalTicks, TriHandler handler) {
        this.type = type;
    ...
    boolean tryFire(long gameTime, LivingEntity entity, ChestCavityInstance chestCavity, ActiveLinkageContext context) {
        if (handler == null) {
            return false;
        }
