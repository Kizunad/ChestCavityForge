 ...
 *
 * <p>默认关闭，可通过命令或 JVM 选项开启：-Dchestcavity.soul.vacuum.enabled=true
 */
public final class ItemVacuumHandler implements SoulRuntimeHandler {

  private static final AtomicBoolean ENABLED =
      new AtomicBoolean(Boolean.getBoolean("chestcavity.soul.vacuum.enabled"));
  ...
  private static volatile int INTERVAL =
      (int) Math.max(1, parseDouble("chestcavity.soul.vacuum.interval", 5.0));
  private static volatile double PICKUP_RANGE =
      parseDouble("chestcavity.soul.vacuum.pickupRange", 1.25);
  ...
  public static void setRadius(double r) {
    RADIUS = Math.max(0.5, Math.min(24.0, r));
  }
  ...
  public void onTickEnd(SoulPlayer player) {
    ...
    List<ItemEntity> items =
        player
            .level()
            .getEntitiesOfClass(
                ItemEntity.class, box, it -> it != null && it.isAlive() && !it.getItem().isEmpty());
