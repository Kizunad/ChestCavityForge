 ...
 * 灵魂运行时组件的统一引导入口。
 *
 * <p>在服务器生命周期内调用一次 {@link #bootstrap()}，注册 AI、动作、战斗与周期处理器，确保 FakePlayer 所需的各项子系统全部就绪。
 */
public final class SoulRuntimeHandlers {
  ...
  public static void bootstrap() {
    if (BOOT.compareAndSet(false, true)) {
      ...
      SoulRuntimeHandlerRegistry.register(
          new net.tigereye.chestcavity.soul.runtime.SoulPeriodicDispatcher());
      ...
      SoulRuntimeHandlerRegistry.register(
          new net.tigereye.chestcavity.soul.runtime.SelfHealHandler());
      // Item vacuum (optional, default off)
      SoulRuntimeHandlerRegistry.register(
          new net.tigereye.chestcavity.soul.runtime.ItemVacuumHandler());
      // Minimal AI orders (FOLLOW/IDLE/GUARD)
      SoulRuntimeHandlerRegistry.register(
          new net.tigereye.chestcavity.soul.ai.SoulAIOrderHandler());
