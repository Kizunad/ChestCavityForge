...
/**
 * Reactive hurt hook: if attacker is present, decide flee vs retaliate. - myHP < 2x enemyHP => try
 * flee away from attacker (anchor at owner). - else => set goal to target and try immediate attack.
...
public final class HurtRetaliateOrFleeHandler implements SoulRuntimeHandler {
  ...
  public SoulHurtResult onHurt(SoulPlayer player, DamageSource source, float amount) {
    ...
    var owner =
        ownerId == null
            ? null
            : player.serverLevel().getServer().getPlayerList().getPlayer(ownerId);
