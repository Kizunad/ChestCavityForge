 ...
 * 面向 LLM / 脚本的线程安全指令通道。
 *
 * <p>- 外部系统调用 {@link #submit(UUID, String, Map)} 推送指令； - 运行时处理器通过 {@link #poll(UUID)} 消费，并使用 {@link
 * #publishResult(Result)} 写回执行结果； - 外部系统可以调用 {@link #pollResult(UUID)} 或 {@link #getResult(UUID)}
 * 查询执行结论； - {@link #updateEnvironment(UUID, EnvironmentSnapshot)} 用于向外部暴露最新环境摘要（供提示词参考）。
 */
public final class SoulLLMInstructionChannel {

  private SoulLLMInstructionChannel() {}

  private static final Map<UUID, ConcurrentLinkedQueue<Instruction>> INSTRUCTIONS =
  ...
  public static Instruction submit(UUID soulId, String command, Map<String, String> parameters) {
    ...
    Instruction instruction =
        new Instruction(id, soulId, command.trim(), params, Instant.now().toEpochMilli());
  ...
  public record Result(
    ...
    public static Result success(
        Instruction instruction, String message, Map<String, String> metadata) {
  ...
  public record EnvironmentSnapshot(
    ...
    public String toJson() {
      ...
      builder
          .append("\"position\":{")
          .append("\"x\":")
          .append(String.format(Locale.ROOT, "%.2f", x))
          .append(',')
          .append("\"y\":")
          .append(String.format(Locale.ROOT, "%.2f", y))
          .append(',')
          .append("\"z\":")
          .append(String.format(Locale.ROOT, "%.2f", z))
          .append('}')
          .append(',');
      builder
          .append("\"health\":{")
          .append("\"current\":")
          .append(String.format(Locale.ROOT, "%.1f", health))
          .append(',')
          .append("\"max\":")
          .append(String.format(Locale.ROOT, "%.1f", maxHealth))
          .append('}');
      if (!metadata.isEmpty()) {
    ...
    public static EnvironmentSnapshot of(
        long gameTime,
        String dimension,
        Vec3 position,
        float health,
