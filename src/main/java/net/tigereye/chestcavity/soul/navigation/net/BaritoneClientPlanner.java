...
final class BaritoneClientPlanner {
    ...
    static void onRequest(SoulNavPlanRequestPayload req) {
        ...
        try {
            ...
            if (SoulLog.DEBUG_LOGS) SoulLog.info("[soul][nav][baritone] client plan err: {}", t.toString());
        }
    ...
    private static List<Vec3> extractNodes(Object path) {
        List<Vec3> points = new ArrayList<>();
        Object list = null;
        try { list = path.getClass().getMethod("positions").invoke(path); } catch (Throwable ignored) {}
        if (list == null) { try { list = path.getClass().getMethod("getPositions").invoke(path); } catch (Throwable ignored) {} }
        if (list == null) { try { list = path.getClass().getMethod("path").invoke(path); } catch (Throwable ignored) {} }
