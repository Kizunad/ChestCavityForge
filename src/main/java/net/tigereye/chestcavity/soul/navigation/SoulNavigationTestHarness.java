...
/**
 * Lightweight driver that exercises {@link SoulGoalPlanner#collectEntityGoals} results by steering a SoulPlayer
 * through each collected goal sequentially. For every goal the soul will pathfind towards the target, perform a jump
 * once it reaches the stop distance, wait a short delay, and continue with the next goal until exhausted.
 *
 * <p>This helper is intentionally isolated to debug commands so it can be removed or replaced without affecting the
 * production navigation loop.</p>
 */
public final class SoulNavigationTestHarness {
    ...
    /**
     * Queues a new test run for the given SoulPlayer. Any previous run for the same soul will be replaced.
     *
    ...
    public static void tick(MinecraftServer server) {
        ...
        while (it.hasNext()) {
            ...
            if (soul == null) {
                if (SoulLog.DEBUG_LOGS) {
                    SoulLog.info("[soul][test] Aborting TestCollectEntityGoals for missing soul {}", entry.getKey());
                }
                it.remove();
                continue;
            }
