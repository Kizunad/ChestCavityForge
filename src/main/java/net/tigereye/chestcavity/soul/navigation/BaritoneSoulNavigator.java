...
/**
 * Baritone-backed navigator skeleton. 当前阶段只做“可用性检测 + 基本配置”， 实际导航仍委托给 VirtualSoulNavigator，待后续接入
 * Baritone 路径规划。
 */
final class BaritoneSoulNavigator implements ISoulNavigator {
  ...
  private final java.util.concurrent.ExecutorService plannerExec =
  ...
  public void tick(SoulPlayer soul) {
    ...
    if (this.goalTarget != null
        ...
        && this.waypointIndex < this.waypoints.size()) {
  ...
  private void submitPlan(
      net.minecraft.server.level.ServerLevel level, SoulPlayer soul, Vec3 target) {
    if (planning) {
  ...
  private static long getTimeoutMs() {
    ...
    try {
      return Math.max(250L, Math.min(10_000L, Long.parseLong(t)));
  ...
  private double blocksPerTick(SoulPlayer soul) {
    var attr =
        soul.getAttribute(net.minecraft.world.entity.ai.attributes.Attributes.MOVEMENT_SPEED);
  ...
  public String debugLine() {
    ...
    return "engine=baritone, planning="
        + planning
        + ", waypoints="
        + count
        + ", idx="
  ...
  private static int getReplanStuckTicks() {
    ...
    try {
      ...
      return Math.max(20, Math.min(400, n));
    } catch (NumberFormatException e) {
      return 60;
    }
  }
