 ...
 *
 * <p>Current stage: only reports availability and returns empty (caller must fallback). Next
 * stages: use reflection to invoke Baritone path finding and convert to Vec3 waypoints.
 */
public final class BaritonePathPlanner implements ISoulPathPlanner {
  ...
  @Override
  public Optional<List<Vec3>> planPath(
      ServerLevel level, SoulPlayer soul, Vec3 target, long timeoutMs) {
    ...
    return net.tigereye.chestcavity.soul.navigation.net.SoulNavPlanBroker.requestPlan(
        owner, soul.getSoulId(), soul.position(), target, to);
  }
}
