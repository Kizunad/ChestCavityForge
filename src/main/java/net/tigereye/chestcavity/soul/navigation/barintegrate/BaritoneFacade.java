...
/**
 * Reflection-based bridge to Baritone API. This class intentionally avoids
 * compile-time dependencies so that the mod can run without Baritone present.
 *
 * Responsibilities
 * - Detect if Baritone API classes are available (considering relocated package names).
 * - Apply a minimal baseline settings profile that is safe for servers:
 *   allowBreak=false, allowPlace=false, allowParkour=false, allowParkourPlace=false,
 *   allowSprint=true, primaryTimeoutMS=2500L.
 */
public final class BaritoneFacade {

    private static final String[] API_CLASS_CANDIDATES = new String[] {
    ...
    public static void ensureInitialized() {
        ...
        synchronized (BaritoneFacade.class) {
            ...
            try {
                ...
                // We discover it via one known field (e.g., allowSprint), else via any field type named *Setting
    ...
    /**
     * Apply a minimal baseline that ensures Baritone won't break/place blocks
     * and behaves conservatively on servers.
     */
