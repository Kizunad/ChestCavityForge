...
/**
 * Evade/kite helper for SoulPlayer: performs an instant blink 10–20 blocks away (prefers higher
 * valid landing blocks) and consumes 50 jingli when used. Applies a 5-second cooldown per soul to
 * avoid spam.
 */
public final class SoulEvadeHelper {
  ...
  public static boolean tryEvade(
      SoulPlayer soul, LivingEntity hazard, double distance, double threshold) {
    ...
    if (!cost.succeeded()) {
      net.tigereye.chestcavity.soul.util.SoulLog.info(
          "[soul][evade] denied: soul={} reason={} dist={}/{}",
          soul.getSoulId(),
          cost.failureReason(),
    ...
    if (!found) {
      ...
      net.tigereye.chestcavity.soul.util.SoulLog.info(
          "[soul][evade] no-spot soul={} dist={}/{} refunded",
          soul.getSoulId(),
          String.format("%.2f", distance),
          String.format("%.2f", threshold));
      return false;
    ...
    net.tigereye.chestcavity.soul.util.SoulLog.info(
        "[soul][evade] success soul={} dist={}/{} actual={} to=({},{},{})",
        soul.getSoulId(),
        String.format("%.2f", distance),
        String.format("%.2f", threshold),
        String.format("%.2f", bestDist),
        String.format("%.2f", bestX),
        bestY,
        String.format("%.2f", bestZ));
