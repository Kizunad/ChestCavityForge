...
public final class SoulPersistence {
  ...
  private static final boolean LOG_PERSISTENCE =
      Boolean.getBoolean("chestcavity.debugSoul.persistence");
  ...
  public static void saveAll(ServerPlayer player) {
    ...
    if (LOG_PERSISTENCE) {
      SoulLog.info(
          "[soul] persistence-save owner={} profiles={}", player.getUUID(), snapshots.size());
    }
  }
  ...
  public static void saveDirty(ServerPlayer player) {
    ...
    if (LOG_PERSISTENCE) {
      SoulLog.info(
          "[soul] persistence-save-dirty owner={} profiles={}", player.getUUID(), snapshots.size());
    }
  }
  ...
  public static void loadAll(ServerPlayer player) {
    ...
    if (LOG_PERSISTENCE) {
      SoulLog.info(
          "[soul] persistence-load owner={} profiles={}", player.getUUID(), pending.size());
    }
  }
}
