...
public final class SoulProfileOps {
  ...
  private static final boolean LOG_CONTAINER_DIRTY =
      Boolean.getBoolean("chestcavity.debugSoul.containerDirty");

  private SoulProfileOps() {}

  public static void markContainerDirty(
      ServerPlayer player, SoulContainer container, String reason) {
    player.setData(CCAttachments.SOUL_CONTAINER.get(), container);
    if (LOG_CONTAINER_DIRTY) {
      SoulLog.info(
          "[soul] container-dirty reason={} owner={} active={}",
          reason,
          player.getUUID(),
  ...
  public static void applyProfileToPlayer(SoulProfile profile, ServerPlayer player, String reason) {
    profile.restoreBase(player);
    profile.position().ifPresent(snapshot -> teleportToSnapshot(snapshot, player));
    SoulLog.info(
        "[soul] apply-profile reason={} soul={} dim={} pos=({},{},{})",
        reason,
        profile.id(),
        profile
            .position()
            .map(PlayerPositionSnapshot::dimension)
  ...
  private static void teleportToSnapshot(PlayerPositionSnapshot snapshot, ServerPlayer player) {
    ...
    if (targetLevel != null) {
      ...
      player.teleportTo(
          targetLevel, snapshot.x(), safeY, snapshot.z(), snapshot.yaw(), snapshot.pitch());
