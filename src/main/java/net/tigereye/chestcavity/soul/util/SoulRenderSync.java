...
public final class SoulRenderSync {
  ...
  public static void syncEquipment(LivingEntity entity) {
    ...
    serverLevel
        .getServer()
        .getPlayerList()
        .broadcastAll(new ClientboundSetEquipmentPacket(entity.getId(), list));
  }

  /**
   * If the player's main hand is empty, pick the first non-empty hotbar slot as selected so the
   * main-hand item can render after inventory restores.
   */
