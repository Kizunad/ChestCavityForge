...
public final class MeleeAttackHandler implements SoulAttackHandler {
    ...
    private final java.util.Map<java.util.UUID, Long> lastAttackTick = new java.util.concurrent.ConcurrentHashMap<>();
    ...
    public boolean tryAttack(AttackContext ctx) {
        ...
        if (last != null) {
            ...
            if (diff < 0) {
                ...
                net.tigereye.chestcavity.soul.util.SoulLog.info("[soul][attack] melee-cd soul={} remain={}t", self.getSoulId(), remain);
        ...
        if (hit) {
            ...
            self.level().playSound(null, self.blockPosition(), SoundEvents.PLAYER_ATTACK_STRONG, SoundSource.PLAYERS, 0.8f, 1.0f);
            net.tigereye.chestcavity.soul.util.SoulLog.info("[soul][attack] melee soul={} target={} estDmg={}", self.getSoulId(), target.getUUID(), damage);
        }
        return hit;
    }
}
