 ...
 *
 * <p>This handler does not consume the attack; it returns false so downstream handlers (e.g.,
 * melee) still execute. Each ability enforces its own cooldown/resource gates.
 */
public final class GuzhenrenAttackAbilityHandler implements SoulAttackHandler {

  // Avoid class-loading optional compat behaviours during static init to prevent crashes when mods
  // are absent.
  // Start conservatively with HuoYiGu; more abilities can be appended as string IDs here without
  // touching classes.
  private static final ResourceLocation[] ABILITY_IDS =
      new ResourceLocation[] {
        ...
        ResourceLocation.fromNamespaceAndPath(
            "guzhenren", "luo_xuan_gu_qiang_gu"), // 螺旋骨枪蛊（主动同 organId）
  ...
  public boolean tryAttack(AttackContext ctx) {
    ...
    if (attempts > 0) {
      SoulLog.info(
          "[soul][attack] guzhenren-abilities soul={} tried={} target={}",
          self.getSoulId(),
          attempts,
          ctx.target().getUUID());
    }
