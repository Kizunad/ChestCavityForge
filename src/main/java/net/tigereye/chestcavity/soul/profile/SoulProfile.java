 ...
 * 灵魂存档（单个槽位）
 *
 * <p>职责 - 表示一个完整的“灵魂”存档槽，记录玩家背包、原版属性与位置。 - 提供从玩家捕获快照、回写到玩家与 NBT 序列化/反序列化的能力。
 *
 * <p>约束 - 当前仅覆盖 vanilla 数据；后续可扩展：能力附件/ChestCavity/其他兼容。
 */
public final class SoulProfile {
  ...
  public static SoulProfile capture(ServerPlayer player, UUID id) {
    Map<ResourceLocation, CapabilitySnapshot> capabilitySnapshots =
  ...
  public static SoulProfile fromSnapshot(
      UUID id,
      ...
      PlayerPositionSnapshot position) {
    return fromSnapshot(
        id, snapshot, stats, effects, position, CapabilityPipeline.createDefaultSnapshots());
  }
  ...
  public <T extends CapabilitySnapshot> java.util.Optional<T> capability(
  ...
  public static SoulProfile load(CompoundTag tag, HolderLookup.Provider provider) {
    ...
    PlayerStatsSnapshot stats =
        tag.contains("stats")
            ...
            : PlayerStatsSnapshot.empty();
    PlayerEffectsSnapshot effects =
        tag.contains("effects")
            ? PlayerEffectsSnapshot.load(tag.getCompound("effects"))
            : PlayerEffectsSnapshot.empty();
    PlayerPositionSnapshot position =
        tag.contains("position")
            ? PlayerPositionSnapshot.load(tag.getCompound("position"))
            : PlayerPositionSnapshot.empty();
    Map<ResourceLocation, CapabilitySnapshot> capabilities =
        CapabilityPipeline.load(tag.getCompound("capabilities"), provider);
