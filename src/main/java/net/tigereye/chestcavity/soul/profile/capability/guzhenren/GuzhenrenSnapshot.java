 ...
 *
 * <p>This snapshot is intentionally conservative: it only mirrors the numeric fields exposed by
 * {@link GuzhenrenResourceBridge}. Non-numeric payloads (e.g. ItemStack based ShaZhao slots) are
 * ignored for now because they require additional inventory style handling to avoid duping.
 */
    ...
    for (Map.Entry<String, Double> entry : cachedValues.entrySet()) {
      ...
      if (result.isEmpty()) {
        ChestCavity.LOGGER.debug(
            "[soul] Failed to apply Guzhenren field {} for {}",
            identifier,
