...
public interface BrainActionStep {
  ...
  Comparator<BrainActionStep> PRIORITY_COMPARATOR =
      Comparator.comparingInt(BrainActionStep::priority).reversed();

  /**
   * Whether this step should execute for the provided context. Returning {@code false} skips {@link
   * #execute(SubBrainContext)} for the current tick.
   */
  ...
  static BrainActionStep of(
      Predicate<SubBrainContext> predicate, Consumer<SubBrainContext> executor) {
