...
/**
 * 集成调试脚本：将评分器、策略、仲裁器组合在一起，
 * 为后续串行接线提供可复用的离线决策逻辑。
 * <p>
 * 该脚本并不会直接修改当前运行时，只提供一个可配置的
 * 决策器，用于计算“应当切换到哪个子大脑”以及“并行动作”建议。
 */
public final class BrainIntegrationScript {
     ...
     */
    public Decision evaluate(ScoreInputs inputs,
        ...
        double distanceScore = normalizeDistance(inputs.primaryTarget() != null ? inputs.distanceToTarget() : Double.POSITIVE_INFINITY);
        double healthRatio = inputs.healthRatio();
        boolean hasRegen = inputs.hasRegen();
        boolean inDanger = inputs.inDanger();

        double combatScore = combatScorer.score(threatLevel, distanceScore, healthRatio, hasRegen, inDanger);
        double survivalScore = survivalScorer.score(Math.max(threatLevel, inDanger ? 1.0 : 0.0), distanceScore, healthRatio, hasRegen, inDanger);
        double explorationScore = explorationScorer.score(1.0 - threatLevel, 1.0 - distanceScore, healthRatio, hasRegen, !inDanger);
        double idleScore = Math.max(0.0, 1.0 - Math.max(Math.max(combatScore, survivalScore), explorationScore));

        ModeCandidate current = pickCurrent(normalizedId, combatScore, survivalScore, explorationScore, idleScore);
        ModeCandidate target = pickTarget(current, elapsedTicks, combatScore, survivalScore, explorationScore, idleScore, allowStateChange);
        ...
        Arbitrator.Result<ActionCandidate> arbitration = allowPlanning
    ...
    private static ModeCandidate pickCurrent(String currentId,
                                             double combatScore,
                                             double survivalScore,
                                             double explorationScore,
                                             double idleScore) {
    ...
    private ModeCandidate pickTarget(ModeCandidate current,
                                     int elapsedTicks,
                                     double combatScore,
                                     double survivalScore,
                                     double explorationScore,
        ...
        List<ModeCandidate> candidates = List.of(
    ...
    private static List<ActionCandidate> defaultActionCandidates(double combatScore,
                                                                 double survivalScore,
                                                                 double explorationScore) {
