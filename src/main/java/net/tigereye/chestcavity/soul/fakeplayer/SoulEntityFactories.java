...
public final class SoulEntityFactories {
    ...
    public static void register(ResourceLocation id, SoulEntityFactory factory) {
        ...
        if (previous != null && previous != factory) {
            SoulLog.warn("[soul] factory-overwrite id={} prev={} new={}", id, previous.getClass().getName(), factory.getClass().getName());
        } else {
    ...
    public static Optional<SoulEntitySpawnResult> spawn(SoulEntitySpawnRequest request) {
        ...
        if (factory == null) {
            SoulLog.warn("[soul] spawn-abort reason=factory-missing id={} entity={} cause=noFactory", request.factoryId(), request.entityId());
            return Optional.empty();
        }
        request.fallbackLevel().ifPresent(level -> {
            if (request.ensureChunkLoaded()) {
                try {
                    level.getChunkAt(BlockPos.containing(request.fallbackPosition()));
                } catch (Throwable throwable) {
                    SoulLog.error("[soul] spawn-chunk-load-failed id={} dim={} pos=({},{},{})", throwable,
                            request.factoryId(),
        ...
        try {
            ...
            SoulLog.error("[soul] spawn-factory-error id={} entity={} reason={}", throwable, request.factoryId(), request.entityId(), request.reason());
            return Optional.empty();
        }
    }
