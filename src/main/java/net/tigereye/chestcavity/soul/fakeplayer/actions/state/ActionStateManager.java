...
/**
 * Minimal per-soul action registry supporting priorities and concurrency.
 * Not persisted yet; suitable as a runtime coordinator.
 */
public final class ActionStateManager {
    ...
    public Collection<ActionRuntime> active() { return active.values(); }

    public boolean isActive(ActionId id) { return active.containsKey(id); }

    /** Cancel all exclusive (non-concurrent) actions before starting a new exclusive one. */
    private void preemptFor(Action newAction, ServerLevel level, SoulPlayer soul, ServerPlayer owner) {
        ...
        for (var entry : new java.util.ArrayList<>(active.entrySet())) {
            ...
            Action current = net.tigereye.chestcavity.soul.fakeplayer.actions.registry.ActionRegistry.find(aid);
            if (current != null && !current.allowConcurrent()) {
                try { current.cancel(new ActionContext(level, soul, owner)); } catch (Throwable ignored) {}
                active.remove(aid);
            }
        }
    }
    ...
    public boolean tryStart(ServerLevel level, SoulPlayer soul, Action action, ServerPlayer owner) {
        ...
        ActionScheduler.schedule(level, soul, action.id(), (Runnable)() -> tick(level, soul, action, owner), (int)Math.max(0, next - now));
        return true;
    }
