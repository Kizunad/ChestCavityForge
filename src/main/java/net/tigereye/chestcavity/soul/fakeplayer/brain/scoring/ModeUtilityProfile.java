...
/**
 * Encapsulates the heuristics for turning {@link ScoreInputs} into {@link ModeScore} instances for
 * a specific {@link BrainMode}.
 */
public final class ModeUtilityProfile {
  ...
  public ModeScore score(ScoreInputs inputs) {
    ...
    double raw =
        scorer.score(threat, proximity, effectiveHealth, inputs.hasRegen(), inputs.inDanger());
  ...
  private double computeEffectiveHealth(ScoreInputs inputs) {
    ...
    double absorptionContribution =
        absorptionAsHealth <= 0.0 ? 0.0 : Math.min(1.0, absorption / absorptionAsHealth);
