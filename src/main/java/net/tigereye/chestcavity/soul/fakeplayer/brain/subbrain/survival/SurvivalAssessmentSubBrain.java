 ...
 *
 * <p>作用 - 每 tick 计算一次 SurvivalSnapshot（包含 fleeScore、shouldRetreat、healthRatio、威胁等）， 提供给其他子大脑（如
 * SurvivalRetreatSubBrain）作为决策依据。 - 评分由 SurvivalScorecard + WeightedUtilityScorer 完成，通过多指标加权得到撤退倾向。
 *
 * <p>关键输入 - 最近伤害来源或扫描半径内最近的敌对实体作为“威胁”； - 自身生命比例、吸收护盾、是否有再生等； - 是否处于“危险”粗判（有威胁/半血/吸收低）。
 */
public final class SurvivalAssessmentSubBrain extends SubBrain {
  ...
  public SurvivalAssessmentSubBrain() {
    ...
    this.scorecard =
        new SurvivalScorecard(new WeightedUtilityScorer(0.35, 0.2, 0.3, -0.15, 0.3), 0.55, 0.35);
  ...
  private void computeSnapshot(SubBrainContext ctx) {
    ...
    double maxDistance =
        Math.max(
            scanRadius,
            ctx.personality().getDouble(BrainTuningKeys.SURVIVAL_RETREAT_MAX_DISTANCE, 18.0));
