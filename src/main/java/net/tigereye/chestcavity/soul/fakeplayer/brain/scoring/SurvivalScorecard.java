...
public final class SurvivalScorecard {
  ...
  public SurvivalScorecard(
      WeightedUtilityScorer fleeScorer, double retreatThreshold, double holdThreshold) {
  ...
  public SurvivalSnapshot evaluate(ScoreInputs inputs) {
    ...
    double fleeScore =
        fleeScorer.score(
            threatScore(threat), closeness, health, inputs.hasRegen(), inputs.inDanger());
