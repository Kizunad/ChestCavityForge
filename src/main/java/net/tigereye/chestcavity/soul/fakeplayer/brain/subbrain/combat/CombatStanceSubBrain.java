...
/**
 * Maintains the primary combat stance action (force fight or guard) based on the most up-to-date
 * intent snapshot and legacy order system.
 */
public final class CombatStanceSubBrain extends SubBrain {

  private static final ResourceLocation FORCE_FIGHT =
      ResourceLocation.fromNamespaceAndPath(ChestCavity.MODID, "action/force_fight");
  private static final ResourceLocation GUARD =
      ResourceLocation.fromNamespaceAndPath(ChestCavity.MODID, "action/guard");
  ...
  private ResourceLocation resolveDesiredAction(SubBrainContext ctx) {
    if (ctx.intent() != null
        && ctx.intent().isPresent()
        && ctx.intent().intent() instanceof CombatIntent combatIntent) {
