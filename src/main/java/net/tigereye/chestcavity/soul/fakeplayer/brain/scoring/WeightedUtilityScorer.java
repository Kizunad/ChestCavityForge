...
/**
 * Simple weighted utility scorer. Produces a value in [0, 1] for each behaviour
 * candidate based on normalized inputs.
 */
public final class WeightedUtilityScorer {
    ...
    public WeightedUtilityScorer(double wThreat, double wDistance, double wHealth, double wRegen, double wDanger) {
     ...
     */
    public double score(double threat, double distance, double health, boolean hasRegen, boolean inDanger) {
        double regen = hasRegen ? 1.0 : 0.0;
        double danger = inDanger ? 1.0 : 0.0;
        double raw = wThreat*clamp01(threat) + wDistance*clamp01(distance) + wHealth*clamp01(1.0 - health)
                + wRegen*regen + wDanger*danger;
        // Normalize by total positive weights to keep within [0,1]
        double denom = Math.max(1e-6, Math.abs(wThreat)+Math.abs(wDistance)+Math.abs(wHealth)+Math.abs(wRegen)+Math.abs(wDanger));
