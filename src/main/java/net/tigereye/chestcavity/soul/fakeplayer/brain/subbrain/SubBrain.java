...
/**
 * A sub-brain encapsulates a slice of behaviour that can be orchestrated by a higher level {@link
 * net.tigereye.chestcavity.soul.fakeplayer.brain.Brain}.
 *
 * <p>Implementations compose their behaviour by registering a sequence of {@link BrainActionStep}
 * instances. Each step can gate its execution via {@link
 * BrainActionStep#shouldExecute(SubBrainContext)} which makes the orchestration similar to building
 * blocks in a behaviour tree.
 */
public abstract class SubBrain {
  ...
  /**
   * Called when the sub-brain becomes active for the current soul. This is triggered on the
   * transition where {@link #shouldTick(SubBrainContext)} returns {@code true} after being {@code
   * false}.
   */
  ...
  /**
   * Whether this sub-brain should be allowed to tick for the current cycle. Implementations can
   * gate expensive behaviours based on context.
   */
  ...
  /**
   * Registers a new behaviour step. Steps are iterated in priority order and executed sequentially
   * when {@link #tick(SubBrainContext)} is invoked.
   */
