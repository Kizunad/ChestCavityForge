 ...
 *
 * <p>作用 - 围绕 owner 在一定半径内随机巡逻，周期性重选目标点； - 通过 SoulNavigationMirror 设置行走目标，达到或超时后重新规划； - 使用
 * MultiCooldown 控制“重规划”节奏，避免过于频繁的目标切换。
 *
 * <p>参数 - PATROL_RADIUS：巡逻半径（水平随机落点）； - STOP_DIST：到达判定半径； - SPEED：移动速度修正； -
...
public final class ExplorationPatrolSubBrain extends SubBrain {
  ...
  private void tickPatrol(SubBrainContext ctx) {
    ...
    MultiCooldown cooldowns =
        ctx.memory().get(MEMORY_COOLDOWNS, ExplorationPatrolSubBrain::createCooldowns);
    ...
    if (target == null || reached(ctx, target) || replan.isReady(now)) {
      ...
      BrainDebugProbe.emit(
          BrainDebugEvent.builder("exploration")
