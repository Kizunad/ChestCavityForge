 ...
 *
 * <p>作用 - 基于外部计算好的 SurvivalSnapshot（共享内存中提供）决定何时进入/退出“撤退”状态； - 撤退优先尝试调用 SoulFleeRegistry
 * 的逃生策略（例如朝安全点或远离威胁的路径）； - 若逃生策略不可用，则回退为本地计算的“远离威胁向量 + 地形落点”目标，并通过 SoulNavigationMirror 设置高优先级寻路； -
 * 使用 SafetyWindowPolicy 提供的“安全窗口”抖动保护：短时间内的安全/不安全切换不会频繁进出撤退状态。
 *
 * <p>关键输入 - SurvivalSnapshot：来自生存评估模块，包含威胁实体、生命比例、撤退评分等； - SubBrainContext：提供
 * soul/owner/level/memory 等运行期上下文。
 *
 * <p>内部状态 - State.fleeing：是否处于撤退中； - MultiCooldown safeWindow：安全窗口冷却，配合 SAFETY_WINDOW 判定“可以退出撤退”。
 */
public final class SurvivalRetreatSubBrain extends SubBrain {
  ...
  private void tickRetreat(SubBrainContext ctx) {
    ...
    MultiCooldown cooldowns =
        ctx.memory().get(MEMORY_COOLDOWNS, SurvivalRetreatSubBrain::createCooldowns);
    ...
    if (snapshot.shouldRetreat()) {
      ...
      int windowTicks =
          ctx.personality()
              .getInt(BrainTuningKeys.SURVIVAL_SAFE_WINDOW_TICKS, SAFETY_WINDOW.windowTicks());
      if (windowTicks <= 0) {
      ...
      state.fleeing = false;
      SoulNavigationMirror.clearGoal(ctx.soul());
      BrainDebugProbe.emit(
          BrainDebugEvent.builder("survival")
              .message("retreat_end")
  ...
  private Vec3 computeFallbackTarget(
      SubBrainContext ctx, SurvivalSnapshot snapshot, FleeContext fleeContext) {
    var soul = ctx.soul();
    var level = ctx.level();
    ...
    double jitterRange =
        ctx.personality().getDouble(BrainTuningKeys.SURVIVAL_RETREAT_JITTER_RADIANS, Math.PI / 6.0);
    if (jitterRange < 0.0) {
    ...
    Vec3 rotated = new Vec3(dir.x * cos - dir.z * sin, 0.0, dir.x * sin + dir.z * cos);
    ...
    double configuredMin =
        ctx.personality().getDouble(BrainTuningKeys.SURVIVAL_RETREAT_MIN_DISTANCE, 10.0);
    double configuredMax =
        ctx.personality().getDouble(BrainTuningKeys.SURVIVAL_RETREAT_MAX_DISTANCE, 14.0);
    ...
    for (int attempt = 0; attempt < 6; attempt++) {
      ...
      int surfaceY =
          level.getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, column.getX(), column.getZ());
  ...
  private List<LivingEntity> collectHostiles(SubBrainContext ctx, SurvivalSnapshot snapshot) {
    ...
    double radius =
        Math.max(
            6.0, ctx.personality().getDouble(BrainTuningKeys.SURVIVAL_THREAT_SCAN_RADIUS, 18.0));
    AABB box = new AABB(center, center).inflate(radius);
    // 过滤敌对目标（怪物或在 ConstantMobs 定义为敌对）
    // TODO(survival): 敌对合成向量权重加入“生命值因子”（半血降低权重而非忽略），
    //  形式建议：weight = f(healthRatio) / max(dist^2, 1.0)，其中 f = clamp(0.2 + 0.8*healthRatio, 0.2,
    // 1.0)。
  ...
  private static MultiCooldown createCooldowns() {
    ...
    return MultiCooldown.builder(state).withLongClamp(value -> Math.max(0L, value), 0L).build();
  }
