...
/**
 * Coordinates residence, hysteresis and budget rules to decide whether the brain should switch to
 * another {@link BrainMode}.
 */
public final class ModeSelectionPolicy {
  ...
  public ModeSelectionPolicy(
      HysteresisPolicy hysteresis, ResidencePolicy residence, BudgetPolicy budget) {
  ...
  public Decision decide(
      BrainMode currentMode,
      ScoreInputs inputs,
      ModeScoringEngine engine,
      int ticksInCurrentMode,
      long gameTime) {
    ...
    if (currentScore == null) {
      currentScore =
          engine.tryScore(currentMode, inputs).orElse(ModeScore.zero(currentMode, inputs));
    ...
    if (!residence.canLeave(currentMode, ticksInCurrentMode)) {
      return new Decision(
          currentMode, currentScore, currentScore, DecisionReason.RESIDENCE_LOCKED, ordered);
    }

    if (!budgetTracker.canSwitch()) {
      return new Decision(
          currentMode, currentScore, currentScore, DecisionReason.BUDGET_EXHAUSTED, ordered);
    }

    if (bestAlternative == null) {
      return new Decision(
          currentMode, currentScore, currentScore, DecisionReason.NO_ALTERNATIVE, ordered);
    }

    if (!hysteresis.shouldSwitch(currentScore.utility(), bestAlternative.utility())) {
      return new Decision(
          currentMode, currentScore, currentScore, DecisionReason.STAY_CURRENT, ordered);
    }
    ...
    return new Decision(
        ...
        bestAlternative,
        currentScore,
        DecisionReason.SWITCHED_FOR_UTILITY,
        ordered);
  }
