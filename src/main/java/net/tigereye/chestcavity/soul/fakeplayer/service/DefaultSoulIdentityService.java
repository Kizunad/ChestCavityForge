...
public final class DefaultSoulIdentityService implements SoulIdentityService {
    ...
    @Override
    public GameProfile ensureIdentity(UUID soulId, GameProfile sourceProfile, boolean forceDerivedId) {
        if (soulId == null) {
        ...
        return identities.compute(soulId, (id, existing) -> {
    ...
    public void updateIdentityName(UUID soulId, String newName) {
        identities.compute(soulId, (id, existing) -> {
            if (existing == null) {
                return null;
            }
    ...
    public boolean isIdentityNameInUse(String candidate) {
        ...
        for (GameProfile profile : identities.values()) {
            if (profile != null && profile.getName() != null && profile.getName().toLowerCase(Locale.ROOT).equals(target)) {
                return true;
            }
        }
        return false;
