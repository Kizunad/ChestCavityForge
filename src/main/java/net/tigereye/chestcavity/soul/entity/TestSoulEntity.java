...
public class TestSoulEntity extends PathfinderMob {
  ...
  private final SoulPersonality personality =
      SoulPersonalityRegistry.resolve(SoulPersonalityRegistry.CAUTIOUS_ID);
  ...
  protected ResourceKey<LootTable> getDefaultLootTable() {
    ...
    return ResourceKey.create(
        Registries.LOOT_TABLE, ResourceLocation.parse("chestcavity:entities/test_soul"));
  }
  ...
  protected void registerGoals() {
    ...
    this.goalSelector.addGoal(
        1,
        DynamicGoalDispatcher.<TestSoulEntity>builder(this)
  ...
  public void tick() {
    ...
    if (!level().isClientSide) {
      ...
      ItemAbsorptionOps.pullAndProcess(
          this, ITEM_RADIUS, ITEM_PULL, item -> true, this::handleItemPickup);
      if (tickCount % 40 == 0) {
        double surface =
            level()
                .getHeight(
    ...
    if (result && !level().isClientSide) {
      ...
      if (aggressor != null && aggressor.isAlive() && aggressor != this) {
        ...
        if (ratio >= 0.9) {
          ...
          this.reactiveMode =
              this.getRandom().nextDouble() < 0.5 ? ReactiveMode.RETALIATE : ReactiveMode.EVADE;
        }
   ...
   *
   * <p>设计意图： - 受击后根据 {@link ReactiveMode} 切换“反击”或“脱离”。 - 反击：锁定最近的攻击者，转向并持续寻找攻击机会。 -
   * 脱离：向远离攻击者的位置移动，必要时重新取样逃离路径。
   *
   * <p>状态驱动： - {@link ReactiveMode#RETALIATE} 由实体逻辑设置，Goal 负责目标与朝向。 - {@link ReactiveMode#EVADE} 在
   * {@link #canUse()} 时会取样逃离点并持续尝试保持距离。 - Goal 在 {@link #canContinueToUse()} 中持续校验攻击者存活与剩余反应时间。
   */
  ...
  public static boolean checkSpawnRules(
      EntityType<TestSoulEntity> type,
      LevelAccessor world,
      MobSpawnType reason,
      BlockPos pos,
      RandomSource random) {
