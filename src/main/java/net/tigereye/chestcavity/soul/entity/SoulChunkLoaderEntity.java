...
public class SoulChunkLoaderEntity extends Entity {
  ...
  @Nullable private ChunkPos ticketPos;
  ...
  private void releaseTicket(@Nullable ServerLevel level) {
    if (level != null && ticketPos != null && ticketRadius > 0) {
      level
          .getChunkSource()
          .removeRegionTicket(CHUNK_TICKET, ticketPos, ticketRadius, this.getUUID());
    }
    ticketPos = null;
    ticketRadius = 0;
  }
  ...
  protected void checkFallDamage(
      double heightDiff, boolean onGround, BlockState state, BlockPos pos) {
    // ignore
  }
}
