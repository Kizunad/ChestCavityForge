 ...
 *
 * <p>This is intentionally separate from capability snapshots so we can evolve different
...
public final class SoulInitializers {
  ...
  /**
   * Default preset used by spawn_soul_custom: directly clone Guzhenren numeric fields from the
   * owner into the new profile's Guzhenren snapshot (if present). Other capabilities are left as
   * created.
   */
  ...
  /**
   * Reads every numeric field exposed by GuzhenrenResourceBridge from the owner and writes them
   * into the target profile's GuzhenrenSnapshot via its load path. This avoids depending on
   * implementation details (e.g., presence of mutators) and keeps semantics aligned with snapshot
   * persistence.
   */
  ...
  /**
   * Caps-only preset: set shouyuan/max zhenyuan/max hunpo/max niantou to provided values, zero
   * everything else. This builds a values tag using the owner's known key set (from snapshotAll) so
   * we can explicitly zero them.
   */
  public static void applyGuzhenrenCapsOnly(
      ServerPlayer owner,
  ...
  /**
   * Convenience wrapper: shouyuan=100, zuida_zhenyuan=20, zuida_hunpo=1, niantou_zhida=100;
