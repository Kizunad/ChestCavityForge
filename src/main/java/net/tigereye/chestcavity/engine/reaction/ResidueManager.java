 ...
 * 搬迁自 util.reaction.engine.ResidueManager。
 */
public final class ResidueManager {
    ...
    public static void tickFireResidues(MinecraftServer server) {
        ...
        for (var e : entries) {
            UUID id = e.getValue();
            AreaEffectCloud aec = null; ServerLevel foundLevel = null;
            for (ServerLevel lvl : server.getAllLevels()) {
                var ent = lvl.getEntity(id);
                if (ent instanceof AreaEffectCloud cloud) { aec = cloud; foundLevel = lvl; break; }
            }
            if (aec == null || foundLevel == null) { FIRE_KEYS.remove(e.getKey()); FIRE_FINALIZED.remove(e.getKey()); continue; }
            ...
            if (server.getTickCount() % interval == 0) {
                ...
                int applied = 0; int max = Math.max(1, net.tigereye.chestcavity.ChestCavity.config.REACTION.HUO_TAN.emberMaxEntitiesPerTick);
                for (var le : list) {
            ...
            if (aec.getDuration() <= 20 && !Boolean.TRUE.equals(FIRE_FINALIZED.get(e.getKey()))) {
                ...
                for (var le : list) {
                    net.tigereye.chestcavity.util.reaction.tag.ReactionTagOps.add(le,
                            net.tigereye.chestcavity.util.reaction.tag.ReactionTagKeys.IGNITE_WINDOW, 40);
                }
            ...
            for (var be : new java.util.ArrayList<>(BLOOD_KEYS.entrySet())) {
                ...
                double dx = aec.getX()-blood.getX(); double dy = aec.getY()-blood.getY(); double dz = aec.getZ()-blood.getZ();
                double dist2 = dx*dx+dy*dy+dz*dz;
                if (dist2 < 0.25) {
                    EffectsEngine.queueAoEDamage(foundLevel, aec.getX(), aec.getY(), aec.getZ(), 2.0F, 3.0D, null, EffectsEngine.VisualTheme.STEAM);
                    aec.discard(); blood.discard();
                    FIRE_KEYS.remove(e.getKey()); FIRE_FINALIZED.remove(e.getKey()); BLOOD_KEYS.remove(be.getKey());
                    break;
                }
            }
        }
    ...
    private static int detectNearbyHuoTanTier(ServerLevel level, double x, double y, double z, float r) {
