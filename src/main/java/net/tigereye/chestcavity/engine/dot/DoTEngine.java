...
/** DoT 核心引擎。 - 负责持续伤害的调度、聚合与执行； - 在 ServerTick(Post) 由 TickEngineHub 以优先级晚于 Reaction 执行。 */
public final class DoTEngine {
  ...
  public static void schedulePerSecond(
    ...
    if (debugEnabled()) {
      LOGGER.info(
          "[dot] queued DoT type={} seconds={} attacker={} target={} dps={}",
          typeId,
          durationSeconds,
  ...
  private static DoTEntry toEntry(Pulse p) {
    ...
    return new DoTEntry(
        p.dueTick, p.attackerUuid, p.targetUuid, p.amount, p.typeId, soundId, p.fxId, p.fxAnchor);
  }
  ...
  public static void handleServerTick(ServerTickEvent.Post event) {
    ...
    for (Map.Entry<Integer, List<Pulse>> entry : due) {
      for (Pulse pulse : entry.getValue()) {
        LivingEntity target =
            targetCache.computeIfAbsent(pulse.targetUuid, id -> findLiving(event.getServer(), id));
        if (target == null || !target.isAlive()) continue;
        LivingEntity attacker =
            pulse.attackerUuid != null
                ? attackerCache.computeIfAbsent(
                    pulse.attackerUuid, id -> findLiving(event.getServer(), id))
                : null;
