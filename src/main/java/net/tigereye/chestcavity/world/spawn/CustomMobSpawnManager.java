...
public final class CustomMobSpawnManager {
  ...
  private static final Map<MinecraftServer, Map<ResourceLocation, EntityTracker>> SERVER_TRACKERS =
      new WeakHashMap<>();
  ...
  public static void onServerTick(ServerTickEvent.Post event) {
    ...
    Map<ResourceLocation, EntityTracker> trackerMap =
        SERVER_TRACKERS.computeIfAbsent(server, s -> new HashMap<>());
    for (CustomMobSpawnDefinition definition : CustomMobSpawnRegistry.definitions()) {
      EntityTracker tracker =
          trackerMap.computeIfAbsent(definition.id(), key -> new EntityTracker(definition));
      tracker.tick(server);
    }
  }
  ...
  private static final class EntityTracker {
    ...
    private void attemptSpawn(MinecraftServer server) {
      ...
      for (int i = 0; i < batchSize; i++) {
        ...
        Optional<SpawnLocation> spawnLocation =
            definition.locationProvider().find(server, definition, random);
        ...
        for (SpawnConfigurator configurator : definition.configurators()) {
          try {
            ...
            ChestCavity.LOGGER.error(
                "[spawn] configurator error id={} mob={}",
                definition.id(),
                mob.getUUID(),
                throwable);
          }
        ...
        if (!definition.brainConfigurators().isEmpty()) {
          ...
          for (BrainConfigurator brainConfigurator : definition.brainConfigurators()) {
            try {
              ...
              ChestCavity.LOGGER.error(
                  "[spawn] brain-configurator error id={} mob={}",
                  definition.id(),
                  mob.getUUID(),
                  throwable);
            }
  ...
  private static final class TrackedMob {
    ...
    private TrackedMob(Mob mob, CustomMobMessageEmitter emitter) {
      ...
      this.messageState =
          emitter == null ? null : new CustomMobMessageEmitter.RuntimeState(emitter);
