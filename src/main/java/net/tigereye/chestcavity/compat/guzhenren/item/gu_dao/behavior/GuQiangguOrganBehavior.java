...
/**
 * Base slow-tick behaviour for 骨枪蛊： - 读取 bone_growth 通道 - 按阈值转换为 Charge 并写入器官 NBT - 参考骨竹蛊的调试播报输出增量
 */
public enum GuQiangguOrganBehavior
  ...
  private static final int MAX_CHARGE =
      BehaviorConfigAccess.getInt(GuQiangguOrganBehavior.class, "MAX_CHARGE", 10);
  ...
  private static void sendDebugMessage(int gained, int total, double remainingEnergy) {
    ChestCavity.LOGGER.info(
        "[GuQiang] CHARGE +{} -> {} (energy={})",
        gained,
        total,
        String.format("%.1f", remainingEnergy));
  }
  ...
  public float onHit(
    ...
    sendHitDebug(
        String.format(
            "damage +%.1f => %.1f (charge=%d, efficiency=%.2f)",
            bonusDamage, result, updated, guDaoEfficiency));
    return result;
  }
  ...
  private static void applyBleed(LivingEntity target, double guDaoEfficiency) {
    ...
    Optional<Holder.Reference<net.minecraft.world.effect.MobEffect>> holder =
        BuiltInRegistries.MOB_EFFECT.getHolder(BLEED_EFFECT_ID);
    holder.ifPresent(
        effect ->
            target.addEffect(
                new MobEffectInstance(effect, 200, scaledAmplifier, false, true, true)));
  }
