...
## 文档元信息
- **创建日期**: 2025-11-14
- **修订日期**: 2025-11-14 (评审后大幅修订)
- **版本**: v2.0 (收敛MVP，修正关键纰漏)
...
- [ ] 定义 EntityDataAccessor (OWNER, SKIN_*)
- [ ] 实现 ItemStackHandler(7) 与能力系统
- [ ] 实现 `serializeToItemNBT` / `deserializeFromItemNBT`
- [ ] 实现 `addAdditionalSaveData` / `readAdditionalSaveData`
- [ ] 实现 `baseTick()` (分频AI调用)
- [ ] 实现 `spawn()` 静态工厂方法
- [ ] 实现 `isOwnedBy()` / `openInventoryMenu()`
- [ ] 实现 `die()` (清理物品状态)
- [ ] 注册 AI Goals (参考 `NanGuXiuEntity`)

#### 1.2 注册实体
- [ ] 在 `CCEntities.java` 中注册 `PERSISTENT_GU_CULTIVATOR_CLONE`
- [ ] 配置属性 (血量、速度、攻击力)
- [ ] 创建渲染器 (复用 `SwordShadowCloneRenderer` 并适配)
