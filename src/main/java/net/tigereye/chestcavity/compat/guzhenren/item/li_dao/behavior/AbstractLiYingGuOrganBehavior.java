...
abstract class AbstractLiYingGuOrganBehavior extends AbstractLiDaoOrganBehavior
  ...
  private static final float BASE_CLONE_DAMAGE =
      BehaviorConfigAccess.getFloat(
          AbstractLiYingGuOrganBehavior.class, "BASE_CLONE_DAMAGE", 10.0f);
  ...
  protected AbstractLiYingGuOrganBehavior(ResourceLocation organId, String stateRoot) {
  ...
  public float onHit(
    ...
    float cloneDamage =
        (float) ((BASE_CLONE_DAMAGE + baseDamage * DAMAGE_RATIO) * (1.0 + liDaoIncrease));
    if (cloneDamage <= 0.0f) {
      return damage;
    }
  ...
  protected MultiCooldown createCooldown(ChestCavityInstance cc, ItemStack organ) {
    MultiCooldown.Builder builder =
        MultiCooldown.builder(OrganState.of(organ, stateRoot))
