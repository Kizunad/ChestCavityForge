...
public final class ZiLiGengShengGuOrganBehavior extends AbstractLiDaoOrganBehavior
  ...
  private void tickPassive(
    ...
    if (!payment.succeeded()) {
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug(
            "[compat/guzhenren][zi_li_geng_sheng] passive upkeep failed: {}",
  ...
  private void finishAbility(
    ...
    OrganStateOps.setDouble(
        state, cc, organ, ACTIVE_EFFICIENCY_KEY, 0.0, value -> Math.max(0.0, value), 0.0);
    if (entity != null) {
  ...
  private static void activateAbility(LivingEntity entity, ChestCavityInstance cc) {
    ...
    OrganStateOps.setDouble(
        ...
        cc,
        organ,
        ACTIVE_EFFICIENCY_KEY,
        Math.max(efficiency, 0.0),
        value -> Math.max(0.0, value),
        0.0);
    player.addEffect(
        new MobEffectInstance(
            MobEffects.REGENERATION, (int) ABILITY_DURATION_TICKS, 0, false, true, true));
  }
