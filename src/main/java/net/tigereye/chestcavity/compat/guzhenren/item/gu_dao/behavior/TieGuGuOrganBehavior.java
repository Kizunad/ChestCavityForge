 ...
 *
 * <p>Absorption hearts are granted sparingly: once per interval the behaviour checks the
 * bone-growth linkage pool ({@code SteelBoneComboHelper.tryConsumeBoneEnergy}) and only raises
 * absorption if the entity currently sits below the target amount. This keeps the plating from
 * constantly “topping off” gold hearts—consumption of the shield is intentional, and the upkeep is
 * paid with bone energy rather than refreshing on every hit. Actual healing is deferred to the
 * steel combo healer ({@link
 * net.tigereye.chestcavity.compat.guzhenren.item.gu_dao.SteelBoneComboHealing}).
 */
public final class TieGuGuOrganBehavior extends AbstractMetalBoneSupportBehavior {
  ...
  private static final int ABSORPTION_INTERVAL_TICKS =
      BehaviorConfigAccess.getInt(TieGuGuOrganBehavior.class, "ABSORPTION_INTERVAL_TICKS", 20 * 15);
