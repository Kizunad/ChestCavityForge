...
public abstract class AbstractGuzhenrenOrganBehavior {
  ...
  protected boolean matchesOrgan(
      ItemStack stack, ItemLike canonicalItem, ResourceLocation... organIds) {
  ...
  protected RemovalRegistration registerRemovalHook(
    ...
    boolean presentInCurrentList =
        cc.onRemovedListeners.stream()
            .anyMatch(
                existing ->
                    ChestCavityUtil.matchesRemovalContext(existing, slotIndex, organ, listener));
  ...
  protected void ensureChannels(
      ActiveLinkageContext context, Collection<ResourceLocation> channelIds) {
  ...
  protected void logStateChange(
      Logger logger, String prefix, ItemStack stack, String key, OrganState.Change<?> change) {
