...
public final class HuorenguOrganBehavior extends AbstractGuzhenrenOrganBehavior
  ...
  private static final ResourceLocation ORGAN_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "huorengu");
  ...
  private static final int FIRE_RESIST_DURATION_TICKS =
      BehaviorConfigAccess.getInt(HuorenguOrganBehavior.class, "FIRE_RESIST_DURATION_TICKS", 60);
  private static final int HASTE_DURATION_TICKS =
      BehaviorConfigAccess.getInt(HuorenguOrganBehavior.class, "HASTE_DURATION_TICKS", 60);
  private static final double SYNERGY_INCREASE = 0.26;
  private static final int FIRE_LINGER_SECONDS =
      BehaviorConfigAccess.getInt(HuorenguOrganBehavior.class, "FIRE_LINGER_SECONDS", 600);
  private static final int BURN_SOUND_INTERVAL_TICKS =
      BehaviorConfigAccess.getInt(HuorenguOrganBehavior.class, "BURN_SOUND_INTERVAL_TICKS", 40);

  private static final float SHOOT_SOUND_VOLUME =
      BehaviorConfigAccess.getFloat(HuorenguOrganBehavior.class, "SHOOT_SOUND_VOLUME", 0.8f);
  private static final float BURN_SOUND_VOLUME_BASE =
      BehaviorConfigAccess.getFloat(HuorenguOrganBehavior.class, "BURN_SOUND_VOLUME_BASE", 0.25f);
  ...
  public void onEquip(
    ...
    if (!registration.alreadyRegistered()) {
      OrganState state = organState(organ, STATE_ROOT);
      var change =
          state.setInt(SLOT_KEY, registration.slotIndex(), value -> Math.max(-1, value), -1);
  ...
  private void applyFireResistance(LivingEntity entity) {
    ...
    entity.addEffect(
        new MobEffectInstance(
            MobEffects.FIRE_RESISTANCE, FIRE_RESIST_DURATION_TICKS, 0, false, false, true));
  }

  private void handleFlight(
      LivingEntity entity, OrganState state, ChestCavityInstance cc, ItemStack organ) {
  ...
  private void handleSynergyState(
      LivingEntity entity,
    ...
    if (Math.abs(storedIncrease - targetIncrease) > EPSILON) {
      ...
      var change =
          state.setDouble(INCREASE_KEY, targetIncrease, value -> Math.max(0.0, value), 0.0);
      logStateChange(LOGGER, LOG_PREFIX, organ, INCREASE_KEY, change);
      if (change.changed()) {
        sendSlotUpdate(cc, organ);
      }
    ...
    entity.addEffect(
        new MobEffectInstance(MobEffects.DIG_SPEED, HASTE_DURATION_TICKS, 0, false, false, true));
  }
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (random.nextBoolean()) {
        spawnParticle(
            ...
            center.x + offsetX * 0.6,
            center.y + yOffset * 0.6,
            center.z + offsetZ * 0.6,
            0.0,
            0.01,
            0.0,
            0.005);
      ...
      if (synergyActive && random.nextFloat() < 0.45f) {
        spawnParticle(
            server,
            ParticleTypes.LAVA,
            center.x + offsetX * 0.35,
            center.y + yOffset * 0.6,
            center.z + offsetZ * 0.35,
            0.0,
            0.0,
            0.0,
            0.0);
        if (random.nextBoolean()) {
          spawnParticle(
              ...
              center.x + offsetX * 0.4,
              center.y + yOffset * 0.6,
              center.z + offsetZ * 0.4,
              0.0,
              0.0,
              0.0,
              0.01);
  ...
  private void spawnImpactParticles(ServerLevel server, LivingEntity target) {
    ...
    for (int i = 0; i < 12; i++) {
      ...
      if (target.getRandom().nextBoolean()) {
        spawnParticle(
            server, ParticleTypes.SOUL_FIRE_FLAME, x, pos.y + 0.05, z, 0.0, 0.02, 0.0, 0.01);
      }
    }
  }
  ...
  private void playLoopingBurn(
      Level level, LivingEntity entity, OrganState state, boolean synergyActive) {
    if (level == null) {
      return;
    }
    ...
    state.setLong(
        NEXT_BURN_SOUND_KEY,
        gameTime + BURN_SOUND_INTERVAL_TICKS,
        value -> Math.max(0L, value),
        0L);
  }

  private void playSound(
      Level level, LivingEntity entity, SoundEvent sound, float volume, float pitch) {
