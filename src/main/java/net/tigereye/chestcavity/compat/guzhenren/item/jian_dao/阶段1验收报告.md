...
1. **能力系统挂载** ✅ 已修复 (2025-11-14)
   ...
   - **实现代码** (NeoForge 1.21.1 API):
     ```java
     @Override
     public @Nullable Object getCapability(EntityCapability<?, ?> cap, @Nullable Direction side) {
     ...
     ```
   - **注意**: 使用 NeoForge 1.21.1 的新能力系统 API，返回 @Nullable Object 而非 LazyOptional
...
- ✅ 使用 NeoForge 1.21.1 的新 API（返回 @Nullable Object，而非 LazyOptional）
- ✅ 参数类型使用 EntityCapability<?, ?> 而非 Capability<T>

**修复历程**:
- 初次尝试使用了 LazyOptional（已废弃），导致编译错误
- 第二次修复采用正确的 NeoForge 1.21.1 API，编译成功
