...
public enum HuGuguOrganBehavior
    implements OrganSlowTickListener, OrganIncomingDamageListener, IncreaseEffectContributor {
  INSTANCE;
  ...
  private static final int CHARGE_SCALE =
      BehaviorConfigAccess.getInt(
          HuGuguOrganBehavior.class, "CHARGE_SCALE", 100); // store hundredths in NBT
  private static final int MAX_CHARGE_UNITS =
      BehaviorConfigAccess.getInt(HuGuguOrganBehavior.class, "MAX_CHARGE_UNITS", 20 * CHARGE_SCALE);
  private static final int HALF_CHARGE_UNITS =
      BehaviorConfigAccess.getInt(
          HuGuguOrganBehavior.class, "HALF_CHARGE_UNITS", MAX_CHARGE_UNITS / 2);
  private static final int BASE_RECOVERY_LOW_UNITS =
      BehaviorConfigAccess.getInt(
          HuGuguOrganBehavior.class,
          "BASE_RECOVERY_LOW_UNITS",
          (int) Math.round(0.25 * CHARGE_SCALE));
  private static final int BONUS_RECOVERY_LOW_UNITS =
      BehaviorConfigAccess.getInt(
          HuGuguOrganBehavior.class,
          "BONUS_RECOVERY_LOW_UNITS",
          (int) Math.round(0.25 * CHARGE_SCALE));
  private static final int RECOVERY_HIGH_UNITS =
      BehaviorConfigAccess.getInt(
          HuGuguOrganBehavior.class, "RECOVERY_HIGH_UNITS", (int) Math.round(0.10 * CHARGE_SCALE));
  ...
  private static final int LOW_CHARGE_DEBUFF_DURATION =
      BehaviorConfigAccess.getInt(
          HuGuguOrganBehavior.class,
          "LOW_CHARGE_DEBUFF_DURATION",
          60); // 3 seconds refresh per slow tick

  private static final int BASE_BUFF_DURATION =
      BehaviorConfigAccess.getInt(
          HuGuguOrganBehavior.class, "BASE_BUFF_DURATION", 20 * 60); // 1 minute in ticks

  private static final int DAMAGE_TRIGGER_COST_UNITS =
      BehaviorConfigAccess.getInt(
          HuGuguOrganBehavior.class, "DAMAGE_TRIGGER_COST_UNITS", CHARGE_SCALE);
  ...
  private static final int HU_GUGU_CONE_STEPS =
      BehaviorConfigAccess.getInt(HuGuguOrganBehavior.class, "HU_GUGU_CONE_STEPS", 12);
  ...
  private static void applyLowChargeDebuffs(Player player) {
    player.addEffect(
        new MobEffectInstance(MobEffects.WEAKNESS, LOW_CHARGE_DEBUFF_DURATION, 0, true, true));
    player.addEffect(
        new MobEffectInstance(
            MobEffects.MOVEMENT_SLOWDOWN, LOW_CHARGE_DEBUFF_DURATION, 0, true, true));
    player.addEffect(
        new MobEffectInstance(MobEffects.DIG_SLOWDOWN, LOW_CHARGE_DEBUFF_DURATION, 0, true, true));
    player.addEffect(
        new MobEffectInstance(MobEffects.HUNGER, LOW_CHARGE_DEBUFF_DURATION, 0, true, true));
  }
  ...
  private static void applyResistance(Player player, double increaseTotal, int duration) {
    ...
    player.addEffect(
        new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, duration, amplifier, true, true));
  }
  ...
  private static void applySpeed(Player player, double multiplier, int duration) {
    ...
    player.addEffect(
        new MobEffectInstance(MobEffects.MOVEMENT_SPEED, duration, amplifier, true, true));
  }
  ...
  private static void reflectDamage(
      DamageSource source, LivingEntity victim, float incomingDamage, double multiplier) {
    ...
    double reflectAmount =
        Math.min(BASE_MAX_REFLECT * multiplier, BASE_REFLECT_RATIO * incomingDamage * multiplier);
    if (reflectAmount <= 0) {
      return;
    }
  ...
  private static void knockbackAttacker(
      LivingEntity victim, LivingEntity attacker, double multiplier) {
  ...
  private static void spawnHuGuguConeBurst(ServerLevel serverLevel, LivingEntity victim) {
    ...
    Vec3 upReference =
        Math.abs(forward.y) > 0.95 ? new Vec3(1.0, 0.0, 0.0) : new Vec3(0.0, 1.0, 0.0);
    ...
    double distanceStep =
        (HU_GUGU_CONE_MAX_DISTANCE - HU_GUGU_CONE_MIN_DISTANCE) / HU_GUGU_CONE_STEPS;
  ...
  private static boolean consumeChargeUnits(
      ChestCavityInstance cc, ItemStack stack, int costUnits) {
    if (costUnits <= 0) {
      return true;
    }
