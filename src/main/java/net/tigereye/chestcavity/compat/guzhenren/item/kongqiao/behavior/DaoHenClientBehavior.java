...
final class DaoHenClientBehavior implements GuzhenrenPayloadListener {
  ...
  private static final int POLL_INTERVAL_TICKS =
      BehaviorConfigAccess.getInt(
          DaoHenClientBehavior.class, "POLL_INTERVAL_TICKS", 20); // ~1s at 20 TPS
  ...
  static void bootstrap() {
    if (!CLIENT_INITIALISED.compareAndSet(false, true)) {
      DaoHenBehavior.LOGGER.debug(
          "{} DaoHenBehavior client bootstrap skipped (already initialised)",
          DaoHenBehavior.LOG_PREFIX);
      return;
    }
    ...
    DaoHenBehavior.LOGGER.info(
        "{} DaoHenBehavior client hooks initialised (listenerAdded={}, poll={}t)",
        DaoHenBehavior.LOG_PREFIX,
        listenerAdded,
        POLL_INTERVAL_TICKS);
  }
  ...
  @Override
  public void onPlayerVariablesSynced(
