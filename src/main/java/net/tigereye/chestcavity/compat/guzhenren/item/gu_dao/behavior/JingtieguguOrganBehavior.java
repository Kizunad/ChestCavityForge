 ...
 *
 * <p>The refined plating follows the same absorption contract: once the minute-long interval
 * elapses it spends steel-combo bone energy to raise absorption only when the entity is below the
 * per-stack target. It never “instantly refill” absorption every tick, ensuring the shield drains
 * meaningfully during combat while the combo upkeep remains predictable. Direct healing is still
 * delegated to {@link net.tigereye.chestcavity.compat.guzhenren.item.gu_dao.SteelBoneComboHealing}.
 */
public final class JingtieguguOrganBehavior extends AbstractMetalBoneSupportBehavior {
  ...
  private static final int ABSORPTION_INTERVAL_TICKS =
      BehaviorConfigAccess.getInt(
          JingtieguguOrganBehavior.class, "ABSORPTION_INTERVAL_TICKS", 20 * 45); // 1 minute
