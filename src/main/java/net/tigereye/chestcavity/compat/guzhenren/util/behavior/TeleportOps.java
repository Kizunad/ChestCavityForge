 ...
 *
 * <p>Designed to provide deterministic blinks without duplicating caller-specific logic. Performs
 * conservative safety checks (chunk loaded, world border, collision) and supports limited vertical
 * adjustment when the target spot is obstructed.
 */
public final class TeleportOps {
  ...
  /**
   * Attempt to blink an entity towards the provided absolute position. Returns the final teleport
  ...
  public static Optional<Vec3> blinkTo(
      LivingEntity entity, Vec3 target, int verticalAttempts, double verticalStep) {
  ...
  /**
   * Attempt to blink an entity along the provided offset. The final destination equals {@code
   * entity.position() + offset} (subject to vertical adjustments).
   */
  ...
  /**
   * Attempt to blink the entity away from an anchor position by a fixed distance. When the anchor
   * coincides with the entity, the entity's look direction is used.
   */
