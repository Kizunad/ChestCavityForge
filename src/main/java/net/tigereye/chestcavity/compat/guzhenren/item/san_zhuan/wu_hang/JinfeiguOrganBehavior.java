...
public enum JinfeiguOrganBehavior
    implements OrganOnGroundListener, OrganSlowTickListener, IncreaseEffectContributor {
  INSTANCE;

  private static final int FULL_FOOD_LEVEL =
      BehaviorConfigAccess.getInt(JinfeiguOrganBehavior.class, "FULL_FOOD_LEVEL", 20);
  private static final int HUNGER_COST =
      BehaviorConfigAccess.getInt(JinfeiguOrganBehavior.class, "HUNGER_COST", 10);
  private static final double ABSORPTION_HEARTS = 60.0;
  private static final int EFFECT_DURATION_TICKS =
      BehaviorConfigAccess.getInt(JinfeiguOrganBehavior.class, "EFFECT_DURATION_TICKS", 20 * 60);
  ...
  public void onSlowTick(LivingEntity entity, ChestCavityInstance cc, ItemStack organ) {
    ...
    float targetAbsorption =
        Math.max(requiredAbsorption, heartsFromAmplifier(absorption.getAmplifier()));
  ...
  private static MobEffectInstance buildAbsorptionEffect(int stackCount) {
    ...
    return new MobEffectInstance(
        MobEffects.ABSORPTION, EFFECT_DURATION_TICKS, amplifier, false, false, true);
  }
