...
/**
 * Bone Bamboo Gu (骨竹蛊) grows a shared bone_growth linkage channel. Passive: each slow tick adds +5
 * per organ stack. Active: using bone meal near the player injects +20 per stack.
 */
public enum GuzhuguOrganBehavior implements OrganSlowTickListener, IncreaseEffectContributor {
  ...
  private static void sendDebugMessage(
      Player player, String action, double actual, double newValue, int sampleCount) {
  ...
  private static void spawnRing(
      ServerLevel server, Player player, ParticleOptions particle, int count, double radius) {
