...
/**
 * 简化的冲锋体（服务端权威），生成后沿指定方向高速移动，撞到非同盟单位造成伤害与击退，存活少量tick后消散。
 * 说明：实现为自定义 Entity（非 Mob），以避免复杂AI与渲染需求；符合“直接生成并冲刺”的期望。
 */
public class MadBullEntity extends Entity {

    private static final int DEFAULT_LIFESPAN = BehaviorConfigAccess.getInt(MadBullEntity.class, "LIFESPAN_TICKS", 40);
    private static final double DEFAULT_SPEED = BehaviorConfigAccess.getFloat(MadBullEntity.class, "SPEED_PER_TICK", 1.4f);
    private static final float DEFAULT_DAMAGE = BehaviorConfigAccess.getFloat(MadBullEntity.class, "DAMAGE", 8.0f);
    private static final double HIT_RADIUS = BehaviorConfigAccess.getFloat(MadBullEntity.class, "HIT_RADIUS", 0.8f);
    ...
    public void tick() {
        ...
        if (this.level() instanceof ServerLevel server) {
            ...
            for (LivingEntity target : server.getEntitiesOfClass(LivingEntity.class, sweep, this::canHit)) {
