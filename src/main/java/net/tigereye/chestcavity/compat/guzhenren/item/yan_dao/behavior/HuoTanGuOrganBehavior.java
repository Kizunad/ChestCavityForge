...
/**
 * 单窍·火炭蛊（Yan Dao）
 * - 使用通用 Reaction Tag：FIRE_MARK/IGNITE_WINDOW/FIRE_IMMUNE/IGNITE_AMP/CHAR_PRESSURE
 * - 简化版逻辑：命中叠燃痕，达阈值触发小型“炭爆”，清层并赋短免；产生少量蓄压（CHAR_PRESSURE）
 * - 参数从 GUZHENREN_BEHAVIOR 的动态表读取（默认值内置）
 */
public final class HuoTanGuOrganBehavior extends AbstractGuzhenrenOrganBehavior implements OrganOnHitListener {
    ...
    private static final ResourceLocation ORGAN_ID = ResourceLocation.fromNamespaceAndPath(MOD_ID, "dan_qiao_huo_tan_gu");
    ...
    public float onHit(DamageSource source, LivingEntity attacker, LivingEntity target, ChestCavityInstance cc, ItemStack organ, float damage) {
        ...
        int threshold = tier <= 1 ? BehaviorConfigAccess.getInt(HuoTanGuOrganBehavior.class, "burstThresholdTier1", 2)
        ...
        if (tier >= 2 && ReactionTagOps.has(target, ReactionTagKeys.IGNITE_WINDOW) && !ReactionTagOps.has(target, IGNITE_BONUS_CD)) {
            ReactionTagOps.addStacked(target, ReactionTagKeys.FLAME_MARK, 1, markDuration);
        ...
        if (stacks >= threshold) {
            ...
            // 提示（系统消息，便于验证）
            i18n(attacker, "message.chestcavity.reaction.charcoal_pop.attacker", target.getName().getString());
            i18n(target, "message.chestcavity.reaction.charcoal_pop.target", attacker.getName().getString());
        }

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("[compat/guzhenren][yan_dao][huo_tan] onHit: tier={} stacks={} threshold={} target=\"{}\"", tier, stacks, threshold, target.getName().getString());
        }
        ...
        if (tier >= 4 && attacker instanceof Player && attacker.level() instanceof ServerLevel slevel2) {
            if (net.tigereye.chestcavity.util.reaction.tag.ReactionTagOps.has(player, net.tigereye.chestcavity.util.reaction.tag.ReactionTagKeys.IGNITE_AMP)
                ...
                if (count >= 2) {
                    ...
                    net.tigereye.chestcavity.util.reaction.tag.ReactionTagOps.add(player, net.tigereye.chestcavity.ChestCavity.id("reaction/chain_blast_cd"), cd);
                }
            }
        }
        return damage;
    ...
    private static final ResourceLocation IGNITE_BONUS_CD = net.tigereye.chestcavity.ChestCavity.id("reaction/ignite_bonus_cd");
    ...
    private static void activateExtreme(LivingEntity entity, ChestCavityInstance cc) {
        ...
        var HC = net.tigereye.chestcavity.ChestCavity.config.REACTION.HUO_TAN;
        var payment = net.tigereye.chestcavity.compat.guzhenren.util.behavior.ResourceOps.consumeStrict(player, HC.extremeZhenyuanCost, HC.extremeJingliCost);
        ...
        AABB box = new AABB(center.x - ER, center.y - ER, center.z - ER,
                center.x + ER, center.y + ER, center.z + ER);
        java.util.List<LivingEntity> targets = server.getEntitiesOfClass(LivingEntity.class, box,
                t -> t != null && t.isAlive() && t != player && !t.isAlliedTo(player) && t.distanceToSqr(center) <= ER * ER);
        ...
        for (int i = 0; i < EXTREME_SEGMENTS; i++) {
            final int phase = i;
            TickOps.schedule(server, () -> applyStormPhase(server, player, damages, phases[phase]), EXTREME_SEGMENT_TICKS * i);
        }
    ...
    private static void applyStormPhase(ServerLevel level, Player caster, java.util.Map<java.util.UUID, Float> damages, float factor) {
