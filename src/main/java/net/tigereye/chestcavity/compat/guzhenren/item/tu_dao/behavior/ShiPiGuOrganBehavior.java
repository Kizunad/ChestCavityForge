 ...
 *
 * <p>Notes: - Works for both players and mobs (LivingEntity) on the server side. - Players simply
 * receive the timed absorption and damage mitigation; no Guzhenren resource (zhenyuan/jingli) cost
 * is applied by this organ. - Mobs do not have Guzhenren resources. Per request, an embedded
 * non‑player handler is provided at the bottom of this file to convert any future Tu Dao costs into
 * health (HP) at 100:1.
 */
public enum ShiPiGuOrganBehavior implements OrganSlowTickListener, OrganIncomingDamageListener {
  ...
  private static final int MAX_CHARGE =
      BehaviorConfigAccess.getInt(ShiPiGuOrganBehavior.class, "MAX_CHARGE", 10);
  private static final int RECOVERY_INTERVAL_SLOW_TICKS =
      BehaviorConfigAccess.getInt(
          ShiPiGuOrganBehavior.class,
          "RECOVERY_INTERVAL_SLOW_TICKS",
          10); // 10 seconds (slow tick fires once per second)

  private static final int BLOCK_PARTICLE_MIN =
      BehaviorConfigAccess.getInt(ShiPiGuOrganBehavior.class, "BLOCK_PARTICLE_MIN", 15);
  private static final int BLOCK_PARTICLE_MAX =
      BehaviorConfigAccess.getInt(ShiPiGuOrganBehavior.class, "BLOCK_PARTICLE_MAX", 20);
  private static final int DUST_PARTICLE_COUNT =
      BehaviorConfigAccess.getInt(ShiPiGuOrganBehavior.class, "DUST_PARTICLE_COUNT", 5);
  ...
  private static void applyAbsorption(LivingEntity entity, ChestCavityInstance cc, int charge) {
    ...
    double baseLevel =
        charge > 0 ? Math.max(1.0, Math.round((double) charge / (double) MAX_CHARGE)) : 0.0;
  ...
  private void scheduleRechargeIfNeeded(
      LivingEntity entity, ChestCavityInstance cc, ItemStack organ) {
    if (entity == null
    ...
    MultiCooldown cooldown =
        MultiCooldown.builder(OrganState.of(organ, STATE_KEY)).withSync(cc, organ).build();
  ...
  private static void spawnDamageParticles(Level level, LivingEntity entity) {
    ...
    server.sendParticles(
        new BlockParticleOption(ParticleTypes.BLOCK, COBBLESTONE_STATE),
        center.x,
        center.y,
        center.z,
        blockCount,
        ...
        0.15);
    server.sendParticles(
        STONE_DUST, center.x, center.y, center.z, DUST_PARTICLE_COUNT, 0.5, 0.3, 0.5, 0.05);
  }

  /**
   * Embedded non-player handler for Tu Dao style health fallback.
   *
   * <p>Why this exists here (instead of using a shared helper): - The user asked for
   * TuDaoNonPlayerHandler to be embedded into the organ behavior to ensure logic is self-contained
   * and remains stable even if external helpers change. - Mobs lack Guzhenren attachments (no
   * zhenyuan/jingli). When this organ (or future Tu Dao logic) requires a resource payment, convert
   * the resource total to HP at a fixed 100:1 ratio and drain after absorption, without outright
   * killing the entity.
   *
   * <p>Usage: EmbeddedTuDaoNonPlayerHandler.payWithHealth(entity, zhenyuanBaseCost, jingliBaseCost)
   *
   * <p>Threading: - Call from the server thread only (no client mutations).
   */
  private static final class EmbeddedTuDaoNonPlayerHandler {
    ...
    private static final float EPS =
        BehaviorConfigAccess.getFloat(ShiPiGuOrganBehavior.class, "EPS", 1.0E-4f);
