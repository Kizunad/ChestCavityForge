...
/**
 * 黄金舍利蛊（四转）：
 * - 被动：持续性伤害 -25%（火焰/凋零/中毒类/流血等），按 OnIncomingDamage 拦截
 * - 主动（60s冷却）：金刚定身：6s 免击退 + 抗性III，自身缓慢III；并对 8 格内敌对目标施加缓慢IV 6s
 */
public enum HuangJinSheLiGuOrganBehavior implements OrganSlowTickListener, OrganIncomingDamageListener {
    INSTANCE;

    private static final String MOD_ID = "guzhenren";
    private static final ResourceLocation ORGAN_ID = ResourceLocation.fromNamespaceAndPath(MOD_ID, "huang_jin_she_li_gu");
    ...
    private static final ResourceLocation KNOCKBACK_RESIST_ID = ResourceLocation.fromNamespaceAndPath(MOD_ID, "modifiers/no_knockback_huang_jin");
    ...
    public static void activateAbility(LivingEntity entity, ChestCavityInstance cc) {
        ...
        for (LivingEntity target : entity.level().getEntitiesOfClass(LivingEntity.class, area, t -> t != null && t.isAlive() && t != entity)) {
            if (isHostileTo(entity, target)) {
                target.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, ACTIVE_DURATION_TICKS, 3, false, true, true));
            }
        }
    ...
    private static void grantNoKnockback(LivingEntity entity, int durationTicks) {
        var attr = entity.getAttribute(net.minecraft.world.entity.ai.attributes.Attributes.KNOCKBACK_RESISTANCE);
        if (attr == null) return;
        ...
        var mod = new net.minecraft.world.entity.ai.attributes.AttributeModifier(
                KNOCKBACK_RESIST_ID, 1.0, net.minecraft.world.entity.ai.attributes.AttributeModifier.Operation.ADD_VALUE);
        attr.addTransientModifier(mod);
        if (entity.level() instanceof ServerLevel server) {
            net.tigereye.chestcavity.compat.guzhenren.util.behavior.TickOps.schedule(server, () -> {
                var a = entity.getAttribute(net.minecraft.world.entity.ai.attributes.Attributes.KNOCKBACK_RESISTANCE);
