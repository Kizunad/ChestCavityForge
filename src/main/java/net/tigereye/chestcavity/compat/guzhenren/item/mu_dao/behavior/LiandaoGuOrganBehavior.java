...
/**
 * Active behaviour for 镰刀蛊. Handles resource consumption, cooldown management and the delayed blade
 * wave attack sequence.
 */
public enum LiandaoGuOrganBehavior implements OrganSlowTickListener, OrganIncomingDamageListener {
  ...
  private static final int CHARGE_DURATION_TICKS =
      BehaviorConfigAccess.getInt(LiandaoGuOrganBehavior.class, "CHARGE_DURATION_TICKS", 20);
  private static final int RELEASE_TELEGRAPH_TICKS =
      BehaviorConfigAccess.getInt(LiandaoGuOrganBehavior.class, "RELEASE_TELEGRAPH_TICKS", 5);
  private static final int MIN_COOLDOWN_TICKS =
      BehaviorConfigAccess.getInt(
          LiandaoGuOrganBehavior.class, "MIN_COOLDOWN_TICKS", 160); // 8 seconds
  private static final int COOLDOWN_VARIANCE_TICKS =
      BehaviorConfigAccess.getInt(
          LiandaoGuOrganBehavior.class,
          "COOLDOWN_VARIANCE_TICKS",
          80); // +0-80 ticks (8-12s window)
  ...
  private static final int EFFECT_DURATION_TICKS =
      BehaviorConfigAccess.getInt(
          LiandaoGuOrganBehavior.class, "EFFECT_DURATION_TICKS", 20); // 1 second
  ...
  private static void activateAbility(LivingEntity entity, ChestCavityInstance cc) {
    ...
    Vec3 forward =
        baseForward.lengthSqr() < EPSILON ? new Vec3(0.0, 0.0, 1.0) : baseForward.normalize();
    ...
    TickOps.schedule(
        serverLevel,
        () -> playTelegraph(serverLevel, telegraphOrigin, dir, wAxis),
    ...
    TickOps.schedule(
        serverLevel,
        () ->
            applyBladeWave(serverLevel, player, cc, origin, dir, wAxis, damageAmount, impactCenter),
  ...
  public float onIncomingDamage(
    ...
    int cooldown =
        MIN_COOLDOWN_TICKS
            + (COOLDOWN_VARIANCE_TICKS > 0 ? random.nextInt(COOLDOWN_VARIANCE_TICKS + 1) : 0);
  ...
  private static void playChargeStartEffects(ServerLevel serverLevel, Player player) {
    ...
    serverLevel.playSound(
        null, x, y, z, SoundEvents.TRIDENT_RETURN, SoundSource.PLAYERS, 0.7f, 0.7f);
    serverLevel.playSound(
        null, x, y, z, SoundEvents.BEACON_ACTIVATE, SoundSource.PLAYERS, 0.8f, 1.2f);
  }
  ...
  private static void spawnChargeParticles(ServerLevel serverLevel, Player player, int stage) {
    ...
    serverLevel.sendParticles(
        ParticleTypes.ENCHANT, centerX, centerY, centerZ, 6, 0.2, 0.25, 0.2, 0.01);
  }
  ...
  private static void applyBladeWave(
    ...
    AABB hitbox =
        new AABB(origin, origin.add(dir.scale(WAVE_LENGTH)))
            .inflate(WAVE_HALF_WIDTH, WAVE_HALF_HEIGHT, WAVE_HALF_WIDTH);

    List<LivingEntity> targets =
        server.getEntitiesOfClass(LivingEntity.class, hitbox, e -> e != user && e.isAlive());
    ...
    for (LivingEntity target : targets) {
      ...
      target.addEffect(
          new MobEffectInstance(
              MobEffects.MOVEMENT_SLOWDOWN, EFFECT_DURATION_TICKS, 1, false, true, true));
      target.addEffect(
          new MobEffectInstance(MobEffects.WEAKNESS, EFFECT_DURATION_TICKS, 0, false, true, true));
  ...
  private static void spawnResidualParticles(
      ServerLevel s, Vec3 origin, Vec3 dir, Vec3 wAxis) { // CHANGED
  ...
  private static void affectBlocksAlongSlash(
      ServerLevel s, Vec3 origin, Vec3 dir, Vec3 wAxis, LivingEntity user) { // OPTIONAL
    if (!(user instanceof Player player)) {
      return;
    }
  ...
  private static void processBlock(ServerLevel serverLevel, Player player, BlockPos pos) {
    ...
    if (isSoftBlock(state)) {
      ...
      serverLevel.playSound(
          null, pos, state.getSoundType().getHitSound(), SoundSource.PLAYERS, 0.5f, 1.1f);
      // 额外的金属"叮"声
      serverLevel.playSound(
          null,
          pos,
  ...
  private static MultiCooldown createCooldown(ChestCavityInstance cc, ItemStack organ) {
    MultiCooldown.Builder builder =
        MultiCooldown.builder(organ, STATE_KEY).withLongClamp(value -> Math.max(0L, value), 0L);
