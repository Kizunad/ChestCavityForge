...
public enum XingBanDianGuOrganBehavior implements OrganIncomingDamageListener {
  ...
  private static final ResourceLocation ORGAN_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "xing_ban_dian_gu");
  ...
  private static MultiCooldown createSharedCooldown(ChestCavityInstance cc, ItemStack primary) {
    MultiCooldown.Builder builder =
        MultiCooldown.builder(OrganState.of(primary, STATE_ROOT))
  ...
  private static void playFx(LivingEntity victim) {
    ...
    for (int i = 0; i < 24; i++) {
      ...
      server.sendParticles(
          ...
          victim.getX(),
          victim.getY() + victim.getBbHeight() * 0.6,
          victim.getZ(),
          1,
          dx,
          dy,
    ...
    Vec3 center =
        new Vec3(victim.getX(), victim.getY() + victim.getBbHeight() * 0.55, victim.getZ());
