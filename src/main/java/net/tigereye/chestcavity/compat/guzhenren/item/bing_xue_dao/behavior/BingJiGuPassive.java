...
public class BingJiGuPassive implements PassiveHook {
    ...
    public void onTick(LivingEntity owner, ChestCavityInstance cc, long now) {
        ...
        BingJiGuOrganBehavior.INSTANCE.onSlowTick(owner, cc, organ);
    }
    ...
    public void onHurt(LivingEntity self, DamageSource source, float amount, ChestCavityInstance cc, long now) {
        ...
        BingJiGuOrganBehavior.INSTANCE.onIncomingDamage(source, self, cc, organ, amount);
    }

    @Override
    public void onHitMelee(LivingEntity attacker, LivingEntity target, ChestCavityInstance cc, long now) {
        ItemStack organ = findOrgan(cc);
        if (organ.isEmpty()) {
            return;
        }
        BingJiGuOrganBehavior.INSTANCE.onHit(null, attacker, target, cc, organ, 0);
    }
