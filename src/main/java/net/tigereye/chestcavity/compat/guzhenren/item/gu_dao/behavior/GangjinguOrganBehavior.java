...
public final class GangjinguOrganBehavior extends AbstractGuzhenrenOrganBehavior implements OrganSlowTickListener, OrganOnHitListener {
    ...
    private static final int ABSORPTION_INTERVAL_TICKS = BehaviorConfigAccess.getInt(GangjinguOrganBehavior.class, "ABSORPTION_INTERVAL_TICKS", 20 * 30); // 45s
     ...
     */
    private static final float NON_PLAYER_MAINTENANCE_HEALTH_COST = BehaviorConfigAccess.getFloat(GangjinguOrganBehavior.class, "NON_PLAYER_MAINTENANCE_HEALTH_COST", 0.2f);
    /** Leaves at least one heart to prevent upkeep from finishing off weakened mobs. */
    private static final float NON_PLAYER_MINIMUM_HEALTH_RESERVE = BehaviorConfigAccess.getFloat(GangjinguOrganBehavior.class, "NON_PLAYER_MINIMUM_HEALTH_RESERVE", 2.0f);
    ...
    private void scheduleSteelAbsorptionIfNeeded(LivingEntity entity, ChestCavityInstance cc, ItemStack organ, int steelStacks) {
        ...
        ready.onReady(server, now, () -> {
            try {
                ...
                MultiCooldown.Entry e = MultiCooldown.builder(state).withSync(cc, organ).build().entry(ABSORPTION_READY_AT_KEY);
                e.setReadyAt(next);
                e.onReady(server, server.getGameTime(), () -> {});
                OrganStateOps.setLong(state, cc, organ, ABSORPTION_KEY, server.getGameTime(), v -> v, Long.MIN_VALUE);
            }
        });
    }
