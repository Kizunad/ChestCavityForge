 ...
 * 拦截运动寻路器
 *
 * <p>实现护幕飞剑的拦截轨迹算法,让飞剑以最大速度、最优路径快速到达拦截点。
 *
 * <h3>算法特性</h3>
 *
 * <ul>
 *   <li><b>全速冲刺</b>: 拦截阶段始终保持最大速度
 *   <li><b>预判轨迹</b>: 考虑投射物运动,提前预判拦截点
 *   <li><b>精确刹车</b>: 到达拦截点时平滑减速并停留
 *   <li><b>轨迹优化</b>: 计算最短时间到达路径
 * </ul>
 *
 * <h3>与ORBIT的差异</h3>
 *
...
public final class InterceptPathfinder {
   ...
   *
   * <ol>
   *   <li><b>加速阶段</b>: 全力加速到最大速度
   *   <li><b>巡航阶段</b>: 保持最大速度直线飞向拦截点
   *   <li><b>刹车阶段</b>: 提前计算刹车距离,平滑减速到停止
   * </ol>
   *
   * @param ctx 寻路上下文
  ...
  public static Vec3 computeInterceptVelocity(PathfindingContext ctx) {
    ...
    // 加上安全余量
    double effectiveBrakeDistance =
        Math.max(MIN_BRAKE_DISTANCE, theoreticalBrakeDistance * BRAKE_SAFETY_MARGIN);
  ...
  private static Vec3 computeAcceleratingVelocity(
      PathfindingContext ctx, Vec3 direction, double currentSpeed) {
  ...
  private static Vec3 computeBrakingVelocity(
      PathfindingContext ctx, Vec3 direction, double distance, double currentSpeed) {
    // 根据剩余距离计算理想刹车速度
    // v = sqrt(2 * a * d)
