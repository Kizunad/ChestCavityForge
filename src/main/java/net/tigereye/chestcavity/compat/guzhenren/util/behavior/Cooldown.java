...
public final class Cooldown {
    ...
    private Cooldown(OrganState state, String key, java.util.function.BiConsumer<Long, Long> onChange) {
        this.state = state;
        this.key = key;
    ...
    public void setReadyAt(long readyTick) {
        ...
        if (onChange != null && prev != curr) { onChange.accept(prev, curr); }
    }
    ...
    public static final class Int {
        ...
        private Int(OrganState state, String key, java.util.function.BiConsumer<Integer, Integer> onChange) {
            this.state = state;
            this.key = key;
            this.onChange = onChange;
        }

        public static Int of(OrganState state, String key) { return new Int(state, key, null); }
        public static Int bind(ItemStack stack, String rootKey, String key) { return of(OrganState.of(stack, rootKey), key); }
        public Int withOnChange(java.util.function.BiConsumer<Integer, Integer> onChange) { return new Int(this.state, this.key, onChange); }
        public int getTicks() { return Math.max(0, state.getInt(key, 0)); }
        public void setTicks(int ticks) { int prev = getTicks(); state.setInt(key, Math.max(0, ticks)); int curr = getTicks(); if (onChange != null && prev != curr) { onChange.accept(prev, curr); } }
        public void clear() { setTicks(0); }
