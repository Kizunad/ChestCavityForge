...
public final class TiPoGuOrganBehavior extends AbstractGuzhenrenOrganBehavior
  ...
  private static final ResourceLocation SHIELD_MODIFIER_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "modifiers/ti_po_gu_shield");
  ...
  private boolean updateIncreaseContribution(
      ChestCavityInstance cc, ItemStack organ, boolean requestActive) {
  ...
  private void maybeRefreshShield(
    ...
    if (soulBeast) {
      ...
      logStateChange(
          ...
          OrganStateOps.setDouble(
              state, cc, organ, KEY_LAST_SHIELD_AMOUNT, 0.0D, value -> value, 0.0D));
    ...
    boolean shouldRefresh =
        lastRefresh == Long.MIN_VALUE || currentTick - lastRefresh >= SHIELD_REFRESH_INTERVAL_TICKS;
    if (!shouldRefresh) {
      return;
    }
    double maxHunpo = handle.read("zuida_hunpo").orElse(0.0D);
    ...
    logStateChange(
        ...
        KEY_LAST_SHIELD_AMOUNT,
        OrganStateOps.setDouble(
            state, cc, organ, KEY_LAST_SHIELD_AMOUNT, shieldValue, value -> value, 0.0D));
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug(
  ...
  private static MultiCooldown createCooldown(ChestCavityInstance cc, ItemStack organ) {
    MultiCooldown.Builder builder =
        MultiCooldown.builder(OrganState.of(organ, STATE_ROOT_KEY))
