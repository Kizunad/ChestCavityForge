 ...
 *
 * <ul>
 *   <li>Consumes 1000 zhenyuan each slow tick;
 *   <li>Restores 3念头/秒 up to the "念头容量" soft cap;
 *   <li>If the equipped player has 逍遥智心体体质 (tizhi == 8)，restoration gains an extra 0.12 念头/秒;
 *   <li>The restoration bonus does not stack across multiple copies.
 * </ul>
 */
public final class LingGuangYiShanGuOrganBehavior extends AbstractGuzhenrenOrganBehavior
    implements OrganSlowTickListener {

  public static final LingGuangYiShanGuOrganBehavior INSTANCE =
      new LingGuangYiShanGuOrganBehavior();
  ...
  public void onSlowTick(LivingEntity entity, ChestCavityInstance cc, ItemStack organ) {
    ...
    ConsumptionResult payment =
        ResourceOps.consumeStrict(player, BASE_ZHENYUAN_COST_PER_SECOND, 0.0);
    if (!payment.succeeded()) {
      logDebug("zhenyuan payment failed", player, 0.0, 0.0, payment);
      return;
    }

    OptionalDouble handleOpt =
  ...
  private OptionalDouble applyRecovery(Player player, ResourceHandle handle) {
    ...
    OptionalDouble adjusted =
        ResourceOps.tryAdjustDouble(handle, "niantou", appliedAmount, true, "niantou_zuida");
  ...
  private void logDebug(
      String action, Player player, double delta, double result, ConsumptionResult payment) {
