...
/**
 * Client-only skin resolver that registers Minecraft skin textures (and optional overlays) for
 * arbitrary profiles.
 */
@OnlyIn(Dist.CLIENT)
public final class SkinResolver {
  ...
  private static ResourceLocation resolveBase(SkinHandle handle) {
    ...
    if (hash != null) {
      ...
      CompletableFuture<ResourceLocation> download =
          INFLIGHT.computeIfAbsent(hash, h -> downloadSkinAsync(url, h));
      if (download.isDone()) {
  ...
  private static CompletableFuture<ResourceLocation> registerTextureAsync(
      NativeImage image, String hash) {
    if (image == null) {
        ...
        () -> {
          try {
            ...
            ResourceLocation location =
                ResourceLocation.fromNamespaceAndPath("guzhenren", "dynamic_skins/" + hash);
