...
public class ShouPiGuPassive implements PassiveHook {
  ...
  public void onTick(LivingEntity owner, ChestCavityInstance cc, long now) {
    ShouPiGuOrganBehavior.INSTANCE.onSlowTick(owner, cc, ShouPiGuOrganBehavior.findOrgan(cc));
  }

  @Override
  public void onHurt(
      LivingEntity self, DamageSource source, float amount, ChestCavityInstance cc, long now) {
    ShouPiGuOrganBehavior.INSTANCE.onIncomingDamage(
        source, self, cc, ShouPiGuOrganBehavior.findOrgan(cc), amount);
  }

  @Override
  public void onHitMelee(
      LivingEntity attacker, LivingEntity target, ChestCavityInstance cc, long now) {
    ShouPiGuOrganBehavior.INSTANCE.onHit(
        null, attacker, target, cc, ShouPiGuOrganBehavior.findOrgan(cc), 0);
  }
}
