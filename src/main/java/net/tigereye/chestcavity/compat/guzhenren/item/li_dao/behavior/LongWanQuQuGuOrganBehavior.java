...
public final class LongWanQuQuGuOrganBehavior extends AbstractLiDaoOrganBehavior
  ...
  private static final int MAX_CHARGES =
      BehaviorConfigAccess.getInt(LongWanQuQuGuOrganBehavior.class, "MAX_CHARGES", 3);
  ...
  private static final float MIN_DODGE_DISTANCE =
      BehaviorConfigAccess.getFloat(LongWanQuQuGuOrganBehavior.class, "MIN_DODGE_DISTANCE", 0.9f);
  private static final float MAX_DODGE_DISTANCE =
      BehaviorConfigAccess.getFloat(LongWanQuQuGuOrganBehavior.class, "MAX_DODGE_DISTANCE", 1.4f);
  private static final float DODGE_YAW_RANGE =
      BehaviorConfigAccess.getFloat(LongWanQuQuGuOrganBehavior.class, "DODGE_YAW_RANGE", 100.0f);
  ...
  public float onIncomingDamage(
    ...
    if (charges <= 0) {
      boolean dirty =
          OrganStateOps.setBoolean(state, cc, organ, ACTIVE_KEY, false, false).changed();
      if (dirty) {
    ...
    dirty |=
        OrganStateOps.setInt(
                ...
                CHARGES_KEY,
                remaining,
                value -> Math.max(0, Math.min(value, MAX_CHARGES)),
                0)
            .changed();
