...
public final class LiDaoHelper {
  ...
  private static final ResourceLocation HUMAN_MUSCLE =
      ResourceLocation.fromNamespaceAndPath("chestcavity", "muscle");
  ...
  public static double getLiDaoIncrease(ChestCavityInstance cc) {
    ...
    LinkageChannel channel =
        context
            .lookupChannel(LiDaoConstants.LI_DAO_INCREASE_EFFECT)
  ...
  public static String describeMuscleContribution(double base, double liDaoIncrease) {
    return String.format(
        Locale.ROOT, "%.2f (base) * (1 + %.3f)", base, Math.max(0.0, liDaoIncrease));
  }
}
