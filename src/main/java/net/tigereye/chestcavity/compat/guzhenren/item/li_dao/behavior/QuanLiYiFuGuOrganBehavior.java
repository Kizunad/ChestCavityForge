...
public final class QuanLiYiFuGuOrganBehavior extends AbstractLiDaoOrganBehavior
  ...
  private static final int PULSE_INTERVAL_SECONDS =
      BehaviorConfigAccess.getInt(QuanLiYiFuGuOrganBehavior.class, "PULSE_INTERVAL_SECONDS", 15);
  private static final int RETRY_INTERVAL_SECONDS =
      BehaviorConfigAccess.getInt(QuanLiYiFuGuOrganBehavior.class, "RETRY_INTERVAL_SECONDS", 3);

  private static final int MAX_STORED_COOLDOWN =
      BehaviorConfigAccess.getInt(
          QuanLiYiFuGuOrganBehavior.class, "MAX_STORED_COOLDOWN", PULSE_INTERVAL_SECONDS);
  ...
  private boolean decrementCooldown(OrganState state, ChestCavityInstance cc, ItemStack organ) {
    ...
    if (next != remaining) {
      OrganStateOps.setIntSync(
          ...
          COOLDOWN_KEY,
          next,
          v -> Math.max(0, Math.min(v, MAX_STORED_COOLDOWN)),
          0);
    }
    return true;
  }
  ...
  private void setCooldownSeconds(
      OrganState state, ChestCavityInstance cc, ItemStack organ, int seconds) {
    int stored;
    if (seconds <= 0) {
      stored = 0;
    } else {
