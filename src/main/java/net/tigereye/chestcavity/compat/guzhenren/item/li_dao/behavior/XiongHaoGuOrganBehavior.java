...
/** 熊豪蛊（力道·肌肉）： 主动：消耗 300 BASE 真元，10 秒内每次近战命中额外消耗 6 精力并追加 10 点伤害。 冷却：25 秒；仅首个插槽生效。 */
public final class XiongHaoGuOrganBehavior extends AbstractLiDaoOrganBehavior
  ...
  private static final int ACTIVE_DURATION_TICKS =
      BehaviorConfigAccess.getInt(XiongHaoGuOrganBehavior.class, "ACTIVE_DURATION_TICKS", 10 * 20);
  private static final int COOLDOWN_TICKS =
      BehaviorConfigAccess.getInt(XiongHaoGuOrganBehavior.class, "COOLDOWN_TICKS", 25 * 20);
  private static final double ACTIVE_BASE_ZHENYUAN_COST =
      BehaviorConfigAccess.getFloat(
          XiongHaoGuOrganBehavior.class, "ACTIVE_BASE_ZHENYUAN_COST", 300.0f);
  private static final double PER_ATTACK_JINGLI_COST =
      BehaviorConfigAccess.getFloat(XiongHaoGuOrganBehavior.class, "PER_ATTACK_JINGLI_COST", 6.0f);
  private static final float EXTRA_DAMAGE =
      BehaviorConfigAccess.getFloat(XiongHaoGuOrganBehavior.class, "EXTRA_DAMAGE", 10.0f);
  ...
  private void deactivate(
      ChestCavityInstance cc, ItemStack organ, OrganState state, MultiCooldown cooldown) {
