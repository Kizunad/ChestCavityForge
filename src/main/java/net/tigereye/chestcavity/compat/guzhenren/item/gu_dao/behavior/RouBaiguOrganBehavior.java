...
public enum RouBaiguOrganBehavior
  ...
  private static final SaturationPolicy SOFT_CAP_POLICY =
      new SaturationPolicy(SOFT_CAP, SOFT_CAP_FALLOFF);
  ...
  private static final float DIGEST_EXHAUSTION_PER_SECOND =
      BehaviorConfigAccess.getFloat(
          RouBaiguOrganBehavior.class,
          "DIGEST_EXHAUSTION_PER_SECOND",
          (float) (COST_HUNGER / 60.0));
  private static final float BASE_EXTRA_HEAL =
      BehaviorConfigAccess.getFloat(RouBaiguOrganBehavior.class, "BASE_EXTRA_HEAL", 0.5f);
  private static final double BASE_REGEN_BONUS = 0.5;
  private static final int OUT_OF_COMBAT_THRESHOLD_TICKS =
      BehaviorConfigAccess.getInt(
          RouBaiguOrganBehavior.class, "OUT_OF_COMBAT_THRESHOLD_TICKS", 20 * 5);

  private static final double BITE_TRIGGER_CHANCE = 0.10;
  private static final int BITE_COOLDOWN_TICKS =
      BehaviorConfigAccess.getInt(RouBaiguOrganBehavior.class, "BITE_COOLDOWN_TICKS", 20 * 5);
  private static final double BITE_MAX_DISTANCE_SQR = 10.0 * 10.0;

  private static final int RESTORATION_SELECTION_INTERVAL_SECONDS =
      BehaviorConfigAccess.getInt(
          RouBaiguOrganBehavior.class, "RESTORATION_SELECTION_INTERVAL_SECONDS", 180);
  private static final int RESTORATION_WORK_INTERVAL_SECONDS =
      BehaviorConfigAccess.getInt(
          RouBaiguOrganBehavior.class, "RESTORATION_WORK_INTERVAL_SECONDS", 120);
  private static final int RESTORATION_PROGRESS_PER_STEP =
      BehaviorConfigAccess.getInt(RouBaiguOrganBehavior.class, "RESTORATION_PROGRESS_PER_STEP", 10);
  private static final int RESTORATION_REQUIRED_PROGRESS =
      BehaviorConfigAccess.getInt(
          RouBaiguOrganBehavior.class, "RESTORATION_REQUIRED_PROGRESS", 100);
  ...
  private float handlePlayerHit(
      Player player, LivingEntity victim, ChestCavityInstance cc, ItemStack organ, float damage) {
    if (!isEligibleVictim(victim) || !isWithinBiteRange(player, victim)) {
      return damage;
    }

    MultiCooldown cooldown = createCooldown(cc, organ);
    MultiCooldown.Entry biteCooldown =
        cooldown.entry(LAST_BITE_TICK_KEY).withDefault(0L).withClamp(value -> Math.max(0L, value));
  ...
  private static boolean isWithinBiteRange(LivingEntity attacker, LivingEntity victim) {
    return attacker != null
        && victim != null
        && attacker.distanceToSqr(victim) <= BITE_MAX_DISTANCE_SQR;
  }
  ...
  private static MultiCooldown createCooldown(ChestCavityInstance cc, ItemStack organ) {
    MultiCooldown.Builder builder =
        MultiCooldown.builder(OrganState.of(organ, STATE_KEY))
  ...
  private static boolean hasArmorAdvantage(
      LivingEntity attacker, LivingEntity victim, double increaseSum) {
  ...
  private static void playBiteFailure(LivingEntity attacker) {
    ...
    level.playSound(
        null, attacker.blockPosition(), SoundEvents.SKELETON_STEP, SoundSource.HOSTILE, 0.5f, 1.6f);
  }
  ...
  private static void playBiteSuccess(LivingEntity attacker, LivingEntity target) {
    ...
    level.playSound(
        ...
        0.9f);
    level.playSound(
        null, target.blockPosition(), SoundEvents.SKELETON_HURT, SoundSource.HOSTILE, 0.6f, 1.2f);
    if (level instanceof ServerLevel server) {
      spawnBloodSpray(server, target);
    }
  }

  private static void applyPlayerDigestiveGrowth(
      Player player, LinkageChannel boneChannel, double efficiency) {
  ...
  private static void applyMobDigestiveGrowth(
      LivingEntity entity, LinkageChannel boneChannel, double efficiency) {
  ...
  private static void applyPlayerPassiveHealing(
      Player player, LinkageChannel boneChannel, double efficiency) {
    ...
    int lastCombatTick =
        Math.max(player.getLastHurtByMobTimestamp(), player.getLastHurtMobTimestamp());
    int idleTicks = Math.max(0, player.tickCount - lastCombatTick);

    double regenBonus = BASE_REGEN_BONUS;
    boolean boosted = false;
    if (idleTicks >= OUT_OF_COMBAT_THRESHOLD_TICKS
        && boneChannel != null
        && boneChannel.get() >= COST_BONE_GROWTH) {
  ...
  private static void applyMobPassiveHealing(
      LivingEntity entity, LinkageChannel boneChannel, double efficiency) {
  ...
  private static void processOrganRestoration(
      Player player, ChestCavityInstance cc, ItemStack organ, LinkageChannel boneChannel) {
    ...
    MultiCooldown.EntryInt selectionCooldown =
        cooldown.entryInt(SELECTION_COOLDOWN_KEY).withDefault(0);
    MultiCooldown.EntryInt workTimer = cooldown.entryInt(WORK_TIMER_KEY).withDefault(0);
    ...
    if (consumeRestorationResources(player, boneChannel)) {
      ...
      int newProgress =
          Math.min(RESTORATION_REQUIRED_PROGRESS, progress + RESTORATION_PROGRESS_PER_STEP);
      if (newProgress != progress) {
        OrganStateOps.setInt(
            ...
            PROGRESS_KEY,
            newProgress,
            value -> Math.max(0, Math.min(value, RESTORATION_REQUIRED_PROGRESS)),
            0);
      }
  ...
  private static boolean selectMissingOrgan(
    ...
    OrganStateOps.setInt(
        state, cc, organ, TARGET_SLOT_KEY, selected.slot(), value -> Math.max(-1, value), -1);
  ...
  private static void setTargetItemId(ChestCavityInstance cc, ItemStack organ, String value) {
    ...
    NBTWriter.updateCustomData(
        ...
        tag -> {
          CompoundTag state =
              tag.contains(STATE_KEY, Tag.TAG_COMPOUND)
  ...
  private static void playMutationCue(Player player) {
    Level level = player.level();
    level.playSound(
        null, player.blockPosition(), SoundEvents.ENDERMITE_STEP, SoundSource.PLAYERS, 0.8f, 0.8f);
    if (level instanceof ServerLevel serverLevel) {
  ...
  private static void playBiteSuccess(Player player, LivingEntity target) {
    ...
    level.playSound(
        ...
        0.9f);
    level.playSound(
        null, target.blockPosition(), SoundEvents.SKELETON_HURT, SoundSource.PLAYERS, 0.6f, 1.2f);
    if (level instanceof ServerLevel server) {
      spawnBloodSpray(server, target);
    }
  }
  ...
  private static void playBiteFailure(Player player) {
    Level level = player.level();
    level.playSound(
        null, player.blockPosition(), SoundEvents.SKELETON_STEP, SoundSource.PLAYERS, 0.5f, 1.6f);
  }
  ...
  private static void spawnParticlesAround(
      ServerLevel server, LivingEntity entity, ParticleOptions particle, int count, double radius) {
  ...
  private static void applyBiteHealing(Player player, float amount) {
    ...
    if (overflow > 0.0f) {
      ...
      player.addEffect(
          new MobEffectInstance(MobEffects.ABSORPTION, 20 * 180, amplifier, true, true, true));
    }
  }
