...
/**
 * Contains the logic for calculating the parameters of the Dual Strike skill.
 */
public final class DualStrikeLogic {

    private static final List<CooldownTier> FLOW_EXPERIENCE_COOLDOWN_TIERS =
            List.of(new CooldownTier(10001.0D, 20));

    private DualStrikeLogic() {}

    /**
     * Computes the parameters for the Dual Strike skill based on the player's stats.
     *
     ...
     */
    public static DualStrikeParameters computeParameters(double changeDaoMarks, double changeFlowExp) {
        double damageFactor = Math.min(1.0, DualStrikeTuning.DAMAGE_FACTOR * (1.0 + changeDaoMarks));
        int cooldownTicks = computeCooldownTicks(changeFlowExp);
        return new DualStrikeParameters(damageFactor, cooldownTicks);
    }

    private static int computeCooldownTicks(double totalExperience) {
        int result = (int) DualStrikeTuning.COOLDOWN_TICKS;
        ...
        for (CooldownTier tier : FLOW_EXPERIENCE_COOLDOWN_TIERS) {
            ...
            int candidate = (int) Math.round(
                    DualStrikeTuning.COOLDOWN_TICKS - (DualStrikeTuning.COOLDOWN_TICKS - tier.minCooldownTicks()) * ratio);
