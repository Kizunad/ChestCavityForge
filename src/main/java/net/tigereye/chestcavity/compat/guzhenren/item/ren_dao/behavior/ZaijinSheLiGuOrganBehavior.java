...
/**
 * 紫金舍利蛊（五转）：
 * - 被动：每120秒生成“舍利光域”，扣 5000 基础真元 + 20 魂魄 + 20 精力，为友方赋予 再生V+抗性I 持续10秒，并释放粒子/音效
...
public enum ZaijinSheLiGuOrganBehavior implements OrganSlowTickListener {
    ...
    private static final ResourceLocation ORGAN_ID = ResourceLocation.fromNamespaceAndPath(MOD_ID, "zi_jin_she_li_gu");
    ...
    public void onSlowTick(LivingEntity entity, ChestCavityInstance cc, ItemStack organ) {
        ...
        passiveReady.onReady(server, now, () -> {
    ...
    private static void applyPassiveDomain(ServerLevel server, LivingEntity caster) {
        // 友方：再生V + 抗性I 10s
        List<LivingEntity> allies = server.getEntitiesOfClass(LivingEntity.class,
                caster.getBoundingBox().inflate(DOMAIN_RADIUS),
                t -> t != null && t.isAlive() && t != caster && t.isAlliedTo(caster));
        for (LivingEntity ally : allies) {
            ally.addEffect(new MobEffectInstance(MobEffects.REGENERATION, PASSIVE_BUFF_DURATION_TICKS, REGEN_AMP, false, true, true));
            ally.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, PASSIVE_BUFF_DURATION_TICKS, RESIST_AMP, false, true, true));
        }
    ...
    private static void burnHalfLifeAndZhenyuan(LivingEntity caster) {
        ...
        // 真元：按当前真元的一半
        Optional<GuzhenrenResourceBridge.ResourceHandle> handleOpt = GuzhenrenResourceBridge.open(caster);
    ...
    private static void pulseNirvanaDomain(ServerLevel server, LivingEntity caster) {
        ...
        List<Player> players = server.getEntitiesOfClass(Player.class, area, p -> p != null && p.isAlive() && p.isAlliedTo(caster));
    ...
    private static void playPassiveDomainFx(ServerLevel server, LivingEntity caster) {
        ...
        // 音效
        server.playSound(null, caster.getX(), caster.getY(), caster.getZ(), SoundEvents.BEACON_ACTIVATE, SoundSource.PLAYERS, 0.9f, 1.1f);
        // 自定义“咚咚”提示音
        SoundOps.play(server, caster.position(), ResourceLocation.parse("chestcavity:custom.common.dong_dong"), SoundSource.PLAYERS, 0.8f, 1.0f);
    }
    ...
    private static void playNirvanaFxPulse(ServerLevel server, LivingEntity caster) {
        ...
        DustColorTransitionOptions dust = new DustColorTransitionOptions(new Vector3f(1.0f, 0.95f, 0.71f), new Vector3f(1.0f, 1.0f, 1.0f), 2.0f);
        ...
        server.playSound(null, caster.getX(), caster.getY(), caster.getZ(), SoundEvents.GLOW_ITEM_FRAME_ADD_ITEM, SoundSource.PLAYERS, 0.6f, 1.3f);
    }

    private static void playExpandingRing(ServerLevel server, LivingEntity caster, double rStart, double rEnd, int points) {
        Vec3 c = caster.position();
        double y = caster.getY() + 0.1;
        ...
        server.playSound(null, caster.getX(), caster.getY(), caster.getZ(), SoundEvents.AMBIENT_SOUL_SAND_VALLEY_ADDITIONS, SoundSource.PLAYERS, 0.4f, 1.0f);
    }
    ...
    private static MultiCooldown createCooldown(ChestCavityInstance cc, ItemStack organ) {
        ...
        if (cc != null) b.withSync(cc, organ); else b.withOrgan(organ);
        return b.build();
    }
}
