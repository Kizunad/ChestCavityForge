...
public class SingleSwordProjectile extends Entity {
  ...
  private static final EntityDataAccessor<Integer> COLOR =
      SynchedEntityData.defineId(SingleSwordProjectile.class, EntityDataSerializers.INT);
  private static final EntityDataAccessor<ItemStack> DISPLAY_ITEM =
      SynchedEntityData.defineId(SingleSwordProjectile.class, EntityDataSerializers.ITEM_STACK);
    ...
    if (this.level().isClientSide) {
      ...
      if (this.level() instanceof ServerLevel server) {
        server.sendParticles(
            ParticleTypes.SMOKE, this.getX(), this.getY(), this.getZ(), 6, 0.15, 0.1, 0.15, 0.01);
      }
      this.discard();
    }
  }
  ...
  public static SingleSwordProjectile spawn(
      Level level,
      LivingEntity owner,
      Vec3 origin,
      Vec3 target,
      ...
      ItemStack display) {
    SingleSwordProjectile projectile =
        new SingleSwordProjectile(level, owner, origin, target, 0x80222233, display);
  ...
  public static SingleSwordProjectile spawn(
      Level level, LivingEntity owner, Vec3 origin, Vec3 target, PlayerSkinUtil.SkinSnapshot tint) {
