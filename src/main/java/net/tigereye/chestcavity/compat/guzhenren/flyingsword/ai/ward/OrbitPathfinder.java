 ...
 * 环绕运动寻路器
 *
 * <p>实现护幕飞剑的环绕轨迹算法,让飞剑以优雅的圆周运动环绕主人。
 *
 * <h3>算法特性</h3>
 *
 * <ul>
 *   <li><b>切向速度</b>: 飞剑沿环绕圆周的切线方向移动
 *   <li><b>径向调整</b>: 同时向目标槽位径向靠近
 *   <li><b>速度调节</b>: 距离槽位越远,环绕速度越快
 *   <li><b>平滑减速</b>: 接近槽位时逐渐减速并停稳
 * </ul>
 *
 * <h3>运动公式</h3>
 *
...
public final class OrbitPathfinder {
   ...
   * 计算环绕运动的目标速度向量
   *
   * <p>此方法返回的是飞剑应该设置的速度向量(m/tick),已考虑:
   *
   * <ul>
   *   <li>环绕切向运动
   *   <li>径向位置修正
  ...
  public static Vec3 computeOrbitVelocity(PathfindingContext ctx) {
    ...
    // === 1. 切向速度分量 ===
    Vec3 tangentialVel =
        computeTangentialVelocity(relativePos, currentRadius, distToSlot, ctx.vMax());

    // === 2. 径向速度分量 ===
    Vec3 radialVel =
        computeRadialVelocity(
            relativePos, relativeTarget, currentRadius, targetRadius, distToSlot, ctx.vMax());

    // === 3. 垂直速度分量(Y轴调整) ===
    Vec3 verticalVel = computeVerticalVelocity(swordPos, targetPos, distToSlot, ctx.vMax());
   ...
   * 计算切向速度(绕主人环绕的圆周运动分量)
   *
   * <p>切向速度垂直于径向,实现圆周运动效果。 速度大小根据距离槽位的远近动态调整。
   *
  ...
  private static Vec3 computeTangentialVelocity(
      Vec3 relativePos, double currentRadius, double distToSlot, double vMax) {
  ...
  private static Vec3 computeVerticalVelocity(
      Vec3 currentPos, Vec3 targetPos, double distToSlot, double vMax) {
