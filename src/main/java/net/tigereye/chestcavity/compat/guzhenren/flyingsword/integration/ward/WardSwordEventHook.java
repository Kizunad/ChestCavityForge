 ...
 * <p>职责：
 * <ul>
 *   <li>跳过护幕飞剑的维持消耗（护幕飞剑由剑幕蛊器官提供能量维持）</li>
 *   <li>护幕飞剑的资源消耗由剑幕蛊器官的 onSlowTick 统一管理</li>
 * </ul>
 *
 * <p>设计原则：
 * <ul>
 *   <li>护幕飞剑通过 {@link FlyingSwordEntity#isWardSword()} 标识</li>
 *   <li>所有护幕飞剑跳过 UpkeepSystem 的真元消耗</li>
 *   <li>资源消耗改为由剑幕蛊器官统一管理（精力 + 真元）</li>
 * </ul>
 *
...
public final class WardSwordEventHook implements FlyingSwordEventHook {
    ...
    public void onUpkeepCheck(UpkeepCheckContext ctx) {
        ...
        if (ctx.sword.isWardSword()) {
            ...
            LOGGER.debug("[WardSword] Skipped upkeep consumption for ward sword (ID: {})",
                ctx.sword.getId());
        }
    }
}
