...
public final class GuzhenrenNetworkBridge {
    ...
    public static void bootstrap() {
        LOGGER.debug("[compat/guzhenren][network] bootstrap (dist={}, installed={}, bridgeAvailable={})",
                FMLEnvironment.dist, installed, GuzhenrenResourceBridge.isAvailable());
        if (installed) {
            LOGGER.debug("[compat/guzhenren][network] bootstrap aborted: already installed");
            return;
        }
        if (!FMLEnvironment.dist.isClient()) {
            LOGGER.debug("[compat/guzhenren][network] bootstrap aborted: non-client dist");
            return;
        }
        if (!GuzhenrenResourceBridge.isAvailable()) {
            LOGGER.debug("[compat/guzhenren][network] bootstrap aborted: Guzhenren bridge unavailable");
            return;
        }
        synchronized (GuzhenrenNetworkBridge.class) {
            if (installed) {
                LOGGER.debug("[compat/guzhenren][network] bootstrap abort inside sync: installed flag set");
                return;
            }
            try {
                installInterceptor();
                installed = true;
                LOGGER.info("[compat/guzhenren][network] hooked player variable payload");
            } catch (Throwable throwable) {
                LOGGER.warn("[compat/guzhenren][network] Failed to hook Guzhenren player variable payload", throwable);
            }
        }
    }
    ...
    public static boolean registerListener(GuzhenrenPayloadListener listener) {
        if (listener == null) {
            LOGGER.warn("[compat/guzhenren][network] Attempted to register null listener");
            return false;
        }
        boolean added = LISTENERS.add(listener);
        LOGGER.debug("[compat/guzhenren][network] listener {} registered? {} (total={})",
                listener.getClass().getName(), added, LISTENERS.size());
        return added;
    }
    ...
    public static boolean unregisterListener(GuzhenrenPayloadListener listener) {
        ...
        LOGGER.debug("[compat/guzhenren][network] listener {} removed? {} (total={})",
                listener.getClass().getName(), removed, LISTENERS.size());
        return removed;
    }
    ...
    private static void installInterceptor() throws Exception {
        ...
        Class<?> payloadClass = Class.forName("net.guzhenren.network.GuzhenrenModVariables$PlayerVariablesSyncMessage");
        ...
        if (existing == null) {
            LOGGER.warn("[compat/guzhenren][network] PlayerVariablesSyncMessage not present in Guzhenren message map");
            return;
        }
        ...
        @SuppressWarnings("unchecked")
        IPayloadHandler<CustomPacketPayload> originalHandler = (IPayloadHandler<CustomPacketPayload>) handlerAccessor.invoke(existing);
        if (originalHandler == null) {
            LOGGER.warn("[compat/guzhenren][network] PlayerVariablesSyncMessage handler unavailable");
            return;
        }
        ...
        IPayloadHandler<CustomPacketPayload> wrapper = new IPayloadHandler<>() {
            @Override
            public void handle(CustomPacketPayload payload, IPayloadContext context) {
                originalHandler.handle(payload, context);
                if (context.flow() != PacketFlow.CLIENTBOUND) {
                    LOGGER.trace("[compat/guzhenren][network] payload ignored due to flow {}", context.flow());
                    return;
                }
                Player player = context.player();
                if (player == null || !player.level().isClientSide()) {
                    LOGGER.trace("[compat/guzhenren][network] payload ignored (player null or not clientside)");
                    return;
                }
        ...
        LOGGER.debug("[compat/guzhenren][network] installed wrapper handler {} for payload {}",
                wrapper.getClass().getName(), type);
    }
    ...
    private static void dispatch(Player player) {
        GuzhenrenResourceBridge.open(player).ifPresent(handle -> {
            ...
            if (snapshot.isEmpty()) {
                LOGGER.trace("[compat/guzhenren][network] dispatch aborted: empty snapshot for {}", player.getScoreboardName());
                return;
            }
            LOGGER.trace("[compat/guzhenren][network] dispatching snapshot with {} keys for {}", snapshot.size(), player.getScoreboardName());
            ...
            for (GuzhenrenPayloadListener listener : LISTENERS) {
                try {
                    ...
                    LOGGER.warn("[compat/guzhenren][network] payload listener threw", throwable);
                }
            }
        });
    }
