 ...
 * 障碍物避让系统
 * <p>
 * 为护幕飞剑提供基本的避障能力,防止卡在方块中。
 *
 * <h3>避障策略</h3>
 * <ul>
 *   <li><b>射线检测</b>: 检测前方路径是否有方块阻挡</li>
 *   <li><b>简单绕路</b>: 检测到障碍时,尝试向上、向旁边偏移</li>
 *   <li><b>最小干预</b>: 只在必要时调整路径,保持原有运动逻辑</li>
 * </ul>
 *
 * <h3>使用方式</h3>
 * <pre>
...
public final class ObstacleAvoidance {
     ...
     * 策略:
     * <ol>
     *   <li>尝试向上绕行(飞剑可以飞,向上是最简单的)</li>
     *   <li>如果向上也有障碍,尝试侧向绕行</li>
     *   <li>如果都不行,减速并保持原方向</li>
     * </ol>
     *
     * @param ctx 寻路上下文
    ...
    public static Vec3 computeEscapeVelocity(PathfindingContext ctx) {
        ...
        // 卡住了,尝试向上逃逸
        LOGGER.warn("Sword {} is stuck inside block at {}, escaping upward",
                ctx.sword().getId(), currentPos);
