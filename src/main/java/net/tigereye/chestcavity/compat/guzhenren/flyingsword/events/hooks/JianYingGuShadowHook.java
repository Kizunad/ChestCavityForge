...
/**
 * 飞剑命中时，有概率触发“剑影蛊分身”（飞剑版）： - 触发条件：飞剑的胸腔中包含 guzhenren:jian_ying_gu -
 * 触发概率：JianYingTuning.PASSIVE_TRIGGER_CHANCE - 分身伤害：以本次飞剑命中伤害为基础，叠加 Owner 的剑道道痕与流派经验（不扣资源） -
 * 分身存在时间：同样按 Owner 的道痕与流派经验增加
 */
public final class JianYingGuShadowHook implements FlyingSwordEventHook {

  private static final ResourceLocation ORGAN_ID = ResourceLocation.parse("guzhenren:jian_ying_gu");
