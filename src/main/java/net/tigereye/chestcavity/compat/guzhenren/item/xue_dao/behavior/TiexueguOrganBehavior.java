...
public final class TiexueguOrganBehavior extends AbstractGuzhenrenOrganBehavior
  ...
  private static final ResourceLocation ORGAN_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "tiexuegu");
  ...
  private static final int TRIGGER_INTERVAL_SLOW_TICKS =
      BehaviorConfigAccess.getInt(TiexueguOrganBehavior.class, "TRIGGER_INTERVAL_SLOW_TICKS", 60);
  private static final float HEALTH_DRAIN_PER_STACK =
      BehaviorConfigAccess.getFloat(TiexueguOrganBehavior.class, "HEALTH_DRAIN_PER_STACK", 5.0f);
  ...
  private static final float MINIMUM_HEALTH_RESERVE =
      BehaviorConfigAccess.getFloat(TiexueguOrganBehavior.class, "MINIMUM_HEALTH_RESERVE", 1.0f);

  private static final int DAMAGE_PARTICLE_COUNT =
      BehaviorConfigAccess.getInt(TiexueguOrganBehavior.class, "DAMAGE_PARTICLE_COUNT", 24);
  private static final int BLOOD_PARTICLE_COUNT =
      BehaviorConfigAccess.getInt(TiexueguOrganBehavior.class, "BLOOD_PARTICLE_COUNT", 32);
  private static final DustParticleOptions BLOOD_DUST =
      new DustParticleOptions(new Vector3f(0.8f, 0.05f, 0.05f), 1.0f);

  private static final Component DRAIN_MESSAGE = Component.literal("铁血蛊吸食了你的鲜血，化为力量流入体内。");
  private static final Component STARVED_MESSAGE = Component.literal("你的血量不足，铁血蛊暂时沉寂。");

  /**
   * Invoked when the organ is evaluated inside a chest cavity to ensure bookkeeping is initialised
   * once.
   */
  public void onEquip(
    ...
    if (state.getInt(TIMER_KEY, 0) <= 0) {
      var change =
          OrganStateOps.setInt(
              state, cc, organ, TIMER_KEY, initialTimer(cc), value -> Math.max(0, value), 0);
  ...
  private void scheduleEffectIfNeeded(
  ...
  public void onRemoved(LivingEntity entity, ChestCavityInstance cc, ItemStack organ) {
    ...
    var effectChange =
        OrganStateOps.setDouble(
            state, cc, organ, EFFECT_KEY, 0.0, value -> Math.max(0.0, value), 0.0);
    var timerChange =
        OrganStateOps.setInt(state, cc, organ, TIMER_KEY, 0, value -> Math.max(0, value), 0);
  ...
  private void triggerEffect(
    ...
    var effectChange =
        OrganStateOps.setDouble(
            state, cc, organ, EFFECT_KEY, newEffect, value -> Math.max(0.0, value), 0.0);
    logStateChange(LOGGER, LOG_PREFIX, organ, EFFECT_KEY, effectChange);
    ...
    net.tigereye.chestcavity.util.reaction.tag.ReactionTagOps.add(
        entity, net.tigereye.chestcavity.util.reaction.tag.ReactionTagKeys.BLOOD_RITUAL, 60);
  }
  ...
  private static void spawnTriggerParticles(ServerLevel server, LivingEntity entity) {
    ...
    server.sendParticles(
        ...
        0.25);
    server.sendParticles(
        BLOOD_DUST, center.x, center.y, center.z, BLOOD_PARTICLE_COUNT, 0.4, 0.55, 0.4, 0.0);
  }
  ...
  private void handleFailedDrain(
      LivingEntity player,
      ChestCavityInstance cc,
      ItemStack organ,
      OrganState state,
    ...
    var effectChange =
        OrganStateOps.setDouble(
            state, cc, organ, EFFECT_KEY, targetEffect, value -> Math.max(0.0, value), 0.0);
    logStateChange(LOGGER, LOG_PREFIX, organ, EFFECT_KEY, effectChange);
    ...
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug(
          "{} Drain failed for {} (stackCount={}, available={}, required={})",
          LOG_PREFIX,
