...
/**
 * 白银舍利蛊（第三转）： - 被动（致命伤触发，20分钟冷却）：吸收100HP（5分钟），恢复50%生命（上限500），力量V 30s，消耗8年寿元 -
 * 主动（ATTACKABILITY，60s冷却）：金身化舍：10s 抗性II；期间若受致命伤，保留1血并获得1s无敌（每次激活仅触发一次）
 */
public enum BaiYinSheLiGuOrganBehavior
  ...
  private static final ResourceLocation ABSORPTION_MODIFIER_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "modifiers/bai_yin_she_li_absorption");
  ...
  public static void activateAbility(LivingEntity entity, ChestCavityInstance cc) {
    ...
    entity.addEffect(
        new MobEffectInstance(
            MobEffects.DAMAGE_RESISTANCE, ACTIVE_DURATION_TICKS, 1, false, true, true));
  ...
  public float onIncomingDamage(
    ...
    if (now >= passiveReadyAt) {
      ...
      Optional<GuzhenrenResourceBridge.ResourceHandle> handleOpt =
          GuzhenrenResourceBridge.open(victim);
  ...
  private static void applyPassiveShield(LivingEntity victim) {
    ...
    if (victim.level() instanceof ServerLevel server) {
      TickOps.schedule(
          server,
          () -> AbsorptionHelper.clearAbsorptionCapacity(victim, ABSORPTION_MODIFIER_ID),
  ...
  private static void applyPassiveHealAndBuff(LivingEntity victim) {
    ...
    victim.addEffect(
        new MobEffectInstance(
            MobEffects.DAMAGE_BOOST, STRENGTH_DURATION_TICKS, STRENGTH_AMP, false, true, true));
  }
