 ...
 *
 * <p>The behaviour is intentionally incremental – it focuses on delivering the defensive loop
 * (absorption reservoir + hunger upkeep) and the 玉皮蛊反伤联动 so the surrounding infrastructure can
 * build on it later. Active ability support and world interactions (placing temporary walls) will
 * be layered on top in a follow-up patch once the groundwork is proven stable.
 */
public final class TuQiangGuOrganBehavior extends AbstractGuzhenrenOrganBehavior
  ...
  private static final double BARRIER_DAMAGE_RATIO =
      0.10; // 10% local hardening while barrier active
  private static final double BARRIER_MAX_ABSORPTION = 10.0; // 最大吸收10点

  private static final int ABILITY_RANGE =
      BehaviorConfigAccess.getInt(TuQiangGuOrganBehavior.class, "ABILITY_RANGE", 10);
  private static final int PRISON_RADIUS =
      BehaviorConfigAccess.getInt(TuQiangGuOrganBehavior.class, "PRISON_RADIUS", 4);
  private static final int PRISON_DURATION_TICKS =
      BehaviorConfigAccess.getInt(
          TuQiangGuOrganBehavior.class, "PRISON_DURATION_TICKS", 20 * 60); // 60 seconds

  private static final ResourceLocation ORGAN_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "tu_qiang_gu");
  private static final ResourceLocation YU_PI_GU_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "yu_pi_gu");
  private static final ResourceLocation TU_LAO_BLOCK_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "tu_lao");
  private static final ResourceLocation JADE_LAO_BLOCK_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "yu_lao");
  ...
  private static final BlockState EARTH_PRISON_BLOCK =
      resolvePrisonBlock(TU_LAO_BLOCK_ID, Blocks.PACKED_MUD.defaultBlockState());
  private static final BlockState JADE_PRISON_BLOCK =
      resolvePrisonBlock(JADE_LAO_BLOCK_ID, Blocks.EMERALD_BLOCK.defaultBlockState());

  private static final double RANGE_SQUARED = ABILITY_RANGE * ABILITY_RANGE;

  private static final ConcurrentHashMap<UUID, ActivePrison> ACTIVE_PRISONS =
  ...
  public void onSlowTick(LivingEntity entity, ChestCavityInstance cc, ItemStack organ) {
    ...
    double barrierRemaining =
        Mth.clamp(state.getDouble(BARRIER_REMAINING_KEY, 0.0), 0.0, BARRIER_MAX_ABSORPTION);
  ...
  public float onIncomingDamage(
    ...
    if (hasJade && barrierTicks > 0 && barrierRemaining > 0.0) {
      ...
      if (reduction > 0.0) {
        ...
        OrganState.Change<Double> barrierChange =
            state.setDouble(BARRIER_REMAINING_KEY, Math.max(0.0, barrierRemaining - reduction));
  ...
  public void activateBarrier(ChestCavityInstance cc, ItemStack organ, int durationTicks) {
    ...
    OrganState.Change<Double> shieldChange =
        state.setDouble(BARRIER_REMAINING_KEY, BARRIER_MAX_ABSORPTION);
  ...
  private void reflectDamage(
      LivingEntity victim, DamageSource source, float damage, double multiplier) {
  ...
  private static void clearActivePrison(Player owner, ServerLevel level) {
    ...
    LOGGER.info(
        "[compat/guzhenren][tu_qiang_gu][cleanup] skipped removal owner={} version={}",
        ownerId,
        existing.version());
  }

  private static void registerActivePrison(
      Player owner, ServerLevel level, PrisonPlacement placement) {
    ...
    LOGGER.info(
        "[compat/guzhenren][tu_qiang_gu][scheduler] cleanup disabled owner={} version={}",
        owner.getUUID(),
        version);
  }

  private static boolean attemptEmeraldUpgrade(
      LivingEntity user, ChestCavityInstance cc, ItemStack organ, OrganState state) {
  ...
  private void updateHeShiBiChannel(ChestCavityInstance cc, ItemStack organ, int emeraldBlocks) {
    ...
    LedgerOps.set(
        cc, organ, HE_SHI_BI_LINK, Math.max(1, organ.getCount()), value, NON_NEGATIVE, true);
  }
  ...
  private static PrisonPlacement buildPrisonShell(
      ServerLevel level, LivingEntity target, int radius, BlockState blockState) {
  ...
  private static boolean tryPlacePrisonBlock(
    ...
    LOGGER.info(
        "[compat/guzhenren][tu_qiang_gu][placement] shell place owner={} pos={} replaced={} block={}",
  ...
  private static boolean tryClearInterior(
      ServerLevel level, BlockPos pos, PlacementAccumulator stats) {
    stats.interiorAttempts++;
    if (!level.isInWorldBounds(pos)) {
      stats.interiorOutOfBounds++;
      return false;
  ...
  private static void schedulePrisonCleanup(
      ServerLevel level, UUID ownerId, long version, PrisonPlacement placement, int delayTicks) {
    if (placement == null || placement.blocks().isEmpty()) {
      return;
    }
    ...
    LOGGER.info(
        "[compat/guzhenren][tu_qiang_gu][scheduler] owner={} version={} delay={} current_tick={} target_tick={}",
        ownerId,
        version,
        effectiveDelay,
        currentTick,
  ...
  private static void cleanupPrison(MinecraftServer server, UUID ownerId, long version) {
    ...
    if (level != null) {
      LOGGER.info(
          "[compat/guzhenren][tu_qiang_gu][cleanup] owner={} version={} releasing={} coords={}",
          ownerId,
          version,
  ...
  private static void releasePrison(
      ServerLevel level, UUID ownerId, long version, PrisonPlacement placement) {
    if (placement == null || placement.blocks().isEmpty()) {
      return;
    }
    ...
    for (BlockPos pos : placement.blocks()) {
      ...
      LOGGER.info(
          "[compat/guzhenren][tu_qiang_gu][cleanup] remove owner={} version={} pos={}",
          ownerId,
          version,
          pos);
    }
  ...
  private static BlockState resolvePrisonBlock(ResourceLocation id, BlockState fallback) {
    ...
    return BuiltInRegistries.BLOCK.getOptional(id).map(Block::defaultBlockState).orElse(fallback);
  }
  ...
  private record ActivePrison(
      ResourceKey<Level> levelKey, PrisonPlacement placement, long version) {}
}
