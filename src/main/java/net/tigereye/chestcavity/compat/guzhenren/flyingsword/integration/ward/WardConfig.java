 ...
 * 护幕系统的全局常量与默认配置
 *
 * <p>此类包含护幕系统的所有默认数值参数。 这些值用作 {@link WardTuning} 接口的默认实现基准。
 *
 * <h3>设计原则</h3>
 *
 * <ul>
 *   <li>所有常量应为 {@code public static final}
 *   <li>数值应经过平衡性测试
 *   <li>支持通过配置文件覆盖（见 E 阶段）
 * </ul>
 */
public final class WardConfig {
   ...
   * 基础最大速度（米/秒）
   *
   * <p>公式：vMax = SPEED_BASE + 0.02*道痕 + 0.001*经验
   */
   ...
   * 环绕状态下的速度系数
   *
   * <p>环绕速度 = vMax * ORBIT_SPEED_FACTOR 该值设为较低，以实现"缓慢环绕"的效果。
   */
   ...
   * 成功拦截的基础耐久消耗
   *
   * <p>实际消耗会根据经验衰减：costBlock = round(8 * (1 - R))
   */
   ...
   * 成功反击的基础耐久消耗
   *
   * <p>实际消耗会根据经验衰减：costCounter = round(10 * (1 - R))
   */
   ...
   * 失败尝试的基础耐久消耗
   *
   * <p>实际消耗会根据经验衰减：costFail = round(2 * (1 - 0.5*R))
   */
   ...
   * 经验衰减基准值
   *
   * <p>用于计算衰减系数：R = exp / (exp + EXP_DECAY_BASE)
   */
   ...
   * 初始耐久基础值
   *
   * <p>公式：Dur0 = INITIAL_DUR_BASE + INITIAL_DUR_TRAIL*道痕 + INITIAL_DUR_EXP*经验
   */
   ...
   * 环绕半径基础值（米）
   *
   * <p>公式：r = ORBIT_RADIUS_BASE + ORBIT_RADIUS_PER_SWORD * N
   */
   ...
   * 基础反应延迟（秒）
   *
   * <p>公式：reaction = clamp(REACTION_BASE - REACTION_EXP_COEF*经验, REACTION_MIN, REACTION_MAX)
   */
   ...
   * 基础反击伤害（生命值）
   *
   * <p>公式：D = COUNTER_DAMAGE_BASE + 0.05*道痕 + 0.01*经验
   */
   ...
   * 穿甲因子（无量纲）
   *
   * <p>拦截成功时，玩家受到的伤害比例。 默认值 0.3 表示"穿甲保留 30%"，即拦截后玩家只受到原伤害的 30%。 设为 0.0 表示完全免疫，设为 1.0 表示不减免。
   */
   ...
   *
   * <p>当 原伤害 / 玩家最大生命值 >= 此值时，分配1把飞剑拦截 默认值 0.10 表示伤害达到玩家最大生命值的 10% 时触发
   */
   ...
   *
   * <p>当 原伤害 / 玩家最大生命值 >= 此值时，分配2把飞剑拦截 默认值 0.20 表示伤害达到玩家最大生命值的 20% 时触发
   */
   ...
   *
   * <p>当 原伤害 / 玩家最大生命值 >= 此值时，分配3把飞剑拦截 默认值 0.30 表示伤害达到玩家最大生命值的 30% 时触发
   */
   ...
   *
   * <p>当 原伤害 / 玩家最大生命值 >= 此值时，分配4把飞剑（全部）拦截 默认值 0.40 表示伤害达到玩家最大生命值的 40% 时触发
   */
