...
public final class RiderControlSystem {
  ...
  public static void apply(FlyingSwordEntity sword, Player player) {
    ...
    double inputMag = Math.min(1.0, Math.sqrt((double) forward * forward + (double) strafe * strafe));

    // 死区处理：进入慢停
    if (inputMag < net.tigereye.chestcavity.compat.guzhenren.flyingsword.tuning
        .FlyingSwordControlTuning.RIDER_INPUT_DEADZONE) {
      desired = Vec3.ZERO;
      inputMag = 0.0;
    }

    // 计算增幅倍率：域缩放 × 道痕加成 × 基础倍率
    double domainScale = net.tigereye.chestcavity.compat.guzhenren.flyingsword.integration.domain
        .SwordSpeedModifiers.computeDomainSpeedScale(sword);
    double daoHenMult = computeDaohenSpeedMult(sword);
    double baseMult = net.tigereye.chestcavity.compat.guzhenren.flyingsword.tuning
        .FlyingSwordControlTuning.RIDER_BASE_SPEED_MULT;

    double effectiveMax = sword.getSwordAttributes().speedMax * domainScale * daoHenMult * baseMult;

    // 目标速度
    Vec3 targetVel = desired.lengthSqr() > 1.0e-6
        ? desired.normalize().scale(effectiveMax * inputMag)
        : Vec3.ZERO;

    // 平滑插值（手感）
    double a = net.tigereye.chestcavity.compat.guzhenren.flyingsword.tuning
        .FlyingSwordControlTuning.RIDER_VELOCITY_SMOOTHING;
  ...
  /**
   * 客户端侧本地控制（Boat式）：
   * 仅当本地玩家为驾驶者时调用，由客户端预测速度并通过“MoveVehicle”包同步给服务端。
   */
  ...
  private static double computeDaohenSpeedMult(FlyingSwordEntity sword) {
    try {
      ...
      java.util.Optional<net.tigereye.chestcavity.guzhenren.resource.GuzhenrenResourceBridge.ResourceHandle>
          handleOpt = net.tigereye.chestcavity.compat.guzhenren.util.behavior.ResourceOps.openHandle(owner);
      double daohen = handleOpt.map(h -> h.read("daohen_jiandao").orElse(0.0)).orElse(0.0);
      if (!(daohen > 0.0)) return 1.0;
      double coef = net.tigereye.chestcavity.compat.guzhenren.flyingsword.tuning
          .FlyingSwordControlTuning.RIDER_DAOHEN_COEF;
      double maxExtra = net.tigereye.chestcavity.compat.guzhenren.flyingsword.tuning
          .FlyingSwordControlTuning.RIDER_DAOHEN_MAX_EXTRA;
