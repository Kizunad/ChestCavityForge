 ...
 *
 * <p>该类将小魂蛊（以及其他魂道器官）与胸腔系统进行行为绑定。
 *
 * <p>- 在每秒（SlowTick）时执行资源维护与状态同步； - 在命中（OnHit）时触发“魂焰”持续伤害效果与魂魄消耗； -
 * 在器官被移除（Removal）时保留必要的绑定与活跃态； - 在装备时初始化绑定信息并确保联动通道可用。
 *
 * <p>如需扩展更多魂道器官，按同样方式向 {@link #SPECS} 中追加条目即可。
 */
public final class HunDaoOrganRegistry {

  private static final String MOD_ID = "guzhenren";
  public static final ResourceLocation XIAO_HUN_GU_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "xiao_hun_gu");
  public static final ResourceLocation DA_HUN_GU_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "dahungu");
  public static final ResourceLocation GUI_QI_GU_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "guiqigu");
  public static final ResourceLocation TI_PO_GU_ID =
  ...
  private static final List<OrganIntegrationSpec> SPECS =
      List.of(
          ...
          OrganIntegrationSpec.builder(GUI_QI_GU_ID)
              ...
              .build(),
          OrganIntegrationSpec.builder(TI_PO_GU_ID)
              .addSlowTickListener(TiPoGuOrganBehavior.INSTANCE)
              .addOnHitListener(TiPoGuOrganBehavior.INSTANCE)
              .addRemovalListener(TiPoGuOrganBehavior.INSTANCE)
              .ensureAttached(TiPoGuOrganBehavior.INSTANCE::ensureAttached)
              .onEquip(TiPoGuOrganBehavior.INSTANCE::onEquip)
              .build());
