  ...
  public void render(
    ...
    if (prof != null) {
      look =
        ...
        && def.alignMode == SwordModelOverrideDef.AlignMode.TARGET
        && entity.getTargetEntity() != null
        && entity.getTargetEntity().isAlive()) {
      var tgt = entity.getTargetEntity();
      look =
          tgt.position().add(0, tgt.getBbHeight() * 0.5, 0).subtract(entity.position()).normalize();
    } else {
    ...
    float preRoll =
        prof != null
            ? prof.preRollDeg
            : (def != null ? def.preRollDeg : FlyingSwordModelTuning.BLADE_ROLL_DEGREES);
    float yawOffset =
        (prof != null
            ? prof.yawOffsetDeg - 90.0f
            : -90.0f + (def != null ? def.yawOffsetDeg : 0.0f));
    float pitchOffset =
        (prof != null ? prof.pitchOffsetDeg : (def != null ? def.pitchOffsetDeg : 0.0f));
    ...
    if (FlyingSwordModelTuning.USE_BASIS_ORIENTATION) {
      ...
      orientationMode =
          prof != null
              ? prof.orientationMode
              : (def != null ? def.orientationMode : OrientationMode.BASIS);
    } else {
    ...
    Quaternionf orientation =
        OrientationOps.orientationFromForwardUp(
    ...
    if (useGecko) {
      ...
      for (var tex : layers) {
        ...
        && prof.renderer == SwordVisualProfile.RendererKind.GECKO
      ...
      obj.updateTick(entity.tickCount + partialTicks);
      java.util.List<net.minecraft.resources.ResourceLocation> layers =
          (prof.textures != null && !prof.textures.isEmpty()) ? prof.textures : java.util.List.of();
      if (layers.isEmpty()) {
        // 无纹理则跳过（避免渲染错误）
      } else {
        for (var tex : layers) {
      ...
      if (def != null && def.displayItem != null) {
        ...
        if (item != null) {
          ...
          try {
            ...
            SwordVisualProfile.GlintMode glintMode =
                prof != null ? prof.glint : SwordVisualProfile.GlintMode.INHERIT;
            switch (glintMode) {
              ...
              default -> {
                if (ench != null && !ench.isEmpty()) {
                  ...
                  Boolean glint =
                      baseDisplay == null
                          ? null
                          : baseDisplay.get(DataComponents.ENCHANTMENT_GLINT_OVERRIDE);
