 ...
 *
 * <p>分组（默认值见常量定义）： - 目标选择：获取/保持半径（防边界抖动） - IDLE/编队：最大速度、到达半径、停止半径、层高、槽位自转速度、中心平滑系数 -
 * ATTACK/中心绕行：绕行距离（直线距离）、高度抬升、中心绕行角速度、槽位自转速度 - 调度/三段式：离队频率、单体最小离队间隔、各阶段到达/停止半径、命中范围、超时阈值 -
...
public final class QingLianSwarmTuning {
  ...
  // —— 目标选择 ——
  /** 获取目标的最大距离（方块）。 调大：更容易进入攻击；调小：更保守。建议与“保持半径”形成正向滞后。 */
