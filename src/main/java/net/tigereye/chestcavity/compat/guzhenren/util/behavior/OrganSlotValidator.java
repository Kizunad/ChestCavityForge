...
/**
 * Lightweight validator that snapshots an organ's slot + item fingerprint and detects changes
 * efficiently. Typical usage:
 *
 * <p>OrganSlotValidator v = OrganSlotValidator.capture(cc, organ); // later on tick/trigger boolean
 * stable = v.validateAndUpdateIfChanged(cc, organ); if (!stable) return false; // skip logic if
 * organ/slot changed
 */
