...
public final class FlyingSwordTUI {
  ...
  private static Component createSelectedInfo(FlyingSwordEntity selected, ServerPlayer player) {
    ...
    ChatFormatting color =
        ratio > 0.6
            ...
            : ratio > 0.3 ? ChatFormatting.YELLOW : ChatFormatting.RED;

    return Component.literal("已选中: ")
        .withStyle(TUITheme.ACCENT)
        ...
        .append(
            Component.literal("[" + selected.getAIMode().getDisplayName() + "]")
                .withStyle(getModeColor(selected.getAIMode().getDisplayName())))
        .append(Component.literal(" | ").withStyle(TUITheme.DIM))
        .append(
            Component.literal(
                    String.format(
                        "%.0f/%.0f",
                        selected.getDurability(), selected.getSwordAttributes().maxDurability))
                .withStyle(color))
        .append(Component.literal(" | ").withStyle(TUITheme.DIM))
        .append(
            Component.literal(String.format("%.1fm", selected.distanceTo(player)))
                .withStyle(TUITheme.LABEL));
  }

  private static Component createSwordListItem(
      FlyingSwordEntity sword, int idx, ServerPlayer player, int currentPage) {
    double ratio = sword.getDurability() / sword.getSwordAttributes().maxDurability;

    MutableComponent line =
        Component.literal(String.format("#%-2d ", idx))
            .withStyle(TUITheme.LABEL)
            .append(Component.literal("Lv." + sword.getSwordLevel()).withStyle(TUITheme.VALUE))
            .append(sp())
            .append(
                Component.literal("[" + sword.getAIMode().getDisplayName() + "]")
                    .withStyle(getModeColor(sword.getAIMode().getDisplayName())))
            .append(sp())
            .append(
                Component.literal(String.format("%.0f%%", ratio * 100))
                    .withStyle(ratio > 0.5 ? ChatFormatting.GREEN : ChatFormatting.YELLOW))
            .append(sp())
            .append(
                Component.literal(String.format("%.0fm", sword.distanceTo(player)))
                    .withStyle(TUITheme.LABEL));

    // 操作按钮（带刷新）
    line.append(Component.literal("  "))
        .append(
            createRefreshButton("选", "/flyingsword select index " + idx, "选中", currentPage, true))
        .append(sp())
        .append(
            createRefreshButton("修", "/flyingsword repair_index " + idx, "修复", currentPage, true))
        .append(sp())
        .append(
            createRefreshButton("回", "/flyingsword recall_index " + idx, "召回", currentPage, true))
        .append(sp())
        .append(
            createRefreshButton(
                "攻", "/flyingsword mode_index " + idx + " hunt", "出击", currentPage, true))
        .append(sp())
        .append(
            createRefreshButton(
                "守", "/flyingsword mode_index " + idx + " guard", "守护", currentPage, true))
        .append(sp())
        .append(
            createRefreshButton(
                "环", "/flyingsword mode_index " + idx + " orbit", "环绕", currentPage, true))
        .append(sp())
        .append(
            createRefreshButton(
                "悬", "/flyingsword mode_index " + idx + " hover", "悬浮", currentPage, true));

    return line;
  }

  private static Component createStorageListItem(
      FlyingSwordStorage.RecalledSword recalled, int idx, ServerPlayer player, int currentPage) {
    ...
    if (recalled.itemWithdrawn) {
      line.append(Component.literal("  "))
          .append(
              createRefreshButton(
                  "放回", "/flyingsword deposit_index " + idx, "放回物品", currentPage, false))
          .append(sp())
          .append(Component.literal("(已取出)").withStyle(TUITheme.WARNING))
          .append(sp())
          .append(
              createRefreshButton(
                  "删除", "/flyingsword delete_storage " + idx, "删除", currentPage, false));
    } else {
      line.append(Component.literal("  "))
          .append(
              createRefreshButton(
                  "召唤", "/flyingsword restore_index " + idx, "召唤", currentPage, false))
          .append(sp())
          .append(
              createRefreshButton(
                  "取出", "/flyingsword withdraw_index " + idx, "取出物品", currentPage, false))
          .append(sp())
          .append(
              createRefreshButton(
                  "删除", "/flyingsword delete_storage " + idx, "删除", currentPage, false));
  ...
  private static Component createPagination(int page, int pages, boolean isActive) {
    ...
    if (page > 1) {
      String cmd =
          isActive
              ? "/flyingsword ui_active " + (page - 1)
              : "/flyingsword ui_storage " + (page - 1);
  ...
  private static MutableComponent createRefreshButton(
      String label, String command, String hover, int page, boolean isActive) {
    // 组合命令：先执行操作，再刷新当前页面
    String refreshCmd =
        isActive ? "/flyingsword ui_active " + page : "/flyingsword ui_storage " + page;
