...
public enum DianLiuguOrganBehavior
  ...
  private static final int MAX_CHARGE =
      BehaviorConfigAccess.getInt(DianLiuguOrganBehavior.class, "MAX_CHARGE", 10);
  ...
  private static MultiCooldown createCooldown(ChestCavityInstance cc, ItemStack organ) {
    MultiCooldown.Builder builder =
        MultiCooldown.builder(organ, STATE_ROOT)
  ...
  private static void applyDebuff(LivingEntity target, double efficiency) {
    ...
    target.addEffect(
        new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, duration, 10, false, true, true));
  ...
  private static void playActivationEffects(Level level, LivingEntity target) {
    ...
    level.playSound(
        null, x, y, z, SoundEvents.REDSTONE_TORCH_BURNOUT, SoundSource.PLAYERS, 0.8f, pitch);
  ...
  private static void arcAdditionalTargets(
      LivingEntity attacker, LivingEntity primary, DamageSource source, float aoeDamage) {
    ...
    DamageSource chainSource =
        source != null ? source : attacker.damageSources().indirectMagic(attacker, attacker);
