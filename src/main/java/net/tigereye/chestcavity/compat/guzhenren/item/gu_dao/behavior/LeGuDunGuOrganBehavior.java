...
public final class LeGuDunGuOrganBehavior extends AbstractGuzhenrenOrganBehavior
  ...
  private static final int MAX_EFFECTIVE_STACKS =
      BehaviorConfigAccess.getInt(LeGuDunGuOrganBehavior.class, "MAX_EFFECTIVE_STACKS", 1);
  private static final int MAX_BU_QU =
      BehaviorConfigAccess.getInt(LeGuDunGuOrganBehavior.class, "MAX_BU_QU", 10);

  private static final double BASE_ZHENYUAN_COST_PER_SECOND = 200.0;
  private static final int INVULN_DURATION_TICKS =
      BehaviorConfigAccess.getInt(LeGuDunGuOrganBehavior.class, "INVULN_DURATION_TICKS", 40);
  ...
  public void onSlowTick(LivingEntity entity, ChestCavityInstance cc, ItemStack organ) {
    ...
    ensureChannel(context, BONE_GROWTH_CHANNEL)
        .addPolicy(NON_NEGATIVE)
        .adjust(BONE_GROWTH_PER_SECOND);
  ...
  public void onEquip(
    ...
    if (!registration.alreadyRegistered() && ChestCavity.LOGGER.isDebugEnabled()) {
      ChestCavity.LOGGER.debug(
          "[compat/guzhenren][le_gu_dun_gu] registered removal hook for {}", describeStack(organ));
    }
  }
  ...
  public void onRemoved(LivingEntity entity, ChestCavityInstance cc, ItemStack organ) {
    ...
    ledger.verifyAndRebuildIfNeeded();
    net.tigereye.chestcavity.compat.guzhenren.util.behavior.OrganStateOps.setIntSync(
        cc, organ, STATE_ROOT, BU_QU_KEY, 0, value -> 0, 0);
  }
  ...
  public void rebuildIncreaseEffects(
    ...
    int effectiveStacks =
        active ? Math.min(MAX_EFFECTIVE_STACKS, Math.max(1, organ.getCount())) : 0;
  ...
  private void applyBoneGuardInvulnerability(ServerPlayer player) {
    ...
    player.addEffect(
        new MobEffectInstance(
            MobEffects.DAMAGE_RESISTANCE, INVULN_DURATION_TICKS, 4, false, true, true));
    // 自身短时骨系免疫，提示一次
    ReactionTagOps.add(player, ReactionTagKeys.BONE_IMMUNE, INVULN_DURATION_TICKS + 20);
    if (!player.level().isClientSide()) {
      player.sendSystemMessage(
          Component.translatable("message.chestcavity.gu.rib_shield.activated"));
    }
  }
  ...
  private void refreshIncreaseContribution(
      ChestCavityInstance cc, ItemStack organ, boolean active) {
    if (cc == null || organ == null || organ.isEmpty()) {
      return;
    }
    ActiveLinkageContext context = LinkageManager.getContext(cc);
    ...
    double target =
        active
            ? Math.min(MAX_EFFECTIVE_STACKS, Math.max(1, organ.getCount()))
                * GU_DAO_INCREASE_PER_STACK
            : 0.0;
