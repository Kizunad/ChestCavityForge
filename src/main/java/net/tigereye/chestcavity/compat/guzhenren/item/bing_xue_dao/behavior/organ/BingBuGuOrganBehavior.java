...
/** Behavior implementation for 冰布蛊 (Bing Bu Gu). */
public final class BingBuGuOrganBehavior extends AbstractGuzhenrenOrganBehavior
  ...
  private void handlePlayer(
      Player player, CCConfig.GuzhenrenBingXueDaoConfig.BingBuGuConfig config) {
    ...
    if (hasSnowball && tryGrantSaturation(player, config)) {
      ...
      ReactionTagOps.add(
          player, ReactionTagKeys.FROST_IMMUNE, Math.max(40, config.effectRefreshThresholdTicks));
      ...
      if (player.level() instanceof ServerLevel server) {
        ...
        if (!had && !player.level().isClientSide()) {
          player.sendSystemMessage(
              Component.translatable("message.chestcavity.bingxue.bingbu_refresh.saturation"));
        }
      }
      return;
    }
    ...
    if (hasIce && tryGrantPlayerRegeneration(player, config)) {
      ...
      if (player.level() instanceof ServerLevel server) {
        ...
        if (!had && !player.level().isClientSide()) {
          player.sendSystemMessage(
              Component.translatable("message.chestcavity.bingxue.bingbu_refresh.regen"));
        }
      }
    }
  }
  ...
  private void handleNonPlayer(
    ...
    OrganStateOps.setInt(
        state, cc, organ, NON_PLAYER_COOLDOWN_KEY, nextCooldown, value -> value, -1);

    if (applied) {
      boolean hadImmune = ReactionTagOps.has(entity, ReactionTagKeys.FROST_IMMUNE);
      ReactionTagOps.add(
          entity, ReactionTagKeys.FROST_IMMUNE, Math.max(40, config.effectRefreshThresholdTicks));
  ...
  private boolean tryGrantNonPlayerRegeneration(
      LivingEntity entity, CCConfig.GuzhenrenBingXueDaoConfig.BingBuGuConfig config) {
    if (entity == null) {
      return false;
    }
