...
public final class XueqiguOrganBehavior extends AbstractGuzhenrenOrganBehavior
  ...
  private static final ResourceLocation ORGAN_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "xueqigu");
  ...
  private static final float HEAL_PER_SECOND =
      BehaviorConfigAccess.getFloat(XueqiguOrganBehavior.class, "HEAL_PER_SECOND", 3.0f);
  private static final float HEALTH_DRAIN_PER_MINUTE =
      BehaviorConfigAccess.getFloat(XueqiguOrganBehavior.class, "HEALTH_DRAIN_PER_MINUTE", 8.0f);
  private static final float HEALTH_DRAIN_RESERVE =
      BehaviorConfigAccess.getFloat(XueqiguOrganBehavior.class, "HEALTH_DRAIN_RESERVE", 1.0f);
  private static final int SLOW_TICKS_PER_MINUTE =
      BehaviorConfigAccess.getInt(XueqiguOrganBehavior.class, "SLOW_TICKS_PER_MINUTE", 60);
  private static final int FAILURE_RETRY_TICKS =
      BehaviorConfigAccess.getInt(XueqiguOrganBehavior.class, "FAILURE_RETRY_TICKS", 5);
    ...
    if (timer <= 0) {
      ...
      drained =
          GuzhenrenResourceCostHelper.drainHealth(
              entity, drainAmount, HEALTH_DRAIN_RESERVE, entity.damageSources().generic());
      timer = drained ? SLOW_TICKS_PER_MINUTE : FAILURE_RETRY_TICKS;
    }
    OrganStateOps.setIntSync(
        cc, organ, STATE_KEY, TIMER_KEY, timer, v -> Math.max(1, v), SLOW_TICKS_PER_MINUTE);

    // 维持血气流转标记，供反应系统加权判定（短时续期）
    net.tigereye.chestcavity.util.reaction.tag.ReactionTagOps.add(
        entity, net.tigereye.chestcavity.util.reaction.tag.ReactionTagKeys.BLOOD_FLOW, 60);
  }
  ...
  public void onRemoved(LivingEntity entity, ChestCavityInstance cc, ItemStack organ) {
    ...
    context.lookupChannel(XUE_DAO_INCREASE_EFFECT).ifPresent(channel -> channel.adjust(-removed));
  }
