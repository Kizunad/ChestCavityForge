...
/** Utility methods for applying intimidation-style减益。 调用方提供血量阈值和关系范围，可在 flow/能力里直接复用。 */
public final class IntimidationHelper {
  ...
  public static int intimidateNearby(Player performer, double radius, Settings settings) {
    ...
    List<LivingEntity> candidates =
        server.getEntitiesOfClass(
            LivingEntity.class,
            box,
            entity -> entity.isAlive() && (settings.includeSelf || entity != performer));
    int applied = 0;
  ...
  public static boolean applyIntimidation(
      Player performer, LivingEntity candidate, Settings settings) {
    ...
    if (!settings.includeSelf && candidate == performer) {
      LOGGER.info(
          "[Intimidation] skip {} (self) includeSelf={}",
          describe(candidate),
          settings.includeSelf);
      return false;
    }
    ...
    if (!candidate.hasEffect(effectHolder)) {
      LOGGER.info(
          "[Intimidation] effect inactive after addEffect for {} (effect={}) applied={}",
          describe(candidate),
          effectHolder.unwrapKey().map(Object::toString).orElse("<unknown>"),
          applied);
    ...
    if (candidate instanceof Mob mob && !candidate.level().isClientSide) {
      ...
      LOGGER.info(
          "[Intimidation] effect applied to {}, but not a Mob or client-side", describe(candidate));
    }
    return true;
  }
  ...
  private static String describe(LivingEntity entity) {
    return entity == null
        ? "<null>"
        : entity.getName().getString() + "(" + entity.getType().toString() + ")";
  }

  public static Holder<MobEffect> resolveEffect(
      ResourceLocation effectId, Holder<MobEffect> fallback) {
    Holder<MobEffect> defaultEffect =
        fallback != null ? fallback : CCStatusEffects.SOUL_BEAST_INTIMIDATED;
    if (effectId == null) {
      return defaultEffect;
    }
    Optional<? extends Holder<MobEffect>> resolved =
