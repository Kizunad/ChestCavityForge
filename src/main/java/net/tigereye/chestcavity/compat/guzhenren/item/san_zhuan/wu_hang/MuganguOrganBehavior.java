...
/**
 * Mugangu regenerates zhenyuan each slow tick, with optional jingli cost when the five-element set
 * is incomplete.
 */
public enum MuganguOrganBehavior
    implements OrganOnGroundListener, OrganSlowTickListener, IncreaseEffectContributor {
  INSTANCE;

  private static final String MOD_ID = "guzhenren";
  private static final Set<ResourceLocation> COMPANION_GU_IDS =
      Set.of(
  ...
  private static final ResourceLocation COMPLETION_CHANNEL_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "linkage/wuhang_completion");
  private static final ResourceLocation REGEN_RATE_CHANNEL_ID =
      ResourceLocation.fromNamespaceAndPath(MOD_ID, "linkage/mugangu_regen_rate");
    ...
    GuzhenrenResourceBridge.open(player)
        .ifPresent(
            handle -> {
              ...
              LinkageChannel completionChannel =
                  context
                      .lookupChannel(COMPLETION_CHANNEL_ID)
                      .orElseGet(
                          () ->
                              context
                                  .getOrCreateChannel(COMPLETION_CHANNEL_ID)
                                  .addPolicy(UNIT_CLAMP));
              LinkageChannel regenChannel =
                  context
                      .lookupChannel(REGEN_RATE_CHANNEL_ID)
                      .orElseGet(
                          () ->
                              context
                                  .getOrCreateChannel(REGEN_RATE_CHANNEL_ID)
                                  .addPolicy(UNIT_CLAMP)
              ...
              double completionRatio =
                  required == 0 ? 1.0 : Math.min(1.0, companionCount / (double) required);
              ...
              if (!hasCompanionOrgans) {
                ...
                success =
                    ResourceOps.tryReplenishScaledZhenyuan(handle, regenBudget, true).isPresent();
                if (!success) {
                  ResourceOps.tryAdjustJingli(handle, jingliCost, true);
                }
              } else {
  ...
  private static double computeZhenyuanGain(
      GuzhenrenResourceBridge.ResourceHandle handle, double baseAmount) {
    if (baseAmount <= 0.0) {
      return 0.0;
    }
