 ...
 * 拦截规划的结果
 * <p>
 * 此记录包含拦截点预测的所有必要信息，用于护幕飞剑执行拦截任务。
 *
 * <h3>拦截点 P*</h3>
 * 拦截点是预测的最佳拦截位置，通常位于威胁命中轨迹上，
 * 并可能提前一定距离（如 0.3m）以确保有效拦截。
 *
 * <h3>时间窗口</h3>
 * 拦截系统使用时间窗口判定是否可拦截：
 * <ul>
 *   <li>窗口范围：0.1 ~ 1.0 秒</li>
 *   <li>太近（< 0.1s）：反应不及</li>
 *   <li>太远（> 1.0s）：不值得拦截</li>
 * </ul>
 *
...
public record InterceptQuery(
     ...
     * 获取查询创建时的游戏时刻
     * <p>
     * 注意：这个方法返回的是 {@link IncomingThreat#worldTime()}，
     * 而不是当前时刻
     *
    ...
    public String describe() {
        return String.format("InterceptQuery[P*=(%.1f, %.1f, %.1f), t=%.2fs, %s]",
