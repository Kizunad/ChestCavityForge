...
/**
 * 统一的“器官进度/升级（FRP）”工具。 - 维护点数与阶数（clamp + 阈值触发） - 提供一次性 API，便于在命中/反应/环境钩子里直接加点并检查升级 -
...
public final class ProgressionOps {
   ...
   */
  public static boolean addPointsAndCheckTier(
    ...
    if (spForToast != null) {
      boolean consumed =
          tierUpHandler != null && tierUpHandler.onTierUp(spForToast, cc, organ, newTier);
      if (!consumed) {
        // 默认提示走 actionBar，不提供固定文案，交由调用方更进一步定制
        spForToast.displayClientMessage(
            net.minecraft.network.chat.Component.literal("器官升级至第" + newTier + "阶"), true);
      }
    }
    return true;
  }
