...
public enum ShuishenguOrganBehavior
    implements
        OrganOnGroundListener, OrganSlowTickListener, OrganIncomingDamageListener,
        IncreaseEffectContributor {
  INSTANCE;

  private static final double BASE_ZHENYUAN_COST = 80.0;
  private static final Component READY_MESSAGE =
      Component.translatable("message.chestcavity.shuishengu.ready");
  ...
  public void onSlowTick(LivingEntity entity, ChestCavityInstance cc, ItemStack organ) {
    ...
    int current =
        Math.min(NBTCharge.getCharge(organ, ShuishenguShield.STATE_KEY), effectiveMaxCharge);
  ...
  public float onIncomingDamage(
      ...
      ItemStack organ,
      float damage) {
    IncomingDamageShield.ShieldResult result =
        SHIELD.absorb(
            new IncomingDamageShield.ShieldContext(
                source, victim, cc, organ, damage, victim.level().getGameTime()));
  ...
  private static void applyChargingEffect(Player player) {
    ...
    if (existing == null || existing.getDuration() <= 30) {
      player.addEffect(
          new MobEffectInstance(MobEffects.CONDUIT_POWER, duration, amplifier, false, false, true));
  ...
  private static void spawnChargingParticles(LivingEntity entity, int stackCount) {
    ...
    for (int i = 0; i < 3 * stackCount; i++) {
      ...
      server.sendParticles(
          ParticleTypes.BUBBLE_COLUMN_UP, baseX, baseY, baseZ, 1, 0.0, 0.04, 0.0, 0.01);
      if (entity.getRandom().nextBoolean()) {
        server.sendParticles(
            ParticleTypes.BUBBLE_POP, baseX, baseY + 0.2, baseZ, 1, 0.0, 0.05, 0.0, 0.0);
      }
    }
  }
