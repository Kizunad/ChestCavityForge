...
import net.tigereye.chestcavity.compat.common.tuning.YuQunTuning;
...
public final class YuQunCalculator {
  ...
  public static void performVolley(
    ...
    double maxRange = upgraded ? YuQunTuning.RANGE + YuQunTuning.UPGRADE_RANGE_BONUS : YuQunTuning.RANGE;
    AABB searchBox = caster.getBoundingBox().inflate(maxRange, 2.0, maxRange);
    List<LivingEntity> candidates =
        level.getEntitiesOfClass(
            LivingEntity.class,
            searchBox,
            target -> target != caster && target.isAlive() && !target.isAlliedTo(caster) && !target.isInvulnerable());
    ...
    for (LivingEntity target : candidates) {
      ...
      double pushStrength = upgraded ? YuQunTuning.PUSH_STRENGTH_UPGRADE : YuQunTuning.PUSH_STRENGTH_BASE;
      double upward = YuQunTuning.PUSH_UPWARD_BASE + (upgraded ? YuQunTuning.PUSH_UPWARD_UPGRADE_BONUS : 0.0);
      target.push(direction.x * pushStrength, upward, direction.z * pushStrength);
      target.hurtMarked = true;
      target.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN,
          YuQunTuning.SLOW_TICKS, upgraded ? YuQunTuning.SLOW_AMP_UPGRADE : YuQunTuning.SLOW_AMP_BASE));
      bleedEffect.ifPresent(
          holder -> target.addEffect(new MobEffectInstance(holder,
