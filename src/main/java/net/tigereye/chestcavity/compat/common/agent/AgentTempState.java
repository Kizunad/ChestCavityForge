...
/**
 * 轻量运行期临时状态器（仅当前进程内存，不做持久化）：
 * - 管理“按 ID 安装的 transient 属性修饰器”的 TTL 与回滚
 * - NPC/玩家通用
 */
public final class AgentTempState {
  ...
  private static final Map<UUID, Map<ResourceLocation, Applied>> APPLIED = new ConcurrentHashMap<>();
  ...
  /**
   * 为任意生物应用一次 transient 属性修饰器，并在 TTL 到期时自动回滚。
   * 若相同 ID 已存在，会先移除旧的再添加新的。
   */
