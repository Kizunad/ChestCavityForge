...
/**
 * 从 SkillEffect 的一次性标记中提取增伤（如 one_hit_bonus / one_cast_bonus）。
 * 计算阶段消费标记，避免重复累计。
 */
public final class AttackerFlagBonusModifier implements SkillDamageModifier {

  @Override
  public double apply(DamageComputeContext ctx, double current, SkillDamageModifier.SkillDamageSink sink) {
    ...
    EffectContext ectx = EffectContext.build(sp, ctx.skillId(), ctx.attacker().chestCavity(), ctx.castId());
    double bonus = 0.0;
