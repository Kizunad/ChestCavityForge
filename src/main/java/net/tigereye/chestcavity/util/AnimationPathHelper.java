...
public final class AnimationPathHelper {
    ...
    private static final Map<ResourceLocation, Map<String, Map<String, List<AnimationKeyframe>>>> CACHE = new ConcurrentHashMap<>();

    public static List<AnimationKeyframe> loadPositionKeyframes(ResourceManager resourceManager,
                                                                ResourceLocation animationFile,
                                                                String animationName,
                                                                String boneName) {
        if (resourceManager == null || animationFile == null || animationName == null || boneName == null) {
            return List.of();
        }
        ...
        return bones.computeIfAbsent(boneName, key -> loadUncached(resourceManager, animationFile, animationName, boneName));
    }

    private static List<AnimationKeyframe> loadUncached(ResourceManager resourceManager,
                                                        ResourceLocation animationFile,
                                                        String animationName,
                                                        String boneName) {
        ...
        try {
            ...
            ChestCavity.LOGGER.warn("[AnimationPathHelper] Failed to access animation resource {}", animationFile, ex);
            return List.of();
        }
        if (resourceOpt.isEmpty()) {
            ChestCavity.LOGGER.warn("[AnimationPathHelper] Animation resource {} not found", animationFile);
            return List.of();
        }
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(resource.open(), StandardCharsets.UTF_8))) {
            ...
            if (!animations.has(animationName)) {
                ChestCavity.LOGGER.warn("[AnimationPathHelper] Animation {} missing entry {}", animationFile, animationName);
                return List.of();
            }
            ...
            if (!bones.has(boneName)) {
                ChestCavity.LOGGER.warn("[AnimationPathHelper] Animation {} bone {} not found in {}", animationFile, boneName, animationName);
                return List.of();
            }
            JsonObject bone = bones.getAsJsonObject(boneName);
            if (!bone.has("position")) {
                ChestCavity.LOGGER.warn("[AnimationPathHelper] Animation {} missing position data for bone {}", animationFile, boneName);
                return List.of();
            }
            ...
            for (Map.Entry<String, JsonElement> entry : position.entrySet()) {
                ...
                try {
                    ...
                    ChestCavity.LOGGER.warn("[AnimationPathHelper] Invalid keyframe time '{}' in {}", key, animationFile);
                    continue;
                }
            ...
            ChestCavity.LOGGER.warn("[AnimationPathHelper] Failed to read animation {}", animationFile, ex);
            return List.of();
        }
    }
