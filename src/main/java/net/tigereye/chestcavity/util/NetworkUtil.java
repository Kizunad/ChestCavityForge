...
public class NetworkUtil {
    ...
    /**
     * Synchronises a single inventory slot from the owner's chest cavity to the client. The stack reference must come
     * directly from the chest cavity inventory so the helper can resolve the slot index.
     */
    public static boolean sendOrganSlotUpdate(ChestCavityInstance cc, ItemStack stack) {
        ...
        NetworkHandler.sendOrganSlotUpdate(player, new ChestCavityOrganSlotUpdatePayload(slot, stack.copy()));
        return true;
    }
    ...
    private static int findSlot(ChestCavityInstance cc, ItemStack target) {
        ...
        for (int i = 0; i < cc.inventory.getContainerSize(); i++) {
            ItemStack slotStack = cc.inventory.getItem(i);
            if (ItemStack.isSameItemSameComponents(slotStack, target) && slotStack.getCount() == target.getCount()) {
                return i;
            }
        }
        return -1;
