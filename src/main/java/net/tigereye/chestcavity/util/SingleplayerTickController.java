...
/**
 * Provides a safe entry point for manipulating tick behaviour while running the game in
 * singleplayer (integrated server) mode.
 *
 * <p>The helper automatically checks whether a local integrated server is available before
 * attempting to touch tick state, allowing callers to invoke it from common code without worrying
 * about dedicated servers. Supported operations currently include:
 *
 * <ul>
 *   <li>Changing the global tick rate via the integrated server's {@code tickRateManager}.
 *   <li>Toggling the pause state so that all logic updates are frozen or resumed.
 * </ul>
 */
public final class SingleplayerTickController {
  ...
  /**
   * Applies the requested tick adjustments if a singleplayer integrated server is currently active.
   *
   * @param targetTickRate Optional tick rate override. {@code null} leaves the current tick rate
   *     untouched.
   * @param pauseState Optional pause flag. {@code null} leaves the pause state untouched.
   * @return {@code true} if a singleplayer server was found and at least one operation was carried
   *     out, otherwise {@code false}.
   */
  public static boolean apply(Float targetTickRate, Boolean pauseState) {
    return HANDLER.apply(targetTickRate, pauseState);
  }

  /** Convenience wrapper around {@link #apply(Float, Boolean)} that only adjusts the tick rate. */
  public static boolean setTickRate(float targetTickRate) {
    return apply(targetTickRate, null);
  }

  /**
   * Convenience wrapper around {@link #apply(Float, Boolean)} that only adjusts the pause state.
   */
  ...
  private static SingleplayerTickHandler initializeHandler() {
    ...
    try {
      Class<?> implClass =
          Class.forName("net.tigereye.chestcavity.util.SingleplayerTickController$ClientActions");
  ...
  private static final class ClientActions implements SingleplayerTickHandler {
    ...
    private static boolean setTickRateInternal(
        net.minecraft.client.server.IntegratedServer server, float targetTickRate) {
      ...
      if (sanitized != targetTickRate) {
        ChestCavity.LOGGER.debug(
            "Clamped tick rate {} to minimum supported value {}", targetTickRate, sanitized);
      }
    ...
    private static boolean setPauseInternal(
        net.minecraft.client.server.IntegratedServer server, boolean pauseState) {
