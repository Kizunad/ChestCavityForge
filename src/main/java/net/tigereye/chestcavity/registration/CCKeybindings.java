...
public class CCKeybindings {
  ...
  public static void register(RegisterKeyMappingsEvent event) {
    UTILITY_ABILITIES =
        register(event, UTILITY_ABILITIES_ID, ORGAN_ABILITY_KEY_CATEGORY, GLFW.GLFW_KEY_V, false);
    ATTACK_ABILITIES =
        register(event, ATTACK_ABILITIES_ID, ORGAN_ABILITY_KEY_CATEGORY, GLFW.GLFW_KEY_R, true);
    CREEPY =
        register(
            event,
            CCOrganScores.CREEPY,
            ORGAN_ABILITY_KEY_CATEGORY,
    ...
    SILK =
        register(
            event, CCOrganScores.SILK, ORGAN_ABILITY_KEY_CATEGORY, GLFW.GLFW_KEY_KP_DECIMAL, false);

    WOODEN_SHOVEL_TICK_RATE =
        registerStandalone(
            event,
            "key." + ChestCavity.MODID + ".wooden_shovel_tick_rate",
            ORGAN_ABILITY_KEY_CATEGORY,
            GLFW.GLFW_KEY_H);
    GUSCRIPT_OPEN =
        registerStandalone(
            event,
            "key." + ChestCavity.MODID + ".guscript_open",
            ORGAN_ABILITY_KEY_CATEGORY,
            GLFW.GLFW_KEY_G);
    GUSCRIPT_EXECUTE =
        registerStandalone(
            event,
            "key." + ChestCavity.MODID + ".guscript_execute",
            ORGAN_ABILITY_KEY_CATEGORY,
            GLFW.GLFW_KEY_B);
    GUSCRIPT_CANCEL =
        registerStandalone(
            event,
            "key." + ChestCavity.MODID + ".guscript_cancel",
            ORGAN_ABILITY_KEY_CATEGORY,
            GLFW.GLFW_KEY_C);
    WOODEN_SHOVEL_FREEZE =
        registerStandalone(
            event,
            "key." + ChestCavity.MODID + ".wooden_shovel_freeze",
            ORGAN_ABILITY_KEY_CATEGORY,
            GLFW.GLFW_KEY_J);
    WOODEN_SHOVEL_UNFREEZE =
        registerStandalone(
            event,
            "key." + ChestCavity.MODID + ".wooden_shovel_unfreeze",
            ORGAN_ABILITY_KEY_CATEGORY,
            GLFW.GLFW_KEY_K);
    MODERN_UI_CONFIG =
        registerStandalone(
            event,
            "key." + ChestCavity.MODID + ".modernui_config",
            ORGAN_ABILITY_KEY_CATEGORY,
            GLFW.GLFW_KEY_O);
  }
    ...
    KeyMapping keyMapping =
        new KeyMapping(
            "key."
                + id.getNamespace()
                + "."
                + id.getPath(), // The translation key of the keybinding's name
            InputConstants.Type
                .KEYSYM, // The type of the keybinding, KEYSYM for keyboard, MOUSE for mouse.
  ...
  private static KeyMapping registerStandalone(
      RegisterKeyMappingsEvent event, String translationKey, String category, int defaultKey) {
    KeyMapping keyMapping =
        new KeyMapping(
            translationKey,
